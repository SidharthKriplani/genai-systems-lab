...
```
13 tabs  ·  100+ modules  ·  140+ posts  ·  zero backend  ·  no login
```
...
Five curated RAG failure scenarios. Inference bottleneck diagnosis. Agent loop step-through. Eval design. MCP protocol. Context compaction. 100+ production systems modules. All interactive. All in your browser. No account, no backend, no cost.
...
| ⚙️ **Systems** | 16 | Evals, Eval Frameworks, Model Strategy, Cost/Latency, Fine-Tuning, India Scale, Prompt Caching, Model Router, Inference Optimizer, Context Compaction, Incident Room, Observability, A/B Testing, ML CI/CD, Debug Traces |
...
| 🔭 **Explore** | 8 | Embedding Space, Shadow A/B, Latency Planner, Tokenizer Explorer, Model Card Reader, Vector DB Comparison, Structured Outputs, Red Teaming Lab + LLM Comparison Matrix |
...
| 💡 **Ask** | — | Consultation space — keyword search over 140+ posts + all module descriptions. Conversational UI. LLM-upgrade-ready |
...
**Assessment Mode:** Timed exam (15/30/60 min). 57 questions across 8 skill categories. Scores hidden until end — final reveal shows per-category breakdown and "Strong in / Needs work" callout.
...
|---------|--------------|
| **⌘K search** | Jump to any of the 100+ modules instantly |
...
├── Systems.jsx             # 16 systems modules — evals through debug traces
...
├── Explore.jsx             # 8 exploration tools with fidelity tags + LLM matrix
...
├── GroundTruth.jsx         # 140+ posts — ELI5, search, reactions, quiz-me, ToC
...
├── groundTruthPosts.js     # All 140+ post content as typed block arrays
└── groundTruthIndex.js     # Search index built from post metadata
public/
├── og-image.png            # Open Graph image for link previews
├── og-failures.png         # OG image for RAG failure content
└── og-start.png            # OG image for /start landing page
```
