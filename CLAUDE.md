...
- **Two-stage retrieval (bi-encoder vs cross-encoder) shipped.** Highest-priority Tier 1 content gap from Microsoft RAG interview signal. 6-file change: (1) `src/groundTruthIndex.js` — new GT post entry `two-stage-retrieval-reranker` (category: rag, readMin: 9, labLink: systems, labModuleId: query-refinement, tags: RAG/bi-encoder/cross-encoder/reranker/two-stage retrieval). (2) `src/groundTruthPosts.js` — full post "Two-Stage Retrieval: Why a Reranker Exists" (12 blocks: bi-encoder/similarity-vs-relevance intro, callout, Stage 1 bi-encoder failure modes list, Stage 2 cross-encoder failure modes list, 6-row comparison table, two-stage pipeline explanation, critical failure mode callout, when-to-add-reranker list, when-one-stage-is-enough list, refs). (3) `src/systems/modules.jsx` — `TWO_STAGE_QUERIES` constant added (3 pre-baked scenarios with bi-encoder ranking + cross-encoder reranking + insight per scenario). `QueryRefinementLab` updated: `tsQuery` state added, "Two-Stage" 4th tab added to selector, `tsScenario` computed variable added. Tab content: framing block, query selector (3 queries), side-by-side Stage 1 vs Stage 2 comparison with rank-change arrows (↑N / ↓N), amber insight callout, bi-encoder vs cross-encoder failure mode cards, "when to add a reranker" decision list. (4) `src/Systems.jsx` — `"query-refinement"` added to `RELATED_GT`. (5) `src/data/preplabQuestions.js` — 4 new RAG questions: `reranker-1` (hard text, gated: medical Q&A precision redesign with failure modes, trap: "increase top-K"), `reranker-2` (hard text, gated: lexical gap failure mode, trap: "cross-encoder hallucinating"), `reranker-3` (medium MCQ, free: bi-encoder primary limitation, trap: "cannot handle long documents"), `reranker-4` (medium text, gated: recall_k=100 but doc at position 120, trap: "increase rerank_k"). All 4 have trap fields, readMore → two-stage-retrieval-reranker. Brace diffs: 0 all files. Commit: `74160e7`. Scale: 56 Systems modules (unchanged), 225 GT posts (was 224), 277 PrepLab questions (was 269 + 4 two-stage + 4 company-tracks improvement context preserved).
- **CompanyTracks fully revamped.** `src/PrepLab.jsx` CompanyPrepMode rebuilt. (1) Bug fixed: `q.answer.toLowerCase()` → `correctOpt = q.options[q.correct]` (MCQ) — scoring was completely broken, always evaluating as incorrect. (2) Hooks order fixed: `useState` for `score`/`done`/`sessionQs` moved before early `if (gated) return` (React hooks rules violation). (3) Text question handling added: textarea input + "Show Answer" button → model answer + keyword chips + trap callout + self-grade "Got it" / "Missed it" buttons (same pattern as TrainerMode). (4) Topic weight visualization added to overview card: sorted bar chart showing relative topic emphasis per archetype (critical/high/medium labels). (5) GT recommendations (`gtRecs`) added to all 4 archetypes + rendered in overview and done card as violet "Read next" links navigating to GT post. (6) Better done card: score with color-coded pct (emerald/amber/red), GT recs block. (7) `TOPIC_WEIGHT_LABELS` constant added. Brace diffs: 0. Included in commit `74160e7`.

**Resolved this session (sprint 34 — LangGraph + HITL):**
- **LangGraph + HITL shipped.** Second Tier 1 content gap from the Senior AI Engineer interview Round 2 signal. 5-file change: (1) `src/groundTruthIndex.js` — new GT post entry `langgraph-reducers-hitl` (category: agents, readMin: 10, labLink: systems, labModuleId: langgraph). (2) `src/groundTruthPosts.js` — full post "LangGraph Reducers and HITL: State Machines for Agentic Workflows" (12 blocks: implicit loop failure modes, state schema + reducer explainer, operator.add/overwrite/custom reducer list, HITL interrupt pattern list, interrupt_before/after/Command(resume)/checkpointer required list, HITL bug callout, conditional edges table, when graph beats custom loop list, when custom loop wins, refs). (3) `src/systems/modules.jsx` — `LangGraphModule` component added (~260 lines): 4-tab interactive module. Tab "Reducers": side-by-side operator.add vs overwrite cards with "Simulate node update" button — left panel shows accumulating list, right panel shows overwrite scalar; common trap callout (parallel nodes + overwrite = silent data loss). Tab "StateGraph": interactive SVG StateGraph (5 rectangular nodes: START/agent/tools/human_review/END; conditional edges; click-to-explore node detail panel). Tab "HITL Flow": animated 6-step interrupt/resume flow with step dots, Play/Next/Prev/Reset controls, 1.8s auto-advance, inline code blocks at interrupt and resume steps. Tab "When to Use": 6-row comparison table (LangGraph vs custom loop: durable state, parallelism, HITL, linear agents, conditional routing, observability) + HITL production bug callout (missing checkpointer). Also fixed `GraphRAGModule` SVG mobile text sizes: layer labels 8→10, edge labels 7→9, node labels 9→11. Added `LangGraphModule` to exports. (4) `src/Systems.jsx` — `LangGraphModule` added to import + SYSTEMS_MODULES (BUILD group, tag AGENTS) + RELATED_GT. (5) `src/data/preplabQuestions.js` — 4 new agents questions: `langgraph-1` (hard: HITL document review agent — StateGraph vs custom loop), `langgraph-2` (hard: parallel node reducer bug — operator.add fix), `langgraph-3` (medium MCQ: when interrupt_before pauses execution), `langgraph-4` (medium text: HITL production KeyError — missing checkpointer). All 4 have `trap` fields; langgraph-1, -2, -4 have `source` fields. Brace diffs: 0 all files. Commit: `cfd4520`. Scale: 56 Systems modules (was 55), 224 GT posts (was 223), 269 PrepLab questions (was 265).

**Resolved this session (sprint 33 — Graph RAG):**
- **Graph RAG shipped.** First Tier 1 content gap from the Senior AI Engineer interview signal (May 2026). 5-file change: (1) `src/groundTruthIndex.js` — new GT post entry `graph-rag-multi-hop` (category: rag, readMin: 10, labLink: systems, labModuleId: graph-rag). (2) `src/groundTruthPosts.js` — full post "Graph RAG: When Vector Search Isn't Enough" (9 blocks: problem statement, multi-hop failure mode with callout, knowledge graph explainer, entity extraction pipeline list, multi-hop traversal table, hybrid Graph+Vector architecture, production failure modes list, production cost insight callout, when-to-use decision table, refs). (3) `src/systems/modules.jsx` — `GraphRAGModule` component added (~210 lines): 4-tab interactive module. Tab "The Failure": side-by-side vector RAG fail vs Graph RAG success on the query "Which investors backed companies that use RAG?", with chunk comparison and amber explanation. Tab "Knowledge Graph": interactive SVG knowledge graph (7 nodes: Sequoia/a16z investors, Anthropic/OpenAI companies, RAG/Fine-tune tech, Pinecone provider; 8 typed directed edges; cyan glow on active nodes; click-to-explore edge list). Tab "Multi-Hop": animated 6-step traversal (step dots + Play/Next/Pause/Reset controls; 1.5s auto-advance; SVG graph highlights active nodes/edges per step). Tab "When to Use": 6-row decision table (vector vs graph per query type) + production cost callout + hybrid pipeline chip diagram. Added to exports. (4) `src/Systems.jsx` — `GraphRAGModule` added to destructured import; `{ id: "graph-rag", label: "Graph RAG", tag: "RETRIEVAL", group: "BUILD" }` entry added to `SYSTEMS_MODULES`; `"graph-rag"` entry added to `RELATED_GT`. (5) `src/data/preplabQuestions.js` — 4 new RAG questions: `graph-rag-1` (hard, text: multi-hop compliance architecture problem with trap), `graph-rag-2` (hard, text: full pipeline construction + 3 failure modes with trap), `graph-rag-3` (medium, MCQ: what is multi-hop retrieval + why vector fails), `graph-rag-4` (medium, text: hybrid routing logic + tradeoffs with trap). All 4 have `trap` fields. Brace diffs: 0 all files. Commit: `2a00754`. Scale: 55 Systems modules (was 54), 223 GT posts (was 222), 265 PrepLab questions (was 261).

**Resolved this session (sprint 32 — Cold-start Home rewrite):**
- **Cold-start belief gap fix shipped.** `src/Home.jsx` new-visitor section updated in 3 passes. (1) Market signal chip added above headline: "Agentic AI engineer roles: +280% YoY · 90K+ open roles (Stanford HAI 2026)" — animated pulse dot, cyan tint, positions belief before mechanic. (2) Subtext rewritten: "Agentic AI engineer roles grew 280% last year. The gap isn't building AI — it's diagnosing production failures under pressure. This lab puts you in the seat." — bridges market signal → product thesis per DECISIONS.md Section 9 and UPGRADES.md Hero Copy spec. (3) PrepLab question card added as primary cold-visitor CTA: "Senior AI engineer interviews" label, hard RAG question ("Why does RAG retrieve the right chunk but still return the wrong answer?"), green "Test your interview readiness →" button linking to PrepLab. Placed above HeroFailureDemo. (4) Door card copy updated: BUILD "I'm building with LLMs" → "Ship AI that holds up in production"; PROVE "Interview prep" → "Interview ready in hours, not weeks" + updated body; NAVIGATE "Career clarity" → "AI career, mapped" + updated body. Returning user view (`ReturningHomeView`) untouched. Brace diff: 0. Commit: `d8c2d11`.

**Resolved this session (sprint 31E — Interview Strategy full rebuild):**
- **Sprint E shipped.** `InterviewPrepMode` fully rebuilt as a 4-step Interview Brief flow. (1) Step 1: JD paste + optional company name input. (2) Step 2: Role type selector (4 cards: AI Engineer / ML Engineer / AI PM / Research Scientist) + round selector (1/2/3+/Final) + interviewer type (Tech Lead/Hiring Manager/Peer) + prior round feedback textarea (shown when round > 1). (3) Step 3: Self-rate detected topics (Weak/Okay/Strong) — preserves exact UI from old step 2. (4) Step 4: "Interview Brief" output gated behind `isAccessGranted()`. Brief card shows: company+role+round+date chips header, top 3 gap areas (each with Hard Q, Medium Q, trap callout amber block, GT post link), prior feedback block (red tint, only if round > 1), day-of checklist (5 checkboxes), Copy Brief button (generates markdown → clipboard), Exit. Helper functions: `generateBrief()` (ranks topics by gapScore, finds hardQ with trap + medQ, resolves GT post + lab forward pointer per topic), `buildBrief()` (calls generateBrief, saves to state, advances to step 4, writes `gsl-preplab-strategy-phase = "4"` to localStorage), `copyBrief()` (builds markdown brief string, copies via clipboard API, 2.5s "Copied!" feedback state). Removed: inline drill flow, `buildDrillQs()`, `startDrill()`, `submit()`, `selfGrade()`, `next()`, old results screen. Violet accent throughout (was indigo). Section comment updated to "MODE 3: INTERVIEW STRATEGY". Brace diff: 0. Commit: `a5af787`.

**Resolved this session (sprint 31E-pre — TrainerMode Browse/List View):**
- **Browse/List View shipped.** `TrainerMode` gains a `viewMode` state (`"drill"` | `"browse"`) and `expandedId` accordion state. (1) Pill toggle added to TrainerMode header: "Drill" | "Browse". Counter adapts: `N / total` in drill, `Nq` in browse. PBar hidden in browse mode. (2) Browse mode: scrollable list of all filtered questions, each as a row with left diff-color accent bar (red/amber/blue at 40% opacity), question text (2-line truncated, expands on click), topic chip + difficulty badge + chevron. (3) Accordion expand: MCQ shows all options with correct highlighted green+checkmark; open questions show keyword chips; both show explanation, `CommonTrapCallout` (if `q.trap`), source line (if `q.source`). (4) "Drill this topic →" chip in expanded row switches to drill mode with that topic group pre-filtered. (5) `expandedId` resets on filter changes. Brace diff: 0. Commit: `eb84135`.

**Resolved this session (sprint 31D — PrepLab Sprint D):**
- **Sprint D shipped.** Assess results screen fully rebuilt. (1) `src/config/gating.js` created: `RESULTS_FREE_LIMIT = 10`, re-exports `FREE_QUESTION_LIMIT` from `utils/accessCode.js` for backward compat. Imported in `PrepLab.jsx`. (2) `EXAM_SESSIONS_KEY = "gsl-preplab-exam-sessions"` added — `useEffect([finished])` saves MCQ-based snapshot on exam completion (pct, tc, total, count, date), capped at 20 entries. (3) `TOPIC_FORWARD_POINTERS` constant: 11 topics → `{ label, tab }` gap pointer chips. (4) `ExamMode` signature updated: `{ onExit, onNavigate, onNavigateTo }`. Root PrepLab call updated: `<ExamMode onExit={exitMode} onNavigate={onNavigate} onNavigateTo={onNavigateTo} />`. (5) `if (finished)` block replaced: score headline (`{pct}%` + `{correct}/{total}` + Strong/Developing/Needs Work badge + session delta "+N% vs last" in green/red); per-topic bars sorted worst-first with per-bar gap pointer chips (shown when `pct < 60%`); free-user gate for >10q exams — shows per-topic data from first 10q only, blur+overlay on blurred placeholder rows with GateModal CTA; open answers self-assess unchanged; wrong answers preserved. Brace diff: 0 all files. Commit: `22cd963`.
- **Difficulty reclassification shipped** (same session). Original distribution 1E/37M/62H was miscalibrated. Reclassified all 261 questions via Python heuristics + 35 explicit hard overrides → 47E/126M/87H (18%/48%/33%). Also fixed 4 duplicate IDs from Sprint C inject (rag-1→rag-reranker, rag-2→rag-hyde, rag-3→rag-litm, rag-4→rag-parent-child). Brace diff: 0. Commit separate.
- **PrepLab landing layout shipped** (same session). Removed `max-w-2xl` constraint causing blank space. Added full-width PAL-style layout: 3-column mode card grid + Browse by Topic tile grid (5 TOPIC_GROUPS + All Questions tile). Clicking a topic tile opens TrainerMode pre-filtered via `trainerInitGroup` state. `openTrainer(groupId)` helper added.

**Resolved this session (sprint 31B+C — PrepLab Sprints B + C):**
- **Sprint B shipped.** `PREP_QUESTIONS` array extracted from `src/PrepLab.jsx` to `src/data/preplabQuestions.js`. Rule 1 compliance (DECISIONS.md Section 8). All 261 questions already had `difficulty` field — Sprint B became a pure structural extraction, no content work needed. PrepLab.jsx reduced from ~5079 lines to 2446 lines. Import added: `import { PREP_QUESTIONS } from "./data/preplabQuestions"`. Brace diff: 0 all files. Commit: `73924a0`.
- **Sprint C shipped.** `trap` + `source` fields injected into 50 hardest questions across 5 topics (RAG 12, Agents 12, Evaluation 11, LLMOps 11, Fine-tuning 5). Each `trap` field answers "what weaker candidates say" — the highest-value piece of information per the LLM/GenAI Interview Master Guide analysis. `src/shared.jsx` created (new file, DECISIONS.md Section 8 Rule 3 — repeated UI patterns as shared components) with named export `CommonTrapCallout({ trap })`: amber chip `bg-amber-950/40 border border-amber-800/50 text-amber-300`. `RevealCard` in `PrepLab.jsx` updated: both text-question and MCQ branches now render `{q.source && <p ...>Source: {q.source}</p>}` + `{q.trap && <CommonTrapCallout trap={q.trap} />}` after the explanation block, before readMore link. Brace diff: 0 all files. Commit: `38d5330`.

**Resolved this session (sprint 31A — PrepLab revamp Sprint A + IDEAS.md idea logging):**
- **Sprint A shipped.** PrepLab naming + visual layer — `src/PrepLab.jsx` only, no logic changes, no new files. (1) `PREPLAB_SIDEBAR` reduced from 6 entries to 3: "Assess" (EXAM), "Interview Strategy" (STRATEGY), "Company Tracks" (ARCHETYPE). Defense Doc and Weakness Map components kept but hidden from sidebar. (2) Sidebar score badges: `sidebarStats` state sourced from `gsl-preplab-history` localStorage — shows "N answered · X% correct" under Assess for returning users, "In progress: Phase N" under Interview Strategy. (3) `QuestionCard` visual upgrade: `border-l-4` left accent (`border-left: 4px`) keyed by difficulty (red = hard, amber = medium, blue = easy); difficulty chip updated to full-pill `bg-red-950/50 text-red-400 border border-red-800/40` pattern; right/top/bottom remain 1px zinc. (4) `MCQOptions` hover upgrade: `hoveredIdx` state added; hover style `rgba(139,92,246,0.08)` + `rgba(139,92,246,0.4)` border; selected state moved from indigo to violet (matching GAL palette); letter badge updates on hover/select. (5) `ExamConfig` copy: headline → "How interview-ready are you?", subtext added, duration labels show question counts (10/20/40q), CTA → "Start Assessment →", all selectors → violet active state, results preview callout block added above CTA. (6) `TrainerMode` topic selector: `TOPIC_GROUPS` constant added (5 groups: RAG & Retrieval, Agents & Systems, Evals & Metrics, LLM & Fine-Tuning, Production & Ops); `topicFilter` → `groupFilter` state; filter logic updated to check group's `topics[]` array; 22 topic pills → 5 group tiles with count badges, hover lift, multi-select (click active tile deselects). Brace diff: 0. Commit: `43e4a92`. Scale: unchanged.
- **IDEAS.md updated:** Tab keyboard shortcuts cluster added to Tier 2 (S effort, single-key navigation R/A/E/P/G/C + ?). Visual product map cluster added to Tier 3 (M effort, revisit condition documented, correct split between navigation value and discoverability value). Commit: `f6d2c7c`.
- **PREPLAB_SPEC.md + NEXT.md:** Sprint A spec committed (`f19228a`). NEXT.md rewritten with Sprint A as committed work, all prior items moved to Pending. Old items 1–4 (interview story, production notes, static corpus, GT stubs) still in Pending.

**Resolved this session (post sprint 30 — field intelligence + architecture + MD sync + cold-start analysis):**
- **No code shipped.** Extended planning + field intelligence session. Work in three phases: (A) Field intelligence: 4 LinkedIn/practitioner posts + 1 interview guide PDF assessed and logged. All MD files audited and synced to sprint 30. Commits `0681ee4`, `43cfb08`, `2e9d428`, `fad8a6c`. (B) Architecture: modularisation and config discipline decision formalised — Rule 1 (new data in data files), Rule 2 (system-wide settings in `src/config/`), Rule 3 (repeated UI patterns as shared components). Logged in DECISIONS.md Section 8. Three UPGRADES.md entries added: `src/config/` folder, PrepLab question extraction, shared UI components. Commit `39c7b86`. (C) Cold-start belief gap: fundamental positioning + onboarding analysis — GAL has a belief gap for new visitors that content and UX fixes cannot solve. Root cause identified, standing rule logged. Commit: (this session). Scale: unchanged.
- **Field intelligence logged (5 sources):** (1) Microsoft RAG interview → bi-encoder/cross-encoder Tier 1 cluster. (2) Qwen3.6 local inference → agentic latency Tier 2 cluster. (3) Gumroad/Etsy builder → early revenue experiment (decision-gated). (4) Senior AI Engineer interview → Graph RAG (Tier 1) + LangGraph/HITL (Tier 1) + NEXT.md Pending. (5) LLM/GenAI Interview Master Guide PDF (136q, 10 categories, 4-layer format) → Common Trap layer added to UPGRADES.md (High priority); scaling laws + semantic caching + dense/sparse retrieval + LoRA/QLoRA added to IDEAS.md Tier 1; fintech/lending elevated from skip to Tier 2 (PrepLab Company Track). Screenshot ("3 businesses to $100K") — noise, one positioning note only, not logged.
- **Cold-start belief gap analysis (Phase C):** Identified that GAL is doing two jobs — building career-relevance belief (cold visitors) and serving practice (believers) — but front-loads the second job while the first is unfinished. Root comparison: PAL's SQL Lab gives visitors instant career-relevant engagement using pre-installed SQL belief; GAL's "configure a RAG Lab" requires the visitor to first understand why that matters for their career before they engage. This is a funnel problem, not a content or UX problem. Key decisions: (1) PrepLab is the correct cold-start entry (connects to pre-installed "I need to pass interviews" belief); correct funnel is PrepLab → Lab, not Lab → PrepLab. (2) Home must surface market signal (agentic AI +280% YoY, 90K+ postings) before describing the product mechanic. (3) "Daily Diagnostic" / interview preview format only works with "this appeared in a real interview at [company]" framing — without it, it reads as a quiz, not interview prep. (4) Do not build more Lab content before fixing the cold-start funnel. Logged in: DECISIONS.md Section 9 (cold-start belief gap standing rule), UPGRADES.md Hero Copy Rewrite entry (root cause + correct copy sequence), IDEAS.md new cluster (cold-start belief gap — 4 buildable items including market signal chip, PrepLab CTA, interview framing, Daily Diagnostic). LINEAGE.md post-sprint-30 row updated.

**Resolved this session (sprint 30):**
- **4 items from NEXT.md completed.** (1) RAG Lab done card prominence fix: forward pointer (✓ "You've seen the failure. What's next?") moved from bottom-of-stack to immediately after the Failure Mode block in `App.jsx` — now visible without scrolling. Pure JSX reorder, no logic changes. (2) Concepts Gym inline progress: `GymRoomView` in `Concepts.jsx` now shows a progress bar (N/total completed, %) and a "Continue: [next module] →" CTA detecting the first incomplete module; shows "All modules done" confirmation when gym is complete. (3) PrepLab keyboard shortcuts: `useEffect` keydown handlers added to both `ExamMode` and `TrainerMode` in `PrepLab.jsx` — keys 1/2/3/4 select MCQ options, Enter submits when option selected / advances when already submitted. (4) EvalLoopModule built and wired: new `EvalLoopModule` component in `Concepts.jsx` (3 tabs: "The 3 Eval Questions" with expandable loop steps, "RAGAS Metrics" with clickable 4-metric cards, "Debug an Eval Run" with 3 broken eval scenarios and root-cause diagnosis); added to MODULES array (id: "eval-loop"), MODULE_META, MODULE_NEXT_STEP (→ Eval Lab), and retrieval gym moduleIds (now 6 modules). Brace diffs: 0 all files. Commit: `ada9b79`. Scale: 16 Concepts modules (was 15), Retrieval gym 5 → 6 modules.

**Resolved this session (sprint 29):**
- **No code shipped.** Architecture + planning session. Evaluated datamart-backed realism concept for GAL — whether static document corpora and/or in-browser execution environments (Pyodide, Marimo) should be used to add realism to lab scenarios. Context: cross-product discussion comparing StrataScratch-style SQL practice, PAL-style datamarts, and MSL-style Pyodide notebooks. GAL-specific conclusion: static JSON corpus for RAG Lab + Eval Lab is Tier 1 (no execution, renders real retrieved chunks + eval triples, zero WASM cost); Pyodide fixed-notebook eval is Tier 2 (conditional on static data shipping + engagement signal); Marimo is the wrong in-browser tool (standalone notebook breaks the lab frame) but valid as downloadable companion content on done cards. Scope constraint enforced: Agent Lab and LLM Lab excluded — their failure modes are behavioral and distributional, not corpus-dependent. MD files updated: IDEAS.md (new "Datamart-backed realism" Tier 1 cluster + new Tier 2 items), DECISIONS.md (Section 7 — data realism + execution architecture ruling), LINEAGE.md (sprint 29 row), NEXT.md (static corpus item added to Pending). Scale: unchanged.
