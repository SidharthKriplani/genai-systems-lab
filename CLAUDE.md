...
**Sprint 55 (June 2026) — Bug audit + nav fixes:**
- Bug fix `5b653a9`: (1) GT posts not opening — `useState(false)` was inside `case "code"` of the `Block` switch statement, violating Rules of Hooks. When block positions shift via key={i} and filtering (simpleMode), React detects hook count mismatch and throws. Fixed by extracting `CodeBlock` component with `useState` at top level. (2) Sidebar chevron button nested inside nav item button — invalid HTML nesting caused unreliable click handling. Restructured: chevron is now a sibling `<button>` with `position: absolute`, not a child of the nav button. (3) Nav subitems ("Concepts", "Posts", "Practice Qs") opened full unfiltered tabs instead of the filtered hub. Simplified subitems to lab-only entries — the hub page (clicked via area label) already shows filtered content.
