...
- **Theme audit phase 2.** `src/PromptLab.jsx`, `src/Home.jsx`, `src/Concepts.jsx`, `src/systems/modules.jsx`, `src/App.jsx`: all hardcoded `#22D3EE` and `rgba(34,211,238,...)` converted to `var(--gal-build)`, `var(--gal-build-tint)`, `var(--gal-build-border)`, `var(--gal-build-tint-md)`, `var(--gal-build-tint-str)`. 6 gradient endpoints at 0.02–0.04 opacity remain (cosmetic, Phase 3). New `--gal-build-dark`, `--gal-build-tint-md`, `--gal-build-tint-str` CSS vars added to index.css. Commit `c859fe6`.
- **Interview Signal mode in PrepLab.** `INTERVIEW_EXPERIENCES` constant (22 entries: FAANG/unicorn/enterprise/startup, roles, rounds, topics, notes). `InterviewIntelMode` component: topic frequency bar chart, tier + role filter pills, experience cards, Tally submission link. Added as 4th sidebar mode (`intexp` / "Interview Signal" / tag: INTEL). Commit `f33a123`.
- **Scenario-type questions in PrepLab.** `ScenarioPlayer` component: incident → 3 investigation steps → each step has 4 choices + reveals → root cause + common trap on completion. 3 scenarios: `scenario-1` (RAG corpus failure), `scenario-2` (agent infinite loop), `scenario-3` (eval rubric lies). Data in `preplabQuestions.js`, player in `PrepLab.jsx` drill view. Commit `246f73f`.
- **Scale after sprint 42 (final):** 6 labs, 304 PrepLab questions (301 + 3 scenario), 27 Concepts, 226 GT posts, 57 Systems. Paper warm theme (light + dark). 4 PrepLab modes. Sun/moon toggle in desktop header.
