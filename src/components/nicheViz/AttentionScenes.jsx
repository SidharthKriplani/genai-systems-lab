  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 space-y-3">
      ...
      <svg viewBox="0 0 560 280" className="w-full">
        <circle cx={QX} cy={QY} r="16" fill="#18181b" stroke="#22d3ee" strokeWidth="2" />
        <text x={QX} y={QY + 4} textAnchor="middle" fill="#67e8f9" fontSize="11" fontFamily="monospace">agreed</text>
        <text x={QX} y={QY + 34} textAnchor="middle" fill="#52525b" fontSize="9">the query — searching</text>

        {CANDIDATES.map((c, i) => {
          const w = mode.weights[c.key];
          const cy = CYS[i];
          return (
            <line key={"e" + c.key} x1={QX + 15} y1={QY} x2={CX - 15} y2={cy}
              stroke="#8b5cf6" strokeWidth={Math.max(1, w * 34)} opacity={Math.max(0.15, w)}
              style={{ transition: "stroke-width .5s, opacity .5s" }} />
          );
        })}
        ...
        {CANDIDATES.map((c, i) => {
          ...
          return (
            <g key={c.key}>
              <circle cx={CX} cy={cy} r="14" fill="#18181b" stroke="#a78bfa" strokeWidth="1.5" />
              <text x={CX} y={cy + 3} textAnchor="middle" fill="#c4b5fd" fontSize="10" fontFamily="monospace">{c.label}</text>
              <text x={CX + 30} y={cy + 4} textAnchor="start" fill="#e4e4e7" fontSize="12" fontFamily="monospace" fontWeight="600">
                {fmtPct(w)}
              </text>
            </g>
          );
