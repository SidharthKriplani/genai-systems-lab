...
export default function CareerApp() {
  ...
  const CAREER_GROUPS = [
    ...
    { label: "NEGOTIATE",  ids: ["negotiation", "negotiate"] },
    { label: "KNOWLEDGE",  ids: ["benchmarks", "salary"] },
  ];
            ...
            {group.ids.map(id => {
              ...
              return (
                <button key={id} onClick={() => setActiveModule(id)}
                  className={`w-full text-left px-4 py-2 text-xs transition-all flex items-center justify-between gap-2 ${active ? "border-l-2 border-amber-500 bg-zinc-800 text-white" : "border-l-2 border-transparent text-zinc-400 hover:text-white hover:bg-zinc-900"}`}>
                  <span className="truncate">{m.label}</span>
                  <span className={`shrink-0 text-[9px] px-1.5 py-0.5 rounded font-mono ${active ? "bg-amber-900/60 text-amber-300" : "bg-zinc-800 text-zinc-600"}`}>{m.tag}</span>
                </button>
              );
            })}
          </div>
