...
re-grow from a corpus far too small to grow them.` },
  ...
  "hallucination": {
    ...
    explanation: [
      ...
      "**Three hallucination types**, and each one is a variant of the reader we just met:\n\n- **Closed-domain** — the reader had the real page in hand, glanced past the right paragraph, and answered from memory instead of what was actually written. The right document *was* retrieved but the model ignored it and answered from parametric memory. Fixable with faithfulness prompting and output validation.\n- **Confabulation** — the torn-page case exactly: the source exists, but a specific detail (a date, a number, a name) falls right where the signal ran thin, and the model fills it in, in the same confident voice as everything real around it. ==Most dangerous, because it sounds authoritative and passes casual review.==\n- **Open-domain** — the never-opened-book case: asked about something outside both training and retrieved context, the model still answers, just as confidently as if the source existed. Fixable with abstention training.\n\nThe pattern to watch for is confabulation: a specific invented detail — a date, a number, a requirement — slotted in exactly where the retrieved signal ran thin.",
