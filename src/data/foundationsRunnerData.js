...
task never touches — the definition of wasted spend.` },
  ...
  "rlhf": {
    ...
    explanation: [
      "The base pre-training objective — next-token prediction — gave a model that can **fluently continue any text.** What it didn't give: any preference for being helpful over harmful, accurate over inaccurate, or honest over deceptive.\n\nPre-training rewarded **statistical accuracy at predicting human text** — and human text contains harmful content, manipulation, and false claims, so the model learned to produce all of these as statistically appropriate completions. ==A base GPT-3 prompted to \"help my users\" might complete the text in any direction the training distribution supports.==",
      ...
      "The reason base GPT-3 couldn't be deployed and InstructGPT could: **this weight-level shift.** InstructGPT was *smaller* than GPT-3 by parameter count but outperformed it on human preference rankings — ==because alignment redirected capabilities, not just guided them.==\n\nSafe behavior across the typical input distribution; still exploitable under adversarial conditions. That remaining gap is why subsequent techniques (**Constitutional AI, DPO**) were developed.",
    ],
    keyPoints: [
...
at the same total compute — at 10× cheaper inference cost.` },
  ...
  "lora": {
    ...
    explanation: [
      "Ten domain-specific models needed from a 70B base. Full fine-tuning produces **10 independent checkpoints**: 10 × 140GB = **1.4TB of storage**, 10 separate inference stacks, 10 deployment pipelines to maintain.\n\n==The problem isn't primarily storage — it's operational.== Each domain requires its own serving environment, and at 20 or 50 domains the infrastructure multiplies proportionally. The underlying question: **does domain adaptation genuinely require updating all 70 billion parameters**, or do the relevant changes operate in a much smaller subspace of the weight space?",
      "LoRA's answer: ==domain adaptation shifts model behavior in a few important directions, not across all dimensions of a weight matrix simultaneously.==\n\nThe mathematical formalization: instead of updating the full m×n weight matrix directly, **decompose the update as `ΔW = A × B`**, where A has shape m×r and B has shape r×n, with **r much smaller** than both m and n.",
...
10 domains:
  ...
  LoRA: 140GB base + ~2GB of adapters + 1 inference stack` },
      ...
      "**QLoRA (Quantized LoRA)** extends reach further: the base model is quantized to **4-bit precision** to reduce memory footprint while the adapter trains in full precision.\n\nA 70B model that normally requires ~140GB at 16-bit precision ==fits in a single 80GB GPU under QLoRA== — making large-model fine-tuning accessible without a multi-GPU cluster.",
    ],
    keyPoints: [
...
FINE-TUNE — bake the change into weights:
  ...
  in minutes vs. re-train in days for every single change.` },
      ...
      "The 3 months of support tickets are **behavioral signal** — what good support looked like, how experts handled edge cases, what tone and format worked. ==They are fine-tuning training data, not RAG content.== Use them for fine-tuning *after* RAG has closed the knowledge gap.",
    ],
    keyPoints: [
