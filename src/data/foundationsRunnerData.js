...
Local max ≠ global max — that gap is exactly what beam recovers.` },
  ...
  "positional-encoding": {
    ...
    explanation: [
      ...
      "This frames the **two ways to extend context**.\n\n**Extrapolation** is what naive RoPE does past its window — feed positions beyond training and hope the unseen angles behave. They don't; attention degrades.\n\n**Interpolation** squeezes the longer range back into *trained* angles. Position interpolation linearly scales indices (position 128K treated as `128K × 4000/128000 ≈ 4000`) so every angle stays in range — but it crushes the fast short-range frequencies and hurts local precision.\n\n**NTK-aware scaling** (named for the Neural Tangent Kernel theory that motivated it) is the better fix: rather than scaling positions uniformly, it stretches the base θ (10000 → larger) so the **slow** long-range frequencies get interpolated (kept in-range) while the **fast** short-range frequencies stay nearly untouched (local detail preserved). YaRN and LongRoPE build on this with per-frequency schedules — instead of one global stretch factor, they tune the interpolation per frequency band, giving finer control over exactly which pairs get remapped and by how much.\n\n==The key point: you must remap the frequencies so long positions land on angles the model was trained on — the model cannot invent behavior for angles it never saw.==",
...
Trained on positions 0–3999 → model has only seen angle = position × θ_i in that range.
  ...
  Interpolate (PI):     scale positions ×(4000/128000) into range   → but squashes fast pairs, hurts local detail
