...
RMSNorm    = x / RMS(x) * g               drops mean-centering + bias   → ~same quality, cheaper (Llama/Mistral)` },
    ...
    recap: [
      ...
      "**Pre-norm > post-norm at depth** (clean identity path, no warmup); **RMSNorm** drops mean+bias, ~same quality, cheaper (Llama/Mistral).",
...
nudging them to raise p("cat") a little next time this context appears.` },
  ...
  "tempgame": {
    depthTier: "light",
    interviewWeight: "medium",
    groundUp: "Pick up exactly where next-token-prediction left off: at every single step, the model's forward pass produces one distribution over the vocabulary — logits, softmax, done. That module stopped at a single step. This module asks what happens across a whole *sequence* of them, strung together: what's the actual strategy for turning distribution after distribution into one finished sentence?\n\nPicture a hiker walking a trail made entirely of forks, who can only ever see the very next junction — never the full route ahead. At each fork, one path looks obviously well-trodden and inviting; the others look thinner, less certain. The tempting rule: always take the well-trodden fork, every single junction, never look back. It sounds unarguable — how could picking the *safest*-looking path at each step ever lead you wrong?\n\nHere's the catch this whole module turns on. A fork that looks perfect right now can lead straight to a cliff face two junctions later, with every path from there scrambling and clumsy — while a slightly less obvious fork now would have opened onto a gentle route to a far better destination. The best next step doesn't always add up to the best whole journey. The model faces exactly this trap at every single token: the word that looks best *right now* is a *local* decision, but the thing you actually care about is the quality of the *whole sentence* — and those aren't the same. The hiker's always-take-the-obvious-fork rule has a name here, **greedy decoding**, and it has exactly this blind spot.\n\nSo there isn't one \"correct\" way to walk the trail — there's a small family of strategies, each right for a different kind of hike. We'll build them up from that one insight: greedy and its blind spot, sending a small team of scouts down several forks at once so one dead end doesn't cost you the whole hike — that's **beam search** — and sampling, for when you're not racing toward one \"right\" destination at all and a little wandering is the point. Take your time; by the end you'll be able to name the right decoder — the right hiking strategy — for any feature on sight.",
    ...
    explanation: [
      "At each fork, score every path by how probable it looks — how heavily trodden it is — and step onto whichever one scores highest, every single time, with no glancing back to reconsider an earlier junction. That one-highest-score-only, never-reconsider rule, applied to the model's own token distribution, is **greedy decoding**: take the single highest-probability token at every step. The trap: it's a **local** maximum, not a global one.\n\nThe model scores each next token given the prefix so far — but the highest-probability token *now* can lead into a region where every continuation is mediocre, while a slightly-lower-probability token now would open onto a far better complete sequence. Sequence probability is the *product* of per-token probabilities, and greedy never reconsiders: once it commits to token N it cannot back out, even if token N+1 reveals the cliff face.\n\n==A word that looks obvious mid-sentence can strand the rest of the sentence.== Greedy optimizes each step in isolation and hopes the steps compose — they often don't.",
      "Recall the scouts from a moment ago: attack the local-maximum trap by sending not one hiker but b scouts down different forks in parallel, each one carrying its own running partial sequence — one of the 'beams.' At each step, every surviving beam expands onto its candidate next tokens, each resulting longer sequence gets scored by *cumulative log-probability*, and the pack prunes back to the best `b` overall — the **top-b partial sequences** — call the weakest scouts back, keep the strongest exploring. That mechanism, keeping several partial sequences alive in parallel and pruning by cumulative score, is **beam search**.\n\nBecause you carry several hypotheses forward, a beam that took a slightly-lower-probability token early can **overtake** the greedy path later once its higher-probability continuations show up — a scout on the less-obvious fork can end up on the better route.\n\n==Beam search approximates 'find the highest-probability whole *sequence*' rather than 'the highest-probability next *token*'== — which is why it dominates on one-right-answer tasks whose quality is the joint sequence probability: machine translation, speech transcription, constrained generation.",
...
Local max ≠ global max — that gap is exactly what beam recovers.` },
      "So why not send the scouts everywhere? Because on **open-ended generation** — chat, story-writing, brainstorming — maximizing sequence probability is the *wrong* objective. There's no single best destination to race the scouts toward.\n\nThe highest-probability human-like continuation is bland and generic: 'I think that is a great question and I am happy to help.' Beam search, hunting for the most probable sequence, converges on exactly this safe, repetitive, low-information text — and it's prone to **high-probability loops** (the same phrase repeats because repeating it is locally very probable) — the scouts all converging on the same well-trodden loop of trail.\n\n==For open-ended text you *want* some lower-probability, more surprising continuations — that's where the interesting, human-sounding output lives.== This is the regime where you stop racing toward one destination and start **wandering the trail on purpose** — weighted toward the nicer-looking forks, but genuinely open to a surprise. That's **sampling** from the distribution.",
      "That deliberate wandering is exactly what **stochastic decoding** hands you the controls for (see the sampling module): temperature reshapes the distribution — sharpening toward greedy when `T<1`, flattening toward diversity when `T>1` — and top-p / top-k truncate the tail so you sample only among plausible tokens: dynamically with top-p, at a fixed count with top-k.\n\nThe point here: ==temperature and top-p are the tools for the open-ended regime that beam search is wrong for.== Newer **contrastive** methods (contrastive search / decoding) split the difference — penalizing tokens too similar to recent context to fight blandness and repetition while staying more grounded than free sampling.",
      "So let's collapse everything into the **decision rule, by task**, because that's the reusable takeaway:\n\n- One-right-answer, quality = sequence probability → **beam search** (translation, ASR, structured/constrained output).\n- Deterministic, exact, one correct string → **greedy / `temperature=0`** (SQL, JSON, tool-call arguments).\n- Open-ended, diversity and voice matter → **sampling with temperature + top-p** (chat, creative, ideation).\n\nAnd whatever you pick, ==**document it**.== Undocumented decode settings are a classic source of unexplained production drift — a future engineer flips a shared default from beam to sampling, or nudges temperature, and quality shifts silently across every downstream feature tuned to the original setting. This module's interactive — running greedy against beam on the same prefix — lets you watch the greedy path walk straight into its dead end while beam's scouts recover the better whole sequence, and the production case at the end is exactly this: a greedy default that quietly cost quality, and which decoder — which hiking strategy — each feature should have used instead.",
    ],
    keyPoints: [
    ...
    recap: [
      "**Greedy = local max:** top token/step, no backtrack → can dead-end the sequence (quality = product of token probs).",
      "**Beam search:** keeps top-b beams, scores cumulative log-prob → approximates the highest-probability whole *sequence*.",
      "**Beam wins** translation/ASR/constrained output — one right answer = joint sequence probability.",
      "**Beam loses** open-ended: bland, repetitive, loop-prone → switch to sampling.",
      "**Sampling (temp + top-p)** owns the open-ended regime; contrastive methods split the difference.",
      "**Rule:** greedy/`T=0` → exact; beam → single-best sequence; sampling → diverse chat. **Document the choice** (else silent drift).",
    ],
    mcqs: [
      {
...
task never touches — the definition of wasted spend.` },
    ...
    recap: [
      "**One-model-fits-all = the most common LLM cost overrun** — top vs. small tier: 20–50× cost, 3–5× latency, every request.",
...
If you have 1.5T tokens of training data:
  ...
  70B model: near-optimal ✓ (compute: 6×70e9×1.5e12 ≈ 6.3×10²³ FLOPs)
  7B model: receives 10× more data than Chinchilla prescribes
             (compute: 6×7e9×1.5e12 ≈ 6.3×10²² FLOPs — 10× LESS)

This is NOT an equal-compute comparison: same tokens ≠ same compute,
because FLOPs scale with parameters too. The 7B trains ~10× cheaper
and infers 5–10× cheaper again. The real "over-trained small beats
undertrained large at equal FLOPs" trade (Llama 2/3's actual move)
compares against a genuinely undertrained large model like GPT-3 or
Gopher, not a near-optimal 70B.` },
...
10 domains:
  ...
  "few-shot": {
    ...
    explanation: [
      ...
      "Look at what \"inconsistent\" actually means here, concretely. \"Translate: Good morning\" uses one label. \"Hello, how are you?\" uses no label at all — the model has to infer from context that translation is still the task. \"Translate to French: Thank you\" uses a third, more specific label. All three outputs are correct, so nothing in the examples themselves looks wrong. But the model is learning a *distribution over input shapes*, not just a task — and these three examples describe three different shapes for the same task, with no signal for which shape a new, unseen input will take. Contrast that with three examples that all read \"Translate to French: [text]\" — same shape every time. Now there's exactly one pattern for the model to match new inputs against, and \"is this a translation request\" stops being an inference the model has to make on its own.",
  ...
  "chain-of-thought": {
    ...
    explanation: [
      ...
      "So does that mean chain-of-thought should go on every prompt? Compare the two query types side by side. On Roger's balls, direct answer is wrong (8), CoT answer is right (11) — worth every one of its 22 tokens. On \"what's the capital of France,\" direct answer is already right (Paris, 1 token) and CoT answer is also right, but takes 31 tokens to say the same thing — a 30× cost (31 ÷ 1 = 31) for zero accuracy gain, because there was no multi-step derivation to route through in the first place; \"thinking step by step\" about a single fact lookup has nothing to constrain. The pattern generalizes past these two examples: CoT helps wherever an answer requires holding and combining multiple intermediate results — multi-step math, complex multi-hop reasoning, moderately on code generation and debugging where planning the approach first catches structural mistakes. It doesn't help, and can actively hurt, wherever the answer is a single retrieval (simple factual queries, where the extra steps add tokens and occasionally invite confabulation) or wherever the task needs unconstrained output rather than a derivation (creative writing, where step-by-step structure interrupts the thing you're actually asking for).",
      "Once you know a task is CoT-worthy, there's a cheap way to invoke it and a more expensive one. **Zero-shot CoT** — literally appending \"let's think step by step\" — needs no examples and is the right starting point; it's what turned \"8\" into \"11\" above. **Few-shot CoT** goes further, supplying complete worked examples with full derivation traces, useful when a domain has its own calculation style you want the model to match exactly. And for the highest-stakes calculations, where a single wrong chain has real consequences, **self-consistency** runs the same CoT prompt several times and takes the majority-vote answer across independent chains — more robust, at the cost of multiple inference calls. The Hands-On interactive on this module lets you browse the full task-type grid and toggle both worked comparisons yourself, and the production case at the end is exactly this decision: two query types in the same system, one that needs the extra tokens and one that's just paying for them.",
    ],
    mcqs: [
      {
      ...
      },
      {
        ...
        options: [
          ...
          "CoT costs about 30× the tokens (31 ÷ 1) for zero accuracy improvement, because a single-fact lookup has no multi-step derivation to route through",
...
Cost gap: ~10⁵–10⁷× cheaper to fine-tune (same order as the token-
count gap above), and you KEEP the
...
itself).` },
    ...
    recap: [
      ...
      "**Behavior changes, knowledge/reasoning don't** — SFT dataset ≈ 10⁻⁷ the size of pretraining.",
    ...
    mcqs: [
      ...
      },
      {
        ...
        options: [
          ...
          "Roughly the 10M–50M range, depending on hardware — beyond it, pgvector's query latency climbs as the index grows, and scaling out requires complex Postgres sharding",
  ...
  "eval-loop": {
    depthTier: "deep",
    interviewWeight: "high",
    scenario: "A new model version scores 94% on your eval suite, up from 89% last release. Do you ship it? Before answering: who graded it, and could they be biased toward their own output? Has anyone on the team seen these exact questions before — and started, even unconsciously, writing answers that please them? Is 94% actually better than what's already in production, or just a number with nothing to compare it to? And was a bar agreed on before this run, or is the team about to argue about what 94% 'really means'? An eval that runs and prints a score can still be worthless — the number is theater with a decimal point unless four properties hold together: a fixed, uncontaminated dataset; an independent scorer; a real baseline; and a threshold set before the score arrives.",
    explanation: [
      "Suppose twelve questions, written a year ago by the founding engineer, have sat in the repo before every release for six months — visible to the whole team, and every release has cleared the bar. That isn't an eval producing a trustworthy signal; it's a regression test for exactly those twelve inputs, because the moment people can read the questions, they start optimizing toward those specific inputs instead of general quality, whether they mean to or not. Call this failure **eval-set contamination** — and avoiding it is **Property 1: is the evaluation set fixed, and not secretly small?** The eval set has to be stable: it shouldn't shift every time the system shifts, or you're comparing two systems on two different tests. But 'fixed' doesn't mean 'twelve questions frozen forever' — a trustworthy eval set is version-controlled separately from the system, sized well beyond what any one person could memorize, and continually augmented with adversarial cases and real failure queries pulled from production.",
      "Suppose the eval set itself is solid — stable, large, unseen by the team. The score still isn't safe, because of who's doing the grading.",
      "An automated scorer turns outputs into a number — but only if it has no reason to be generous. Suppose a team lets GPT-4 both generate an answer and grade it: the score climbs release over release, not because the answers are getting better, but because the judge has a thumb on the scale for anything that sounds like itself. Same-family grading isn't neutral — the judge shares training distributions and stylistic habits with the thing it's grading, and systematically rewards text that resembles its own generation style. This is **Property 2: is the scorer actually independent of what it's grading?** The honest hierarchy: human annotation is the gold standard. LLM judges are acceptable for scale only when calibrated against human labels and drawn from a different model family than the system under test.",
...
Cross-family, calibrated judge
  ...
  "rag-pipeline": {
    ...
    explanation: [
      "Two questions that use completely different words can mean almost the same thing — 'What is the refund policy for SaaS subscriptions?' and 'Can I get my money back on a monthly plan?' share almost no vocabulary but ask for the same fact. Two questions that share several words can mean nearly opposite things — 'refund policy for subscriptions' and 'refund policy exceptions for subscriptions' overlap heavily in wording but ask for opposite information. Keyword matching can't tell these apart; it can only count shared words. RAG's retrieval step instead represents meaning as geometry: every chunk of text is converted into an **embedding vector** — a list of numbers positioned so that texts with similar meaning land close together in that space, regardless of shared vocabulary — and a query is compared against every indexed chunk using **cosine similarity**, a measure of how close two vectors point in that space. The closer the vectors, the more likely the texts mean the same thing.",
      "**Step 1 — Query → Retrieval.** The query 'What is the refund policy for SaaS subscriptions?' is embedded and compared against every indexed chunk. Rather than returning every chunk above some raw similarity score, the retriever returns the **top_k** most similar chunks — a fixed count, set in advance. With top_k set to 3, the top-3 chunks by similarity score are retrieved.",
  ...
  Chunk 3 (score 0.72) · enterprise-sla.md § 1  [borderline]
    "Enterprise contracts are governed by separate SLAs negotiated at
     time of signing and may override standard refund terms."` },
      "**Step 2 — Augmentation.** The retrieved chunks are assembled into the prompt alongside a system instruction and the user's question — system message, Chunk 1, Chunk 2, Chunk 3, then the user question. Chunk 3, at a similarity score of 0.72, is borderline: many production systems set their inclusion cutoff somewhere in the 0.70–0.75 range, so a score this close could go either way depending on exactly where the line is drawn — and it's enterprise-contract language that may not apply to this user.",
      "**Step 3 — Generation.** The model's response: 'Based on your subscription type: if you're on a standard SaaS plan, you can get a full refund within 14 days of billing (Chunk 1). For annual plans, refunds are prorated minus a 10% fee (Chunk 2). If you're on an enterprise contract, your specific SLA applies — please check your agreement or contact your account manager.' Every claim maps to a retrieved chunk — Chunk 1 and Chunk 2 are directly cited, Chunk 3 is acknowledged but not asserted as fact. Grounding like this sharply reduces hallucination risk relative to relying on the model's memorized training data — but it isn't a guarantee: retrieval itself can fail in several distinct ways, covered next, and even well-grounded generation can still override its context in some cases.",
      "Remember Chunk 3's borderline score from Step 1? That's the detail worth pausing on. In production, a poorly-calibrated similarity threshold — set too loose, the way Chunk 3's inclusion here is a close call — is one of the most common sources of confident-but-wrong answers, because the model treats all retrieved chunks as equally authoritative regardless of how close they came to being excluded.",
      ...
      "**Noise Injection (retrieval config):** top_k set to 15 instead of 3 pulls in 12 marginally-relevant chunks alongside the 3 correct ones — billing dispute procedures, enterprise renewal terms, EU VAT reclaim notes, payment method FAQ, general cancellation flow — pushing the context to roughly 3,800 tokens. Too many chunks forces the model to reconcile noise with signal: the answer becomes a hallucinated amalgamation of adjacent topics, technically sourced but not grounded in the relevant chunk. The retriever did its job; the config broke it. Fix: start with top_k=3–5 for most use cases, measure **Precision@k** — the fraction of the top-k retrieved chunks that are actually relevant — on a golden question set before going higher, and use a **reranker** — a second-pass model that re-scores retrieved chunks for relevance before they reach the LLM — to filter before passing to the LLM instead of raw top_k expansion.",
      "Stale retrieval and noise injection both happen before generation — the wrong or excessive chunks are already sitting in the context by the time the model starts writing. Suppose retrieval does everything right this time: the correct chunk is retrieved, it's the only chunk that matters, and the system prompt says 'answer only from the provided context.' Is the answer now guaranteed to be correct? Not necessarily — the model still has to choose to use what it was given.",
