...
ALiBi (no rotation):   score −= m·(i−j)   linear distance penalty, no trained params
  ...
  "kv-cache": {
    ...
    explanation: [
      ...
      "The memory cost is the **other side of the trade**. The cache stores K and V for every token, across every layer and every KV head:\n\n`2 × num_layers × num_kv_heads × head_dim × seq_length × bytes_per_element`\n\nFor a 70B model in fp16 using **grouped-query attention** (GQA — several query heads share one key/value head, so fewer separate K/V sets need caching; the exact sharing mechanics are worth a closer look, coming up next) — the real Llama 2/3 design (80 layers, 8 KV heads, 128 head_dim): `2 (one K set, one V set) × 80 (layers) × 8 (KV heads) × 128 (head_dim) × 2 bytes (fp16 precision) = 0.33MB per token`. ==Each additional token adds another 0.33MB across the full layer stack — linear in conversation length.==",
    ...
    recap: [
      ...
      "**Binding concurrency constraint:** shared memory pool → one new user shrinks batching + context for everyone.",
      "**Architectural shrink (pre-train):** MQA (~64×), GQA (4–8×) — fixed, can't change at inference.",
