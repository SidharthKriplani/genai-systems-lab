...
in English, on the exact same tokenizer.` },
    ...
    recap: [
      ...
      "**Prose ~1.0–1.35 tok/word; numbers/IDs/code/JSON run well above that (2×+)** because they have no mergeable frequent pairs.",
...
Softmax:
  ...
  the      = 1.13 / 12.10 = 0.09   ███                 9%
    ...
    recap: [
      ...
      "**Softmax sharpens** (2.3× raw gap → 2.8× weight gap; top token gets 51% vs. the 20% equal weighting would give it); the **value vector V** is then weighted-summed to build the context-rich representation.",
      "**Cost is quadratic** (n² pairs/layer, 16K → 256M) — the reason long context is expensive. **Attention sinks** are a separate effect: softmax normalization plus position 0's constant training-time visibility, not the O(n²) cost itself.",
    ],
    mcqs: [
      {
...
RMSNorm    = x / RMS(x) * g               drops mean-centering + bias   → ~same quality, cheaper (Llama/Mistral)` },
  ...
  "seq-parallel": {
    ...
    recap: [
      ...
      "**Training/prefill parallelize; generation doesn't** — token t+1 needs token t to exist, in any architecture; that's why the KV cache exists.",
    ],
    mcqs: [
      {
...
ALiBi (no rotation):   score −= m·(i−j)   linear distance penalty, no trained params
  ...
  "context": {
    ...
    explanation: [
      ...
      "Now, *where* does that attention concentrate, and why? Because models are trained on human text where the key information tends to sit near the **beginning** (journalism, executive summaries) or near the **end** (conclusions, results), they learn an attention pattern that favors both edges. Which means recall follows a **U-shaped curve**: highest near the start and end, and degrading gradually as content moves toward the middle — troughing lowest around the middle third to half of the window, with no single sharp percentage cutoff — regardless of how semantically relevant that content is. This is 'lost in the middle': not a theoretical worry but a documented empirical effect (Liu et al., 2023), and it follows directly from the training distribution.",
    ...
    takeaway: "Long context does not equal good context. Position within the context window materially affects extraction quality — attention/recall follows a U-shaped curve that troughs in the middle third to half of the window, well below either end, with no sharp cutoff percentage. For long-document synthesis, use map-reduce rather than stuffing, place critical content at the beginning or end, and test extraction specifically at middle-document positions.",
  },
...
because long-prompt applications have 10-50× more input tokens than output tokens.` },
  ...
  "latency-planner": {
    ...
    explanation: [
      ...
      "**Because** latency has those two halves, the levers sort naturally by which half they move — in order of typical impact: (a) **Reduce output length** — if the model writes 300-word explanations where 80 words suffice, a concision instruction cuts the generation term by roughly 70-75% (300→80 words is a ~73% reduction, not a rounding-error-sized cut) with no model change; check this first by logging TPOT × token_count, it's the most overlooked win. (b) **Streaming** — render as the first tokens arrive, so TTFT *becomes* the perceived latency rather than total generation time; a 5.4s response that starts streaming in 400ms feels responsive at unchanged wall-clock time. (c) **Reduce input length** — fewer prompt tokens lower TTFT; trim the system prompt, cut retrieved context from 10 chunks to 3, cache stable prefixes. (d) **Smaller or faster model** — a 3× cheaper model is often 2–3× faster at the same tier, and Q&A rarely needs frontier capability.",
      "**But** notice which gap those levers can and can't reach. The p95-to-p99 jump — here 2.8 seconds (8.2s − 5.4s) — is a *different failure mode*: it reflects infrastructure events (cold starts on serverless endpoints, shared-resource queuing spikes, token-rate throttling), not generation-time variance, **so** application-layer optimizations simply cannot fix it. **Therefore** the p99 fixes are infrastructural: provision a dedicated throughput endpoint to kill shared-queue spikes; set timeout-plus-retry so a request past 4s falls back to a smaller model (3s beats 8s for Q&A); push the slowest requests to async-with-callback so they leave the perceived-latency bucket entirely. The interactive is a stage-by-stage latency budget allocator — set an SLA target, then slide each pipeline stage's (retrieval, LLM, reranker, etc.) millisecond budget and skip optional stages to see how much headroom you buy, with a fixed 15% overhead reserved for network and serialization; it's the tool for the *planning* half of this problem (deciding budgets before you build), not a live percentile or streaming simulator. The closing scenario is exactly this p50/p95/p99 profile waiting to be read.",
    ],
    mcqs: [
      {
      ...
      {
        question: "The text treats the p95-to-p99 gap (5.4s to 8.2s) as a different failure mode than the p50-to-p95 gap. What does it say the 2.8-second p95-to-p99 gap indicates, and what follows for fixing it?",
        options: [
      ...
      },
      {
        ...
        options: [
          "Generation time equals output_tokens times TPOT, so cutting a 300-word answer down to the 80 words actually needed cuts generation time by roughly 70-75% with no model change required",
  ...
  "observability-concepts": {
    ...
    explanation: [
      ...
      "**And that's possible because** each of those causes leaves a fingerprint in a measurable signal. Retrieval quality shows up in the average retrieval-score distribution and the rate at which the top-k chunk contains a query keyword. Answer quality shows up in proxies — answer-length distribution, response-format compliance, citation completeness. User-facing behavior shows up in thumbs-up/down rate, follow-up-question rate, and post-response session abandonment. And a silent model swap shows up directly in the **model version from the API response body**. Each signal moves when a different root cause moves.",
      "**Therefore** the minimum viable stack is small but pointed: log every request with input tokens, output tokens, retrieval scores, model name (from the response body), and response time; sample 5% of responses for lightweight LLM-as-judge scoring; and alert on answer-length distribution Z-score > 2, retrieval-score mean shift > 10%, week-over-week feedback drop > 10%, or a model-version field differing from expected. Organize this into four pillars: **traces** (the end-to-end record of one call — prompt, response, tool calls, and latency per **span**, where a span is one measurable sub-step inside a call, like retrieval or generation), **metrics** (aggregated performance across many calls), **quality signals** (the ones that actually catch drift — feedback, hallucination rate, retrieval hit rate), and **alerts** (thresholds on all of the above). The interactive is a click-to-expand browser over exactly those four pillars, plus a second tab showing where standard APM tools (Datadog, uptime monitors, plain logging) stop short — and the closing scenario is exactly that: a two-week regression, green error dashboards, and the signals that would have caught it in hours.",
    ],
    mcqs: [
      {
      ...
      {
        question: "A team confirms the model version read from the API response body now differs from what they expected, with no other change. Select the two correct statements (choose 2).",
        options: [
          ...
          "The model version field is the exact signal the text recommends logging to catch this cause, since it comes straight from the response body",
          ...
          "This finding rules out data drift entirely, since a stale vector index would also change the model version reported in the response body",
        ],
        correct: [0, 1],
        explanation: "Both are correct: the text names silent model version updates - providers updating weights behind a stable endpoint name without announcement - as the matching cause, and it specifically recommends logging the model version from the API response body to detect exactly this; a mismatch in that field confirms it. Option C is wrong: a traffic distribution shift is about new query patterns from a new cohort, which would not show up as a changed model version field. Option D is wrong: data drift is about stale indexed documents, detectable through retrieval-score shifts, and has no bearing on the model version field.",
      },
    ],
    takeaway: "LLM systems degrade silently — error rate won't tell you. Log retrieval scores, answer length distributions, model version from the response body, and sample responses for lightweight quality scoring. These signals catch regressions in hours. Silence from error monitoring is not evidence that quality is healthy.",
  },
...
Gopher, not a near-optimal 70B.` },
    ...
    recap: [
      ...
      "**Over-trained small ≈ genuinely undertrained large, at equal FLOPs** (Llama 2/3 strategy) — same token count ≠ same compute.",
...
10 domains:
  ...
  LoRA: 140GB base + ~2GB of adapters + 1 inference stack` },
    ...
    recap: [
      ...
      "**Quality near-parity with full FT at r=32–64** (single-digit % gap on most tasks) — widens only for pre-training-absent capabilities.",
...
itself).` },
  ...
  "vector-migration-patterns": {
    ...
    recap: [
      "Embeddings from different models aren't comparable — switching models silently breaks every stored vector, no error thrown.",
      "Pattern: dual-write (new docs to both indexes) → backfill (re-embed the old ones in the background) → cutover (atomic read switch) → decommission.",
      "5M docs at 500 docs/sec ≈ 2.8 hours of backfill — schedule off-peak, don't read from a partially-backfilled index.",
      "Keep the old index warm 24-72h post-cutover as rollback insurance before decommissioning.",
      "Same dual-write/backfill/cutover mechanics apply to a vector-DB migration or a re-embedding — only the trigger differs.",
    ],
    mcqs: [
      {
