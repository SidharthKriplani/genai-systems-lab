...
At encode time a NEW word reuses these merges:
  "lowering"  →  low · er · ing   (in-vocab subwords, no OOV, morphology captured)` },
      ...
      "So the whole chain lands on one rule: measure your budget in **tokens**, benchmarked against the *kind* of content you actually feed the model — never in words.\n\nThe interactive lets you watch merges form and token counts diverge from word counts as you switch content types. ==Then the closing scenario puts it to work on a document that truncates for exactly this reason — see if you can call the cause before the reasoning does.=="
    ],
    keyPoints: [
...
RMSNorm    = x / RMS(x) * g               drops mean-centering + bias   → ~same quality, cheaper (Llama/Mistral)` },
      "The final choice is **causal masking**.\n\n**Encoders** use bidirectional attention — every token attends to every other — producing the richest possible per-position representation. This makes them ideal for classification, retrieval, and embedding, where you need to *understand* text, not generate it.\n\n**Decoders** apply a **causal mask** that prevents each token from attending to *future* positions, because at generation time those tokens don't exist yet. This forces the model to predict each next token from left context only — exactly what autoregressive generation requires.\n\nIn production: embedding models for RAG use *encoder* attention; generation models use *decoder* attention. ==Using a generation model's internal representations for retrieval produces poor results because the causal mask distorts the vector geometry.==\n\nThat completes the block: FFN for nonlinearity, positional encoding for order, residual + norm for trainable depth, causal masking for what the layers can be *used* for. The interactive lets you assemble and stack it. Then the closing scenario hands you a tempting one-line proposal about depth — weigh it against the four levers you just built before the reasoning does.",
    ],
    keyPoints: [
