...
export const RUNNER_DATA = {
  ...
  "tokenizer": {
    ...
    explanation: [
      "You're staring at a context window reading **4,096 tokens** — and a document that's only **3,200 words**. Those numbers should be closer.\n\nSomething about how text becomes numbers is making legal contracts cost more tokens than plain prose does. ==Before you can fix the truncation, you need to understand that gap.==",
      "The first naive fix is obvious: assign an integer ID to **each character**. `'a'` is 97, `'b'` is 98, and so on — a standard ASCII lookup.\n\nThis keeps vocabulary tiny (128 characters) but creates a different problem: **every letter becomes its own token**. The word `'indemnify'` is 9 characters, so it costs **9 tokens**. The model burns context capacity processing individual letters, and has to learn spelling, morphology, and word boundaries from scratch instead of from meaning.\n\n==Sequences become extremely long for almost no semantic gain.==",
      "The second naive fix: assign an integer ID to **each word**. `'indemnify'` gets one token, `'indemnified'` a different one, `'indemnification'` yet another.\n\nMore efficient — until you count the vocabulary. English has **hundreds of thousands** of word forms, and those three variations are completely unrelated entries. Any word outside the vocabulary is unknown (**OOV**), and the model can't generalize — it has to memorize every inflected form as if it were a different concept.\n\n==Vocabulary size explodes, and generalization collapses on novel forms.==",
...
At encode time a NEW word reuses these merges:
  "lowering"  →  low · er · ing   (in-vocab subwords, no OOV, morphology captured)` },
      "BPE is one member of a **family**, and the variants matter in practice.\n\n**WordPiece** (BERT) is BPE-like but merges the pair that most increases *corpus likelihood* rather than the raw most-frequent pair, and marks intra-word continuations (e.g. `'##ing'`).\n\n**Unigram / SentencePiece** (T5, Llama) works top-down instead: start from a large candidate vocabulary and prune the tokens that hurt likelihood the least. SentencePiece treats the raw text — spaces included — as input, so it is **language-agnostic and reversible**.\n\n**Byte-level BPE** (GPT-2/3/4) runs BPE over raw *bytes* rather than Unicode characters. Because every possible byte is in the base vocabulary, there is **zero out-of-vocabulary** — any Unicode string, emoji, or non-Latin script always encodes, at worst falling back to multiple byte-tokens. ==That zero-OOV guarantee is why byte-level BPE became the default for general-purpose LLMs.==",
      "The direct consequence: **token count ≠ word count** — and the ratio depends heavily on content type.\n\nStandard English prose sits at roughly **0.65–0.75 tokens per word**. But financial figures, legal identifiers, UUIDs, and structured data tokenize far less efficiently.\n\n==BPE was built on prose — unusual character sequences have no frequent pairs to merge, so each character stays its own token or joins only short subwords.==",
      ...
      "Close the loop on the scenario: the 3,200-word document consuming 4,096 tokens is exactly this.\n\nThe financial exhibits, section references (`§ 14.2(b)(iii)`), and legal identifiers tokenize at **1.5–2× the rate** of plain prose. You measured *word* count; the context window measures *tokens*.\n\n==The gap grows with every exhibit — which is why truncation hits contracts with dense financial schedules harder than pure narrative sections.=="
    ],
    keyPoints: [
  ...
  "attention": {
    ...
    explanation: [
      ...
      "If you represent them separately — one learned vector for **what a token is searching for**, one for **what a token exposes to searchers** — you can compute a match between any two tokens at any two positions.\n\nMultiply the two vectors element by element, sum the results. ==That's a dot product.==",
...
Softmax:
  ...
  the      =  0.6 / 21.8 = 0.03   ▌                  3%
      ...
      "Now take the **weighted sum** of what each token contributes — its **value vector V**. *Surgeon* holds 75% of the weight; *the* holds 3%. The result is a representation of *agreed* that is heavy with *surgeon* and nearly untouched by the noise around it.\n\nThat's **Q, K, and V**. Q and K determine *how much* each token matters. V determines *what* it contributes.\n\n==You computed relevance without writing a single rule — the model learns which queries match which keys, from training data, across every domain and language simultaneously.==",
      "The cost is **quadratic**: n queries × n keys = **n² attention pairs** per layer. A 4K-token sequence has ~16 million pairs per layer; a 16K-token sequence has **256 million**. Memory scales the same way — the full n×n attention matrix must be materialized.\n\nThe **attention sink** diagnosis in this scenario now follows directly: token position 0 is always visible to all subsequent positions during training, making it a reliable target for attention mass that doesn't belong to any semantic target. Concentration *there* — rather than on mid-document content — produces the extraction failures you're seeing.\n\n==You didn't look this up. You derived it.==",
    ],
    keyPoints: [
  ...
  "transformer": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Let's start from what we already have. Attention gives every word the ability to look back and pull in the words that matter to it — a genuinely powerful idea. So here's a fair question: if attention is so good, why not just *stack a hundred attention layers* and call it a language model?\n\nIt turns out that doesn't work, for two quiet reasons that surprise people. First, attention only ever *mixes* words together — it blends what's already there but never transforms it in a richer, per-word way, so stacking it just gives you a fancier blend, not deeper thinking. Second, attention treats a sentence like an unordered *bag* of words — swap two words and it computes the same thing — so on its own it has no sense of order at all. A **Transformer block** is the small, carefully-designed bundle of parts that patches both gaps and, crucially, stays trainable even when you stack it dozens of layers deep.\n\nWe'll build that block up one piece at a time — the part that adds real per-word transformation, the part that restores word order, and the two tricks (residual connections and normalization) that keep a very deep stack from falling apart during training. No rush. Each piece exists to fix a specific problem, and once you see the problem, the piece feels inevitable.",
    scenario: "Now let's put all of that to work on a real one. An engineer proposes the tempting one-liner: *just add more layers* to make the model reason better on hard multi-step questions — and there's no compute budget attached. Take a moment before reading on: with the block's anatomy fresh in mind, what's actually wrong with that proposal? Here's the reasoning, step by step. Depth *is* one of the three structural requirements we built up, so more layers isn't crazy on its face — but notice everything the proposal ignores. It says nothing about *width* (the FFN, where most of the model's knowledge actually lives), nothing about the residual-plus-norm machinery that makes added depth trainable at all, and nothing about the encoder-versus-decoder asymmetry that decides what those layers can even be used for. And it has no compute budget, when depth is exactly the axis that scales cost hardest. So the right response isn't yes or no — it's to probe whether the engineer understands that depth is one lever among several, each with its own price, and that reaching for it blindly is how you burn a compute budget without buying reasoning.",
    explanation: [
      "Attention gives every token selective access to every other token — but each attention output is still a **linear combination** of value vectors.\n\nStacking attention layers compounds this: composing linear functions collapses to a **single linear transformation** regardless of depth. And attention is a *set* operation — it produces the same output whether token A precedes or follows token B, so the model has no notion of order.\n\n==Two structural gaps remain after attention alone — no per-position nonlinearity, no positional awareness — and neither can be fixed by adding more attention layers.==",
      "The **Feed-Forward Network (FFN)** resolves the nonlinearity gap. After each attention sublayer, an FFN applies a learned nonlinear transformation independently to each position — two linear layers with a ReLU or GeLU activation between them, at **4× the hidden dimension**.\n\n==This is where most of the model's parametric knowledge concentrates — factual associations are predominantly stored in FFN weights.==\n\nPosition is handled separately via **positional encoding** added before the block stack. Modern models use **RoPE** (Rotary Positional Embedding), which encodes *relative* position directly in the attention computation and generalizes past training length.",
      "The third structural requirement is **depth** — and depth creates a training problem.\n\nBackpropagation multiplies gradients through every layer from output to input. Without shortcut paths, gradients at early layers approach **zero** before the optimizer can use them — the network is too deep to train.\n\n**Residual connections** fix this: `output = input + block(input)`. Each block learns a *correction* to the identity path rather than a complete transformation, creating a ==direct gradient highway from the loss to every layer.==\n\n**Layer normalization**, applied after each residual addition, handles a second problem residuals alone don't solve: as blocks stack, activation magnitudes **drift** — later layers receive inputs at wildly different scales, destabilizing training. Layer norm re-centers and rescales after each sublayer, keeping the signal stable regardless of depth.\n\nThe 'add & layer-norm' in every block — attention sublayer, then FFN sublayer, each with residual + norm — is **both gradient highway and activation stabilizer** together.",
      "**Where** you put the norm matters — and modern models moved it.\n\nThe original Transformer used **post-norm**: `LayerNorm(x + Sublayer(x))` — the norm sits *on* the residual stream, after the add. That works but is fragile at depth: the norm is on the gradient highway, so gradients get rescaled every layer, and deep post-norm models need careful learning-rate warmup or they diverge.\n\n**Pre-norm** moves the norm *inside* the residual branch: `x + Sublayer(LayerNorm(x))`. Now the identity path `x` is never normalized — a clean, unscaled highway from the loss all the way to the input — and only the sublayer's input is normalized. ==That single move makes very deep models (GPT-2 onward, essentially every modern LLM) trainable without warmup and far more stable.==\n\nA further simplification is **RMSNorm**: standard LayerNorm both re-centers (subtract mean) and rescales (divide by std), plus a learned bias. RMSNorm **drops the mean-centering and bias entirely** — it only divides by the root-mean-square and applies a learned scale. The mean-centering contributes little empirically, so RMSNorm reaches about the same quality with less arithmetic — which is why Llama, Mistral, and most recent models use **pre-norm + RMSNorm**.",
...
RMSNorm    = x / RMS(x) * g               drops mean-centering + bias   → ~same quality, cheaper (Llama/Mistral)` },
      "The final choice is **causal masking**.\n\n**Encoders** use bidirectional attention — every token attends to every other — producing the richest possible per-position representation. This makes them ideal for classification, retrieval, and embedding, where you need to *understand* text, not generate it.\n\n**Decoders** apply a **causal mask** that prevents each token from attending to *future* positions, because at generation time those tokens don't exist yet. This forces the model to predict each next token from left context only — exactly what autoregressive generation requires.\n\nIn production: embedding models for RAG use *encoder* attention; generation models use *decoder* attention. ==Using a generation model's internal representations for retrieval produces poor results because the causal mask distorts the vector geometry== — an asymmetry the 'add more layers' proposal ignores entirely.",
    ],
    keyPoints: [
...
Local max ≠ global max — that gap is exactly what beam recovers.` },
      ...
      "The **decision rule, by task**:\n\n- One-right-answer, quality = sequence probability → **beam search** (translation, ASR, structured/constrained output).\n- Deterministic, exact, one correct string → **greedy / `temperature=0`** (SQL, JSON, tool-call arguments).\n- Open-ended, diversity and voice matter → **sampling with temperature + top-p** (chat, creative, ideation).\n\nAnd whatever you pick, ==**document it**.== Undocumented decode settings are a classic source of unexplained production drift — a future engineer flips a shared default from beam to sampling, or nudges temperature, and quality shifts silently across every downstream feature tuned to the original setting.",
    ],
    keyPoints: [
...
ALiBi (no rotation):   score −= m·(i−j)   linear distance penalty, no trained params
      ...
      "Closing the scenario: 'just use RoPE, it extrapolates automatically' is **wrong on both counts**. RoPE doesn't extrapolate — it degrades gracefully within a range and unreliably beyond it.\n\nExtending a 4K-context model to 128K requires **explicit context-extension techniques**: YaRN and LongRoPE rescale the rotation frequencies so the full 128K range maps to angles the model can handle, then validate **perplexity at the target length** to confirm it's working.\n\n==RoPE is relative position encoding, not infinite-range position encoding — using it is the starting point, not the conclusion.==",
    ],
    keyPoints: [
...
because long-prompt applications have 10-50× more input tokens than output tokens.` },
  ...
  "model-families": {
    ...
    explanation: [
      "The stakeholder's 'just use GPT-4 for everything' feels safe — **pick the strongest model and you never get caught under-powered.** It is also the single most common source of avoidable inference-cost overruns, because it treats capability as free.\n\nIt isn't: the gap between the top tier and the small tier is **20–50× in cost and 3–5× in latency**, paid on every request forever. ==\"Which model?\" is not a platform default you set once — it is a per-use-case decision==, and defaulting to the biggest model means paying frontier prices for capability most of your traffic never uses.\n\nThe tiers, cheapest capability last: **frontier** (GPT-4o, Claude Opus) — highest capability, highest cost; **mid-tier** (GPT-4o-mini, Claude Sonnet) — balanced; **small/fast** (Claude Haiku, Gemini Flash Lite) — optimized for speed and cost; **open-source** (Llama 3, Mistral) — self-hosted across the same spectrum.",
      "More parameters means more capability — that's true. But ==\"more capability\" doesn't mean \"better per-use-case quality\" for tasks already within a smaller model's ceiling.==\n\nIntent classification with 5 categories is within the ceiling of Claude Haiku. Running GPT-4o on it adds **zero accuracy** while adding 20–50× the cost per request and 3–5× the latency. The frontier model's extra capability doesn't help — you're paying for **unused headroom**, and that headroom compounds at volume.",
...
task never touches — the definition of wasted spend.` },
      "For the three use cases:\n\n- **Real-time chat widget** — 500ms latency budget, but frontier models typically take **2–5s TTFT** (Time To First Token — how long until the model begins outputting text) at peak load. ==A mid-tier or small model is required.==\n- **Nightly batch job** — no latency constraint but high volume; a mid-tier model at **1/10th the cost** may be acceptable if summarization quality holds.\n- **Coding assistant** — needs strong reasoning and code generation. ==This is where frontier models justify their cost, because errors in generated code compound into debugging time that costs more than the API premium.==",
      "Define the **minimum acceptable quality bar** per use case — not \"best possible quality\" but \"good enough to ship.\"\n\nTest your actual task — benchmarks like MMLU measure reasoning, not your specific summarization quality. Measure cost and latency at your expected request volume. ==The answer is almost never \"use the same model for everything.\"==",
    ],
    keyPoints: [
  ...
  "scaling-laws": {
    ...
    explanation: [
      "Before scaling laws, compute allocation was **intuition.** Teams trained large models because large models performed better — but there was no principled way to predict *by how much*, or whether a given budget was better spent on more parameters or more training data.\n\nThe field interpreted early results as: ==given a compute budget, maximize parameters.== GPT-3 (175B parameters, ~300B training tokens) followed exactly this logic.",
...
at the same total compute — at 10× cheaper inference cost.` },
      "Applied to the scenario — **1.5T tokens, fixed compute budget.** By Chinchilla, 1.5T tokens is compute-optimal for a **~75B parameter model**, so the 70B model is close to optimal. The 7B model at 1.5T tokens is receiving **10× more data than Chinchilla prescribes.**\n\nCounterintuitively, ==a deliberately over-trained smaller model often matches or outperforms an under-trained larger model at the same total FLOPs== — it absorbed more signal per parameter. This is the logic behind **Llama 2 and Llama 3**: Meta over-trained smaller models past compute-optimal to get better inference performance per dollar at deployment.",
      "Scaling laws predict **pre-training loss, not downstream task performance** on domain-specific benchmarks — that still requires evaluation.\n\nBut the **inference-time implication** is the piece the researcher's claim misses entirely: a 70B model is **5–10× more expensive per inference token** than a 7B model on the same hardware. If a 7B model can reach equivalent loss by training on more tokens, every future query is permanently cheaper. ==The question is not \"which parameter count wins at training?\" — it's \"what compute-optimal size maximizes quality-per-inference-dollar given this token budget?\"==",
    ],
    keyPoints: [
...
(area scales as linear² → token count scales as area)` },
  ...
  "pretraining": {
    ...
    explanation: [
      "The CTO's intuition — \"train from scratch on our legal corpus and the model will understand law better than any general model\" — feels obviously right and is **almost always wrong.** The hidden assumption is that legal understanding comes from *seeing legal text.* ==It doesn't.==\n\nThe capabilities that actually make a model useful on legal work — tracking entity references across a contract, following conditional logic, reasoning through multi-step arguments, following instructions — **are not domain facts.** They emerge from raw pretraining **SCALE**, and three years of proprietary documents is nowhere near enough scale to grow them. ==Train from scratch and you get a model that has seen a lot of law and cannot reason about any of it.==",
      "Pretraining is the phase where those capabilities form: the model learns by predicting the next token across **hundreds of billions to trillions of tokens** of internet text, books, code, and structured data.\n\nWhat emerges at scale is **not memorization of facts but learned representations** — internal structure for grammar, world knowledge, reasoning patterns, and the ability to generalize. ==A 7B model trained on 1T tokens has fundamentally different capabilities than a 7B model trained on 100B tokens, even if both are then fine-tuned on the same task data.== Same parameters, different scale, different mind.",
      "The domain-pretraining argument — \"our data is so unique a general model won't understand it\" — fails for a second reason: **legal, medical, and financial text is already well-represented** in general pretraining corpora, because it appears in large volumes online.\n\nWhat domain fine-tuning adds on top of a foundation model is a **shift in token distribution** for domain-specific terminology and patterns — ==the same shift you'd get from domain pretraining, at 100–1,000× lower compute cost.== The compute gap *is* the whole argument, so it's worth pricing out rather than asserting.",
...
re-grow from a corpus far too small to grow them.` },
      "The genuine case for domain pretraining exists **only when** the domain has data volumes exceeding **100B tokens** AND a fundamentally different token structure that a general model actively misrepresents — rare programming languages, highly specialized scientific notation, or languages underrepresented in general corpora.\n\nFor standard legal text: ==the reasoning and language capabilities — argument structure, entity references, conditional logic — come from pretraining scale and cannot be acquired from 3 years of proprietary documents alone.==",
...
FINE-TUNE — bake the change into weights:
  ...
  "instruction-tuning": {
    ...
    explanation: [
      "Pretraining established **what the base model can do**: predict the next token well across a vast range of text. What it *didn't* establish: **instruction-following behavior.**\n\nA base model prompted with \"Summarize this document:\" is statistically as likely to **continue writing more document text** as to produce a summary — because both patterns appear in training data. ==The base model's capability is real; its reliable compliance with arbitrary instruction formats is not.==",
...
itself).` },
      "The **\"more capable\" claim from benchmark scores** needs careful interpretation. Some benchmarks are designed for **completion-style prompting**, where the base model's flexibility is an advantage.\n\nInstruction-tuned models score lower on these because ==the benchmark format conflicts with the instruction-following prior — not because capability decreased.== For any real task — answer a question, extract a field, follow a format — the instruction-tuned model outperforms the base, because instruction-following is the **load-bearing capability** for user-facing applications.",
      "For the scenario: ==the instruction-tuned variant is correct for a customer-facing Q&A tool, full stop.== Every user interaction is an instruction, and the base model's benchmark advantage is a **measurement artifact** of completion-style tests.\n\nUse the base model **only** when building a custom fine-tuning pipeline that requires clean parameter initialization without instruction-following bias baked in.",
    ],
    keyPoints: [
