...
Raw scores:  surgeon=1.82  patient=0.8  who=0.3  treated=0.2  the=0.12
...
Temperature rescales:      softmax( logit / T )
  ...
  T = 0.5  (sharpen — logits ×2 before softmax → 4.0, 2.0, 0.0)
     A ≈ 0.87    B ≈ 0.12    C ≈ 0.02     ← near-greedy, top token dominates
...
ALiBi (no rotation):   score −= m·(i−j)   linear distance penalty, no trained params
  ...
  "kv-cache": {
    ...
    explanation: [
      ...
      "The memory cost is the **other side of the trade**. The cache stores K and V for every token, across every layer and every KV head:\n\n`2 × num_layers × num_kv_heads × head_dim × seq_length × bytes_per_element`\n\nFor a 70B model in fp16 using grouped-query attention — the real Llama 2/3 design (80 layers, 8 KV heads, 128 head_dim): `2 × 80 × 8 × 128 × 2 bytes = 0.33MB per token`. ==Each additional token adds another 0.33MB across the full layer stack — linear in conversation length.==",
