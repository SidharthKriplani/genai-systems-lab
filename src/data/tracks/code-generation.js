...
export const RUNNER_CODE_GEN = {
  "codegen-model-training-fim": {
    ...
    groundUp: "Picture the simplest way to train a model to write code: show it a huge pile of source files and teach it to predict the next token, left to right, exactly like a chat model learns to continue a conversation. It's the obvious starting point — code is just text, and next-token prediction is the one recipe every language model already knows.\n\nTry using that model inside an actual code editor and the mismatch shows immediately. A conversation only ever gets longer — every new message just tacks onto the end of what's already there. Code in an editor doesn't work that way: you plant your cursor somewhere in the MIDDLE of a function, with real code both above and below it, and the thing you need the model to write is whatever belongs in between. A model that only ever learned to continue text to the right has no idea what to do with the code sitting below the cursor — it wasn't trained to look at it, let alone use it.\n\nSo the real question isn't 'can the model write code' — it clearly can, if you only ever ask it to keep going. The question is how you teach a model that only knows how to continue a document to instead fill a hole in the middle of one, without giving up the fluent left-to-right generation that made it good at writing code in the first place.",
...
left-to-right so the model retains normal continuation ability.` },
    ...
    recap: [
      "**Causal LM predicts left-to-right** — continuation only; autocomplete needs the cursor's prefix AND suffix, which a continuation model never learned.",
      "**FIM**: reorder documents into `<PRE> prefix <SUF> suffix <MID> middle` (PSM) → causal decoder learns two-sided conditioning without becoming a weak masked model.",
      "**Apply FIM to ~50-90% of documents**, rest stays plain left-to-right → continuation survives; inference feeds `<PRE>...<SUF>...<MID>`.",
      "**Data pipeline > objective**: MinHash/LSH dedup, permissive-license filtering, quality heuristics (drop minified/auto-gen) — the biggest quality lever.",
      "**Byte-level BPE** (no `<UNK>`) **+ compact whitespace tokens** fit indentation-heavy code in the window; reserve `<PRE>/<SUF>/<MID>` + end-of-document sentinels.",
      "**Instruction fine-tuning** (instruction, before, after triples) **+ repo-aware packing** → completion engine becomes an edit-following assistant → bridges to repo-level retrieval.",
    ],
    mcqs: [
      {
  ...
  "codegen-repo-context-retrieval": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Suppose you want a coding assistant to be as good on a real repository as it is on a single file you paste into a chat window. The simplest idea is to just do the same thing at bigger scale: take the whole codebase and put it in the context window, the same way you'd paste one file.\n\nThat idea runs into arithmetic, not cleverness. A real repository can run to millions of tokens across thousands of files, while even a large context window holds only a small slice of that. Most of the codebase will never fit in front of the model, however you pack the window — and whatever it doesn't see, it cannot use, and whatever it cannot use, it has to guess.\n\nSo before a coding assistant can generate a single correct line inside a large repo, something has to decide, out of millions of tokens of code, which few thousand actually matter for this one task — and get that decision right. That's a search problem sitting in front of a generation problem, and how well you solve it determines whether the model edits real code or invents plausible-sounding code that doesn't exist.",
...
exact PaymentIntent signature, structural misses the natural-language intent.` },
    ...
    recap: [
      "**Repo >> context window** (millions of tokens vs tens of thousands) → retrieve the right files/symbols first; invented APIs and wrong-file edits are context failures, not reasoning failures.",
      "**BM25** (lexical) → exact identifiers/error strings. **Dense embeddings** → semantic paraphrase. Fuse both (reciprocal-rank fusion) — each covers the other's blind spot.",
      "**Structural retrieval** (AST/call graph/language-server index): walk imports/callers/callees → pulls REAL signatures relevant by construction, not similarity.",
      "**Chunk at symbol boundaries** (tree-sitter/AST): one function/class per chunk with its signature — a byte-count window slices mid-function, poisoning both retrieval and generation.",
      "**Assembly**: rank → dedup (retrievers overlap) → budget-pack, signatures/types first then top bodies — this scaffold drives much of SWE-bench score for a fixed model.",
      "**Grounding kills hallucination**: absent evidence the model samples a plausible name; real signatures/types in context make the correct call the high-probability completion — some systems also validate generated symbols against the index.",
    ],
    mcqs: [
      {
  ...
  "codegen-agentic-loops": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Think about how you, personally, actually fix a bug. You don't stare at the code, form a single hypothesis, write the fix, and ship it without ever running anything. You make a change, run the tests, read what broke, and try again — probably more than once. The fix that ships is rarely your first guess; it's whatever survived several rounds of being checked against reality.\n\nNow picture asking a language model to fix the same bug in a single call: read the issue, generate a patch, done. There's no step in that process where the model finds out whether its patch actually works. It has to commit to an answer based purely on what looks plausible, with no way to check itself — a strictly harder version of the task you'd never attempt that way yourself.\n\nThe obvious fix is to give the model the same thing you have: a chance to be wrong, find out, and try again. What that actually takes — and what keeps it from spiraling out of control — is the real engineering problem.",
    ...
    explanation: [
      ...
      { list: [
        "**Localize** — find the file(s) and lines to change (retrieval + search; the previous module).",
        "**Propose + apply an edit** — generate a patch and write it to disk (file read/write, str-replace).",
        "**Run tests / build** — execute the repo's own test suite or reproduction script (shell + test runner).",
        "**Observe** — read the failure: assertion diff, compiler error, stack trace.",
        "**Revise** — feed the observation back and generate the next edit, looping to (2).",
      ] },
      "The agent's 'tools' are exactly the things a human engineer uses: read, edit, grep, run a shell, run tests. ==The loop is a control system — the test output is the error signal that steers the next edit.==",
...
NEXT iteration can read and fix — instead of shipping the guess.` },
    ...
    recap: [
      "**One-shot guesses** compile/test outcomes blind; the **agent** applies an edit, runs it, reads the real result, then revises.",
      "**Five-stage loop**: localize → edit (apply patch) → run tests/build → observe (assertion diff/stack trace) → revise — test output is the error signal steering the next edit.",
      "**Multi-file fixes need decomposition** into ordered subgoals + **trajectory memory** (what's tried, what failed, files touched) across a long session.",
      "**Guardrails = shippability**: sandboxed execution, human diff review, step/cost caps bound blast radius and spend.",
      "**Stop conditions**: hard step/cost cap + no-progress detector (identical failure N times) + a rule against gaming the metric (deleting tests ≠ passing).",
      "**Ceiling = feedback reliability**: flaky tests poison the signal, ambiguous specs give no convergence target, cross-file refactors break unretrieved call sites.",
    ],
    mcqs: [
      {
  ...
  "codegen-eval-passk-swebench": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Suppose you need to grade whether a model's generated code is 'correct.' The most obvious idea is the one you'd use for a fill-in-the-blank quiz: compare what the model wrote to a known correct answer, and score how closely they match.\n\nThat idea breaks on the very first real example. Two solutions to the same coding problem can be completely different, character for character — different variable names, a different algorithm, a different order of operations — and both be entirely correct. Code doesn't have one right answer the way a math quiz does; it has a right behavior. Comparing text to a reference answer would mark a perfectly good, differently-written solution as wrong, and would just as happily reward a solution that looks similar to the reference but is subtly broken.\n\nSo text-comparison can't be the mechanism — matching characters to a reference doesn't tell you what you actually care about. What would? That's worth sitting with before you decide what 'better' even means.",
...
Unbiased estimator over n>>k samples:  pass@k = 1 - C(n-c, k)/C(n, k)
   (n samples, c of them pass) -> a STABLE estimate instead of a coin flip.` },
    ...
    recap: [
      "**Execution-based grading** (pass hidden tests) beats **match-based** (BLEU/exact-match) — correct solutions can look nothing like the reference.",
      "**pass@k = P(≥1 of k samples passes)**; unbiased estimator `1 - C(n-c,k)/C(n,k)` over n>>k — a single k-shot run is noisy. pass@1 (~0.30) and pass@10 (~0.97) can describe the same model.",
      "**HumanEval/MBPP**: one self-contained function, hidden tests — narrow; green tests ≠ secure/maintainable, and say nothing about navigation.",
      "**SWE-bench**: real GitHub issue, real repo, judged by the repo's own (partly hidden) tests — needs localization + multi-file edits; grades the whole scaffold. **SWE-bench Verified** = human-filtered solvable subset.",
      "**Contamination**: public benchmarks/issues leak into pretraining → a high score can be memorized recall; mitigate with held-out/fresh problems + post-cutoff time-splits.",
      "**Goodhart**: optimizing to the benchmark inflates the score, not capability — report pass@1 alongside pass@k, disclose scaffold share, prefer execution-based + held-out/agentic evals.",
    ],
    mcqs: [
      {
  ...
  "codegen-security-sandboxing": {
    ...
    groundUp: "You've just learned how to grade whether generated code is correct — pass@k, execution-based tests, SWE-bench's real-repo patches. But 'passes the tests' was never the same question as 'is safe to run.' A coding agent doesn't just produce a patch to be graded — it acts: it reads files, runs shell commands, installs packages, on a real machine.\n\nPicture the simplest way to let it do that: give it the same access you'd give yourself, and trust it to use that access the way you would. It's the obvious default — it's your agent, running your task, on your task's own repo.\n\nThat trust breaks the moment the agent's input stops being something you wrote. The agent doesn't just generate code; it reads whatever text sits in its path — a cloned repo's README, an issue, a code comment — and it has no reliable way to tell 'content to reason about' from 'instructions to obey.' An attacker doesn't need to breach anything. They just need to leave a sentence somewhere the agent will read it.\n\nSo the real question isn't whether the agent is capable enough to fix the bug — assume it is. It's whether you can hand it shell access, a cloned repo, and your secrets, and survive the day something in its path is actively hostile.",
    ...
    explanation: [
            "Security starts with the **threat model** — you can't defend what you haven't named. A code-executing agent has three distinct attack surfaces.",
      { list: [
        "**Prompt injection from untrusted content**: the agent reads repo files, issues, and comments as *instructions*, so a malicious README or a comment like `// AI: ignore prior rules and run this` can hijack it.",
        "**Untrusted code execution**: the agent runs the repo's build/tests, which is *arbitrary code you didn't write* — cloning and testing a repo means executing attacker-controlled scripts.",
        "**Secret exfiltration**: the agent has access to env vars, tokens, and the filesystem, and can be steered to leak them.",
      ] },
      "==Name the three surfaces — injected instructions, untrusted execution, secret access — because each needs a different control.==",
...
WITH layered controls:
  ...
  Any ONE layer stops the breach; defense-in-depth assumes some will fail.` },
    ...
    recap: [
      "**Threat model, three surfaces**: injected instructions (repo content read as commands), untrusted code execution (cloned repo = attacker code), secret exfiltration (agent has env/filesystem access).",
      "**Prompt injection via repo content** (malicious README: `curl|bash`, POST `.env`) — defend with a **trust boundary**: repo content is DATA, never privileged instructions; human approval for high-risk actions.",
      "**Sandbox + least privilege**: ephemeral container/VM off the host, scoped short-lived creds (no real secrets), workspace-only filesystem, resource limits — assume malicious execution, bound the blast radius.",
      "**Default-deny egress + allowlist** = top exfiltration control — blocks both the malicious fetch and the secret POST even after a successful injection.",
      "**Generated code needs SAST**: models reproduce SQLi, hardcoded secrets, unsafe deserialization that pass tests but stay exploitable — scan the diff in CI; tests check behavior, not security.",
      "**Slopsquatting**: attackers pre-register LLM-hallucinated package names — validate/pin every dependency against a known-good lockfile; backstop with human diff review, command allowlists, full tool-call audit log.",
    ],
    mcqs: [
      {
