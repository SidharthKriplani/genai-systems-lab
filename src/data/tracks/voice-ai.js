    ...
    mcqs: [
      {
        ...
        options: [
          "Whisper-large exceeds the GPU's memory budget, so the runtime falls back to CPU inference, which explains the added delay before any reply is heard",
          "The far-end call carrier is transcoding the audio into a lower-quality codec, such as G.729 at 8kbps, adding a re-encoding step that delays every leg of the round trip by a small, roughly fixed amount",
          "Whisper is an attention encoder-decoder that must ingest the whole utterance before decoding, so it can't emit any output until the caller stops speaking",
          "The assistant's text-to-speech engine is queuing behind a slow logging service, so the reply is generated on time but held up before playback",
        ],
        correct: 2,
        explanation: "The third option is correct: Whisper is an attention encoder-decoder that encodes the entire utterance before its decoder can attend over it, so it cannot produce any output until the caller stops talking — a structural wait-then-decode pattern that recorded-clip benchmarks never measure but a live call exposes as dead air followed by a burst of speech. The GPU-memory option is a plausible-sounding but wrong story — even with ample memory and no CPU fallback, the architecture still can't emit partial text mid-utterance; the bottleneck is what the model waits for, not raw throughput. The codec-transcoding option is wrong — that adds a small, roughly constant delay and doesn't produce the 'silent until you stop talking' signature that full-utterance attention does. The logging-queue option is wrong — a logging bottleneck would show up as inconsistent, unpredictable stalls, not the consistent 'nothing happens until end-of-speech' pattern that is Whisper's architectural signature.",
      },
      {
        ...
        options: [
          "RNN-Transducer: its predictor network conditions each token on prior output while streaming, giving native streaming plus context-awareness",
          "CTC, because predicting each frame independently makes it the fastest of the three architectures, and speed alone is what a live call needs most",
          "Whisper-large with a larger inference batch size and FP16 precision, since a bigger batch dimension and lower numerical precision hide the per-request latency behind higher throughput",
          "Any attention encoder-decoder, because attending over the complete utterance is required to reach production-grade accuracy on medical dialogue",
        ],
        correct: 0,
        explanation: "The first option is correct: RNN-T is the one architecture that is both streaming-native (it emits tokens as audio frames arrive) and context-aware (its internal predictor conditions on tokens emitted so far), which is exactly what a responsive live call needs. The CTC option is wrong — CTC does stream and is fast, but because it predicts frames independently it has no built-in language model, so it's weaker on real dialogue; being fastest doesn't make it the right default when accuracy on medical terms matters. The batching option is wrong — a larger batch raises throughput across many concurrent requests, it doesn't change the fact that Whisper needs the full utterance before decoding a single one; per-request latency is untouched. The full-attention option is wrong — full-utterance attention is exactly what makes AED models offline-only; it doesn't buy production accuracy for free at the cost of being unable to stream, which is the opposite of what a live call needs.",
      },
      {
        ...
        options: [
          "Retrain the acoustic model from scratch on a medical corpus, since a general-domain model cannot physically represent the phonemes in drug names",
          "Raise the audio sample rate to 44.1kHz and use 24-bit depth, so the finer frequency and amplitude detail supposedly lets the model separate drug names from ordinary words",
          "Switch from beam search to greedy decoding with a beam width of one so the model commits to its top hypothesis immediately instead of second-guessing rare terms",
          "Bias decoding toward the domain with shallow fusion or word-boosting on a drug vocabulary, so rare correct terms outscore common-sounding words",
        ],
        correct: 3,
        explanation: "The fourth option is correct: the acoustic model already hears the phonemes fine — rare medical terms just score low during decoding because they're underrepresented in general training. Biasing the decoder (shallow fusion with a domain LM, or contextual biasing/word boosting) lets 'metoprolol' outscore a common-sounding alternative without touching the acoustic model. The retrain option is wrong and wasteful — the model can already represent those phonemes; a full retrain fixes a problem that decoding-time biasing solves far more cheaply. The sample-rate option is wrong — 16kHz already captures the full phoneme-distinguishing band (Nyquist gives 8kHz); 44.1kHz adds detail the model can't use and doesn't change which word wins decoding. The greedy-decoding option is backwards — greedy decoding locks onto the single most likely (usually common-word) hypothesis immediately, making the wrong word MORE likely, not less; beam search plus a domain prior is what lets the rare correct term win.",
      },
      {
        question: "Whisper prints fluent text like 'Thank you for watching!' during a silent hold segment, even though nothing was said. Select the TWO accurate explanations for this behavior.",
        options: [
          "Whisper was trained on 30-second windows that always contained speech, so its decoder never learned that silence should map to an empty transcript",
          "On encountering silence, Whisper defaults to fluent, high-frequency phrases it absorbed from its training captions, rather than emitting nothing",
          "The hallucination only occurs when the timestamp task is active at a decoding temperature above 0.6; the timestamp decoder head is generating the extra tokens",
          "A brief burst of speech was present in the audio at roughly -40dB, too quiet for a human to notice, and Whisper correctly transcribed it",
        ],
        correct: [0, 1],
        explanation: "The correct pair is the first two options together: Whisper's training data was drawn from segments that always contained speech, so the decoder was never shown an example where the right answer is 'output nothing' — and when faced with silence it falls back on fluent, commonly-seen phrases from its caption-heavy training data (like sign-off boilerplate), rather than staying quiet. Both facts are needed to explain the behavior: the first explains why it doesn't know to be silent, the second explains what it says instead. The timestamp-task option is wrong — this hallucination shows up in plain transcription with no timestamp request, so it isn't tied to that head. The hidden-speech option is wrong too — no inaudible speech is required; silence alone reliably triggers it, which is the whole point of the failure mode.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "400ms is already past the 800ms budget once you double it for round-trip acknowledgment, so the engineer's math itself was the error",
          "Voice latency must be budgeted at p95/p99, not the mean — a 400ms average can hide a long tail across the additive VAD/ASR/LLM/TTS pipeline",
          "The LLM's average is fine, but the real cause is that the TTS voice was configured to speak unusually slowly, which is unrelated to any measured latency number",
          "The report is accurate and complete; the lag callers notice must be coming from their own home Wi-Fi jitter, a cellular handoff, or Bluetooth headset latency, not the pipeline",
        ],
        correct: 1,
      ...
      },
      {
        ...
        options: [
          ...
          "Switch to semantic endpointing: a model judges completeness from the words, firing fast on complete phrases but waiting through unfinished ones, shrinking the timer safely",
          "Increase the silence timeout to the full 800ms budget plus a 100ms safety margin, so the agent is never wrong about whether the user has finished",
        ],
        correct: 2,
        explanation: "The third option is correct: pure silence timeouts force a binary tradeoff — short timers cut off natural pauses, long timers feel sluggish. Semantic endpointing uses linguistic completeness to decide when to fire, so it can respond quickly to clearly-finished utterances and hold through clearly-unfinished ones, letting you shrink the timer without paying the cutoff cost. The 50ms option is wrong — that triggers on ordinary mid-sentence breaths, producing exactly the cutoff problem product wants avoided; that cost isn't a minor one. The remove-endpointing option is wrong — running the LLM on every partial transcript is expensive and still lacks a principled 'is the user done' signal; it doesn't solve endpointing, it just defers the guesswork elsewhere. The 800ms option is wrong — that consumes the entire latency budget on waiting alone, making the agent maximally sluggish, the opposite of the goal.",
      },
      {
        ...
        options: [
          "Stream every stage — partial ASR downstream, LLM tokens as generated, sentence-chunked TTS — so first-chunk latencies replace full-stage latencies",
          "Buy faster GPUs so each full stage completes more quickly, shrinking the three-second total roughly in proportion to the hardware speedup",
          "Play a pre-recorded filler sound like 'let me think about that' the moment the user stops speaking, so something is audible immediately",
          "Cap the LLM's maximum output length to 50 tokens per turn and disable its reasoning trace and tool-calling ability so the full reply generates faster",
        ],
        correct: 0,
        explanation: "The first option is correct: the three-second figure comes from paying the FULL latency of each stage in sequence before any audio plays. Streaming restructures that — the LLM starts TTS on its first complete sentence, TTS emits its first chunk immediately, ASR forwards partials — so first-audio latency collapses to endpoint + first-token + first-chunk, typically 3–5x smaller. This is a structural fix, not a hardware one. The faster-GPU option is wrong — that shrinks each stage somewhat, but you're still paying full-stage latency in series; the bottleneck is the pipeline's structure, not raw compute. The filler-sound option is a UX band-aid — the actual response is still seconds away; it masks the wait rather than shortening it, and doesn't scale to a real conversational agent. The output-cap option is wrong — a shorter cap speeds up when the FULL reply finishes, but time-to-first-audio depends on time-to-first-token and first-chunk, which output length doesn't touch.",
      },
      {
        question: "The agent has a strong 700ms first-audio latency, but users complain that interrupting it mid-response doesn't work — it talks over them and keeps going. Select the TWO capabilities the agent is missing.",
        options: [
          "Full-duplex audio handling, so the agent can listen for user speech while it is still speaking, instead of only listening between its own turns",
          "Acoustic echo cancellation, so the agent's own audio isn't mistaken by its VAD for user speech, and real user speech isn't masked by the agent's own output",
          "A faster LLM using a distilled 3B-parameter checkpoint with 8-bit quantization and a 2048-token context cap, so the agent finishes its response before the user talks over it",
          "A longer TTS output buffer of several seconds, so the agent can complete its current sentence before considering whether to yield the floor",
        ],
        correct: [0, 1],
        explanation: "The two missing pieces are full-duplex operation and echo cancellation, working together: full-duplex lets the agent's microphone stay active while it's speaking so it can even detect an interruption, and echo cancellation is what makes that detection reliable rather than either drowning in the agent's own audio or false-triggering on it. Without both, the agent literally cannot notice — or trust that it noticed — that the user started talking, so it just keeps going. Low first-audio latency is unrelated to this; the agent already has that and it doesn't help. Making the LLM faster only shortens the reply, it doesn't grant the ability to be interrupted mid-sentence — a short reply can still be talked over. A longer buffer makes the agent HARDER to interrupt, which is the opposite of what's needed.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "The vocoder is non-streaming — it waits for the full spectrogram, then synthesizes the whole sentence at once; a streaming vocoder fixes it",
          "The acoustic model is undersized at only 2 layers, so upgrading it to a larger 12-layer model will make the first audible sample arrive sooner",
          "The reply text is too long at over 40 words with three embedded clauses, so trimming every response down to a handful of words will fix the delay",
          "The delay is caused by network latency to the TTS service, and the recorded-sample benchmarks simply happened to run on a faster local connection",
        ],
        correct: 0,
        explanation: "The first option is correct: first-audio latency lives in the vocoder stage. A non-streaming vocoder waits until the complete spectrogram/token sequence exists and vocodes the entire sentence before emitting any waveform, producing exactly the multi-second wait. A streaming vocoder emits chunks as frames are produced, so the first phoneme is audible almost immediately — and this is invisible to leaderboards that score complete recorded clips. The acoustic-model option is wrong — that model governs what's said and how it sounds, not how quickly the first waveform sample appears; a bigger model doesn't shorten that wait and can even lengthen it. The reply-length option is wrong — reply length affects total playback duration, not time-to-first-audio, which depends on how fast the first chunk is produced; shortening replies is also a UX regression, not a fix. The network option is a distractor — network latency is real but small and roughly constant; it doesn't explain a full second-plus delay or the 'whole sentence synthesized before anything plays' signature.",
      },
      {
        ...
        options: [
          "The model memorizes the reference clip exactly and splices together short recorded slices from it whenever it needs to produce the founder's voice, essentially acting as a soundboard",
          "Zero-shot cloning requires briefly fine-tuning the full model on the reference clip for a handful of epochs before it can speak in that voice",
          "The model transcribes the reference clip and then reads the transcript back using a generic voice that happens to sound similar to the founder",
          "The model conditions on a speaker embedding — a vector pulled from the reference clip — so it generates new text in the founder's voice with no fine-tuning step",
        ],
        correct: 3,
        explanation: "The fourth option is correct: zero-shot cloning conditions a multi-speaker model on a speaker embedding computed from a few seconds of reference audio, then generates arbitrary new text in that voice with no per-voice training step — 'zero-shot' meaning exactly that generalization from one short reference. The splicing option is wrong — the model doesn't splice recorded slices; it synthesizes genuinely new speech for new text, guided by the embedding, so it can say words never present in the clip. The fine-tuning option is wrong — requiring fine-tuning would make this not zero-shot; fine-tuning is an optional path to higher fidelity, not a prerequisite. The transcribe-and-reread option is wrong — the clone reproduces the founder's actual timbre and identity through the embedding, not a merely similar-sounding stock voice.",
      },
      {
        ...
        options: [
          "The vocoder is degrading audio quality with visible spectral artifacts at 8kHz; switching to a higher-fidelity 24kHz vocoder will restore expressive, varied intonation",
          "Neural TTS predicts one average prosody per sentence — natural alone, monotonous in sequence; the fix is explicit prosody control via SSML or control tokens",
          "The speaker embedding was extracted from too short a reference clip, so re-recording a longer sample will add intonation variety to future output",
          "The problem is really about latency; switching to a streaming HiFi-GAN vocoder will also make the delivery sound noticeably more expressive",
        ],
        correct: 1,
        explanation: "The second option is correct: prosody is decided in the acoustic model, and plain neural TTS predicts the single most-likely (average) prosody per sentence — natural per-sentence, but uniform and robotic across a scripted flow like disclaimers. The fix is explicit control: SSML or control tokens for emphasis, pauses, pacing, and pitch, so intonation is specified rather than left to the model's default. The vocoder option is wrong — the vocoder governs waveform fidelity, not intonation; a cleaner-sounding vocoder still delivers flat prosody, just with less noise. The embedding option is wrong — the speaker embedding sets voice identity and timbre, not per-sentence expressiveness; a longer reference clip improves how faithful the voice sounds, not how it's inflected. The streaming option is wrong — vocoder streaming is a first-audio-latency fix and is orthogonal to prosody; it does nothing for monotone delivery.",
      },
      {
        question: "Before shipping the cloned founder voice, select the TWO considerations that are ship-blocking governance requirements, not optional nice-to-haves.",
        options: [
          "Documented consent — a clear, recorded right to use the founder's specific voice, obtained before any cloning takes place",
          "Inaudible watermarking embedded in the synthetic audio so cloned speech can later be detected as AI-generated",
          "Maximizing naturalness scores on public leaderboards so the clone is as indistinguishable as possible from the real founder",
          "Reducing the model to a size that runs fully on-device, since local inference removes the need for consent or disclosure",
        ],
        correct: [0, 1],
        explanation: "Consent and watermarking are the two ship-blocking items here: cloning a voice from a short clip is exactly the capability that enables impersonation and deepfakes, so a documented right to use the specific voice, and a way to later detect that audio is synthetic, are governance requirements rather than optional polish (a third requirement, disclosure to callers, follows the same logic but isn't one of the four options tested here). Maximizing leaderboard naturalness is the wrong direction entirely — pushing indistinguishability without consent or watermarking increases deepfake risk rather than mitigating it. Running the model on-device changes where computation happens, not who has legal rights to a voice or what callers are owed; it doesn't remove the consent obligation.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "The ASR and TTS benchmarks were likely faked or run on unrepresentative data, so the components must be weaker than reported in practice",
          "The LLM in the middle is oversized and therefore slow, and swapping in a smaller model would resolve both the talking-over and multi-turn issues",
          "The problem is purely audio hardware — upgrading the microphones and speaker codecs used on the call would resolve both of these complaints",
          "Component quality is necessary but not sufficient — the failures live in the conversation layer, which component benchmarks never measure",
        ],
        correct: 3,
        explanation: "The fourth option is correct: ASR word accuracy and TTS naturalness are properties of isolated components, but 'talks over people' and 'mishandles multi-turn flows' are properties of orchestration — endpointing, barge-in, dialogue state, error recovery — none of which component benchmarks touch. Strong components are table stakes; the conversation layer is where agents succeed or fail. The faked-benchmarks option is wrong — nothing points to fabricated benchmarks; the numbers can be entirely accurate and still miss what matters for conversation quality. The LLM-size option is wrong — LLM size mainly affects generation latency, not whether turn-taking or dialogue-state logic exists; a smaller model with the same missing logic still talks over people. The hardware option is wrong — codec and microphone quality affect audio fidelity, not when the agent decides to speak or how it tracks a multi-turn flow.",
      },
      {
        ...
        options: [
          "Treat transcripts as probabilistic: check ASR confidence on critical slots and confirm high-stakes entities, re-prompting on low confidence",
          "Keep retraining the ASR model until word error rate reaches zero across every accent and channel condition, since a perfect transcript is the only way to prevent this",
          "Increase the TTS speaking rate to 1.5x so the booking is confirmed and finalized before the caller has time to object to what they hear",
          "Drop the confirmation step from the flow entirely and auto-commit every slot, since confirmations add friction and most callers find them unnecessary",
        ],
        correct: 0,
        explanation: "The first option is correct: ASR is never perfect, so the robust design treats transcripts as probabilistic — check confidence on critical slots and confirm high-stakes entities before acting, re-prompting when confidence is low rather than committing to a guess. A good agent mis-hears safely by confirming first. The zero-WER option is wrong — zero WER is not attainable in practice, and a design that depends on it is brittle; the goal is robustness to the errors that will happen, not their elimination. The faster-speech option is wrong and actively harmful — rushing the caller past the point where they could correct a mistake removes their only safeguard. The drop-confirmation option is backwards — removing confirmation is what let the mis-booking through in the first place; for critical slots, confirm-then-act is the fix, not an obstacle to drop.",
      },
      {
        question: "You're choosing between a cascaded (ASR→LLM→TTS) architecture and a native speech-to-speech model for enterprise phone support that must log every call, apply content guardrails, and reliably trigger scheduling/CRM tool calls. Select the TWO reasons a cascaded architecture fits this requirement better.",
        options: [
          "It exposes structured text at ASR and LLM stages, which a logging and guardrail pipeline needs to inspect before anything reaches the caller",
          "LLM function calls on structured text are more reliable to trigger than an equivalent call from an opaque audio-in, audio-out model",
          "It has structurally lower latency than any speech-to-speech model, since three specialized services running fully in parallel always outrun one big model",
          "It preserves the caller's prosody and emotional tone all the way through to the LLM, giving the guardrail layer richer signal to work with",
        ],
        correct: [0, 1],
        explanation: "The two real advantages are inspectability and reliable tool-calling: cascaded pipelines produce structured text at each stage, so transcripts can be logged, guardrails can filter LLM output before it's spoken, and tool calls are triggered from structured text rather than inferred from raw audio — exactly the enterprise requirements listed. Latency is actually the opposite tradeoff: cascaded pipelines have more hops (ASR, LLM, TTS as separate services) and are generally higher-latency than a single-model speech-to-speech system, not lower — so that option has the direction backwards. Prosody preservation is also backwards: cascading through a text intermediate step is precisely what flattens the caller's tone before the LLM ever sees it; speech-to-speech models are the ones that preserve it.",
      },
      {
        ...
        options: [
          "Block the entire pipeline during the API call so nothing else can interfere, and ignore any user speech or interruption until the tool call returns",
          "Increase the scheduling API's timeout ceiling to a full 30 seconds with automatic retries, so the call eventually completes no matter how long the user waits",
          "Confirm slots before calling the tool, mask the wait with filler and streaming, track dialogue state, and keep barge-in active during the call",
          "Defer the tool call until after the conversation has ended and email the caller the result, so it never competes with the in-call latency budget",
        ],
        correct: 2,
        explanation: "The third option is correct: voice tool-calling is more than emitting a function call. You confirm slots before committing (a wrong slot books the wrong thing), mask the ~2-second wait with conversational filler and streaming so it isn't silent dead air, track dialogue state across turns, and keep barge-in working — if the user interrupts mid-call, the agent cancels in-flight TTS, re-plans, and reconsiders whether the in-flight tool call still matters. The blocking option is wrong — blocking the pipeline and ignoring user speech is exactly what breaks barge-in and makes the wait feel frozen. The timeout option is wrong — a longer timeout doesn't address the dead air or an interruption during the wait; it only risks an even longer silence. The defer option is wrong — deferring the call until after the conversation defeats the purpose, since the caller needs the scheduling result to continue the call; the challenge is handling the latency in-conversation, not avoiding it.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "The WER for those calls was computed with a bug in the alignment code, and a corrected calculation would show a high WER on all of them",
          "WER is an equal-weight edit distance over reference words, so a wrong critical entity scores the same as one dropped filler word",
          "WER is designed for TTS evaluation, not ASR, so applying it to these ASR transcripts was the wrong metric to use from the very start of the project",
          "Those specific calls had corrupted audio files with clipping, so the WER numbers reported for them don't reflect real transcription quality",
        ],
        correct: 1,
        explanation: "The second option is correct: WER counts substitutions, insertions, and deletions over reference length, weighting every word equally. Swapping 'Tuesday' for 'Thursday' is one error, scored identically to dropping a 'the' — but only one breaks the task. So WER can read under 5% while the specific load-bearing entity is wrong, producing exactly this paradox. The computation-bug option is wrong — nothing suggests a bug; the number is genuinely low because most words, including filler and structure, were transcribed correctly. The TTS-metric option is wrong — WER is the standard ASR metric (TTS uses MOS); using it here is correct, it's just insufficient on its own. The corrupted-audio option is wrong — the failures described are entity substitutions visible in the transcript, not audio corruption; WER is very relevant here, it simply weights the wrong things equally.",
      },
      {
        question: "Given the low-WER-but-failed-calls problem, which metric change most directly surfaces the failures WER hid?",
        options: [
          "Report WER to five additional decimal places on a rolling 30-day leadership dashboard so small differences between releases are easier to see",
          "Switch the ASR evaluation from WER to MOS entirely across the whole pipeline, since MOS is generally regarded as the more modern of the two metrics",
          "Only evaluate calls that already have an overall WER above 15% on the weekly report, since those are most likely to contain a critical error",
          "Track entity/keyword error rate — WER over just the task-critical words like dates and account IDs — so a load-bearing error isn't averaged away",
        ],
        correct: 3,
        explanation: "The fourth option is correct: the hidden failures are errors on critical entities, and computing error rate only over those words — dates, names, IDs — surfaces exactly the errors that matter instead of diluting them across correctly-transcribed filler. This weights errors by task impact, the upgrade raw WER lacks. The decimal-places option is wrong — more precision doesn't change what's being averaged; it's still an equal-weight, entity-blind number. The switch-to-MOS option is wrong — MOS rates TTS naturalness through human listening, not ASR transcription accuracy; it has no mechanism for scoring word-level entity errors at all. The high-WER-only option is backwards — the dangerous calls in the scenario had low overall WER, so restricting to high-WER calls would filter out exactly the failures that need catching.",
      },
      {
        ...
        options: [
          "Use an objective proxy — a neural MOS predictor like UTMOS, or MCD against reference audio — to gate nightly CI, then confirm with human MOS before shipping",
          "Skip automated TTS regression testing altogether, since only a human listening panel produces a valid MOS and running one nightly isn't feasible",
          "Report the same 4.2 MOS figure every night on the leadership dashboard, since MOS shouldn't be expected to change unless the model architecture itself changes",
          "Replace MOS with WER for TTS regression testing across every voice in the catalog, since WER can be computed automatically without any human involvement",
        ],
        correct: 0,
        explanation: "The first option is correct: neural MOS predictors (like UTMOS) and MCD are automatic proxies suited to nightly CI, catching regressions without a human panel — but because a predictor is only as reliable as its training distribution and can miss odd prosody or artifacts a person would notice, the proxy gates CI while human MOS confirms before an actual ship. The skip-testing option is wrong — abandoning nightly testing lets quality regressions ship silently; proxies exist specifically to make this feasible without a nightly panel. The static-report option is wrong — any change to the model, input mix, or pipeline can shift perceived quality, and the team isn't re-measuring at all in this option, so it can't be assumed static. The WER-substitute option is wrong — WER scores word-level transcription accuracy for ASR; it has no notion of vocal naturalness and is meaningless as a TTS quality signal.",
      },
      {
        question: "Leadership asks whether the voice product is ready to ship, citing 6% WER and 4.2 MOS. Select the TWO metrics that should actually decide the ship call.",
        options: [
          "End-to-end task success rate on realistic calls — did the caller actually accomplish their goal, such as booking the right appointment",
          "p95 turn / first-audio latency and barge-in / turn-taking quality on real calls, capturing responsiveness component scores miss",
          "The raw component WER number alone, since a sufficiently low WER on last quarter's dashboard was treated as evidence a release is ready",
          "The raw component MOS number alone, since callers' overall satisfaction is assumed to track closely with how natural the synthesized voice sounds",
        ],
        correct: [0, 1],
        explanation: "Task success rate and p95 latency/turn-taking quality are the two end-to-end signals that actually gate a ship decision: task success rate is precisely what would have flagged the scenario's low-WER-but-failed calls, and p95 first-audio latency plus barge-in quality capture the conversational responsiveness that no component score measures. WER and MOS in isolation are the wrong basis for the decision — this is the component-vs-system trap: strong ASR and strong TTS component scores can still describe an agent that fails callers in conversation, which is exactly what happened here, so neither belongs on its own as the ship gate.",
      },
    ],
