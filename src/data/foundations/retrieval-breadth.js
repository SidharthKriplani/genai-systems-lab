...
export const RUNNER_RETRIEVAL_BREADTH = {
  "dense-vs-sparse-retrieval": {
    ...
    scenario: "Six months after that fix ships, a separate team stands up a second RAG bot — this one over your API documentation — and skips straight to embeddings-only, on the theory that 'hybrid felt over-engineered for a docs site.' It works beautifully in the demo. Then a support engineer searches for the exact config key `max_retry_backoff_ms` and gets back a chunk about 'general timeout tuning' — never the one page that actually defines that key. Before you tell them to just bolt BM25 back on: what's the one property of embedding models you'd point to as the actual root cause here — and is there a query pattern on a docs site where you'd expect the *opposite* failure, BM25 missing something dense would have caught?",
    explanation: [
...
RRF (k=60):
  runbook-2048     = 1/(60+1) + 1/(60+3) = 0.01639 + 0.01587 = 0.03227  ← TOP
  ...
  "multi-hop-retrieval": {
    ...
    scenario: "Your RAG bot handles single-fact questions perfectly. Then someone on the support team asks: 'What's the cancellation fee for the plan our biggest customer by revenue is on?' The bot confidently answers with the Starter-plan fee — but your biggest customer is actually on Enterprise, which has a completely different cancellation schedule. You pull the retrieval logs: no chunk mentions both 'biggest customer by revenue' and 'Enterprise cancellation terms' together — one chunk ranks accounts by revenue, a separate chunk lists cancellation terms per plan, and neither was retrieved alongside the other. Before you reach for a decompose-retrieve-reason-retrieve loop: is this actually a case where that loop earns its cost, or is there a cheaper fix that would work just as well here — and what specific property of this question would you check first to know which one you're looking at?",
    explanation: [
      "Some questions can be answered by finding *one* relevant passage. Others can only be answered by *chaining* facts that live in *different* passages — and that difference is a hard architectural boundary, not a matter of degree. Start by naming it precisely.\n\nA **multi-hop question** is one whose answer requires combining information from *two or more documents*, where you can't even *know which second document you need* until you've read the first. The running example ahead is the canonical shape: to find the parental-leave policy you must *first* discover *which country* the VP is in (hop 1: VP → Germany), and *only then* can you look up *that country's* leave policy (hop 2: Germany → policy). ==The identifying signature of a multi-hop question is a chain of dependencies: the answer to the second lookup is gated on the answer to the first.==",
