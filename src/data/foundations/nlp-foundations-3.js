...
export const RUNNER_NLP_3 = {
  "nlp-encoder-decoder-objectives": {
    ...
    groundUp: "The previous module ended on the question that created the modern era: if attention does the real work, why keep the recurrence? This module picks up right after that leap — because the transformer that answered it isn't one architecture, it's a *kit*, and the same blocks assemble into three machines with wildly different talents.\n\nHere's the puzzle: BERT, GPT, and T5 are built from identical parts — same attention, same feed-forward layers — yet one is brilliant at understanding and cannot write, one writes fluently and took over the world, and one quietly owns translation. If the building blocks are identical, where does the difference live?\n\nIn exactly two places. First: what each token is *allowed to look at* — everything on both sides of it, or only what came before it. Second: what game the model plays during pretraining — fill in hidden words, or predict the next one. Those two choices are the **attention mask** and the **pretraining objective**, and together they split transformers into three families: **encoder-only** (BERT), **decoder-only** (GPT), and **encoder-decoder** (T5, BART).\n\nWe'll take the families one at a time — what each mask looks like, what each training game forces the model to learn, which jobs each is built for — and end with the strategic twist interviewers love: why the do-everything family won the platform war even though the specialists are often smaller, faster, and cheaper at their own specialty. By the end, 'which model should we reach for?' stops being a matter of taste and becomes two questions with checkable answers.",
    scenario: "You join a team that has three half-finished NLP prototypes and no one can agree on which model to reach for. One engineer fine-tuned BERT for a support-ticket classifier and it works beautifully, but when the PM asked her to make it 'also write the reply,' she couldn't — the model simply has no way to generate text. Another engineer built a chatbot on GPT and it generates fluent replies, but when asked to produce a single dense embedding for retrieval it felt like the wrong tool. A third is translating docs with T5 and swears by it for that but nothing else. In your architecture-review interview, the question lands: *why* are these three transformers good at different things, when they're all 'just transformers'? Before you answer, run each one through the two questions you now have: what can this token see, and what game did pretraining play? Work it out for each of the three — BERT's classifier, the GPT chatbot, the T5 translator — before you check yourself: which of the two questions is the one each of their failures actually traces back to?",
    explanation: [
      "Every model in the scenario is built from the same transformer block. What makes BERT an *understanding* model, GPT a *generation* model, and T5 a *translation* model is not the block — it's ==two coupled choices:==",
      { list: [
        "which tokens each position is allowed to *attend to* (the attention mask)",
        "what self-supervised game the model plays during pretraining (the objective)",
      ] },
      " Get those two right in your head and the entire 'which family?' question answers itself. There are three canonical families, so we take them one at a time.",
      "**Family 1 — encoder-only (BERT).** Every token attends to *every other token*, both left and right. There is no masking of the future; the attention is fully **bidirectional**. Because a token can see the whole sentence at once, the representation it builds is deeply *contextual* — the word 'bank' resolves to river-bank or money-bank using words on either side. The pretraining game that this enables is **Masked Language Modeling (MLM)**: randomly hide ~15% of the tokens — a fixed masking rate chosen by BERT's designers, not learned or derived — and train the model to predict each hidden token *from both sides at once*. \n\nThat bidirectionality is exactly why BERT is superb at *understanding* tasks — classification, named-entity recognition, sentence-pair scoring, and producing embeddings for retrieval — and exactly why it ==cannot naturally generate text==. To generate, you must produce token 1, then token 2 given token 1, and so on left-to-right; but BERT was trained assuming it always gets to peek at the right-hand context, which during generation does not yet exist. The support-ticket classifier worked; asking it to *write* the reply is asking the wrong architecture to do a job its training forbids.",
...
DECODER (GPT) — causal mask, look LEFT only
  ...
  "nlp-classical-tasks": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Step back from the architecture arms race for one module, because there's a layer of NLP that predates all of it and still quietly runs inside production systems today: the classical tasks. Before a machine can act on text, someone has to answer questions like — which words name a person or a company? What does that pronoun refer back to? What's the grammatical skeleton holding this sentence together?\n\nEach of those questions is an old, precisely-defined task with its own name — part-of-speech tagging, named-entity recognition, parsing, coreference resolution — and here's why they still matter in the LLM era: an LLM can *approximate* all of them from a prompt, but it costs real money per document, runs slowly at scale, and can quietly invent an answer that isn't in the text. A small fine-tuned tagger costs nearly nothing, answers in milliseconds, and can only ever point at words that actually exist in the document. When you're processing bulk documents every night and a downstream tool needs to point at the exact place in the text an answer came from, that difference is the whole ballgame.\n\nThere's also a genuinely elegant idea at the heart of this module, worth the ticket price by itself. How do you teach a model to find *multi-word spans* — 'Bank of America' is one thing, not three — using a tool that can only label one token at a time? Pause on that before reading on: if you can only ever attach one label per token, how would you tell the model where an entity starts and where it stops? The answer is to encode the span boundaries into the labels themselves, and that trick has a name: **BIO tagging**. Its companion insight — score the whole label sequence jointly, so an illegal pattern of labels can never be emitted — is the **CRF**, the workhorse that carried sequence labeling for a decade and still sits on top of neural taggers today.\n\nWe'll climb the task ladder bottom-up, see what each rung means — and how sequence labeling in particular was cracked before neural networks and after — and end with the production decision rule: when to call the LLM, when to run the tagger, and why serious systems usually run both.",
    scenario: "Your team is standing up a pipeline that reads incoming legal contracts and pulls out the parties, dates, governing-law jurisdiction, and obligations. A junior engineer proposes 'just ask GPT-4 to extract everything as JSON,' and in a demo it looks magical. But in the pilot, three problems surface: on 40,000 contracts a night the LLM bill and latency are brutal; occasionally it silently invents a party that isn't in the document; and the downstream database needs *exact character spans* so a lawyer can click a value and jump to where it appears. In the review, the staff engineer asks you to explain the *classical* NLP tasks underneath this — POS tagging, NER, parsing, coreference — how they were solved before LLMs, and why production teams still run small fine-tuned taggers instead of an LLM for everything. Take one concrete pair from a contract — 'Acme Corp' and 'Beta LLC' — and work out for yourself why a fine-tuned BIO/CRF tagger will reliably emit `B-ORG I-ORG` for each span while a free-running LLM might merge them, split one, or invent a third party that was never in the text.",
    explanation: [
...
DEPENDENCY PARSE  (arcs: word -> its head)
  ...
  pobj = object of the preposition. Word-to-word.
...
CONSTITUENCY PARSE  (nested phrases)
  ...
  Brackets inside brackets, not word links. S = sentence, NP = noun
  phrase, VP = verb phrase, PP = prepositional phrase (nested inside VP).` },
  ...
  "nlp-text-classification": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Now put everything to work on the single most common job in applied NLP: read a piece of text, assign it a label. Spam or not. Positive or negative. Which team's queue does this ticket belong in? It sounds almost too simple to deserve a module — and that's exactly the trap, because both the modeling and the measuring hide sharp edges that bite production systems constantly.\n\nStart with a puzzle from the modeling side. The oldest serious text classifier works by pretending that, once you know the label, every word in a document is independent of every other word — an assumption so obviously false that the method is literally named for it. And yet it ships, it wins baselines, and it's often the right first thing to build. Why does a wrong assumption keep working? Because picking the best label only needs the *ranking* of the candidates to be right — the probabilities themselves can be badly off as long as the winner still wins. That classifier is **Naive Bayes**, and it opens a ladder that climbs through logistic regression over TF-IDF features, then fine-tuned BERT, up to prompting an LLM with no training data at all — each rung buying accuracy at a price in cost, latency, or labeled data.\n\nThen the measuring side, where the real damage happens. A model can score 96% accuracy while being completely useless — missing nearly every case you actually care about — whenever the thing you care about is rare. Seeing precisely how accuracy lies, and which numbers to watch instead, is arguably the most transferable skill in this whole gym: it applies to every classifier you will ever ship, text or not.\n\nWe'll climb the model ladder first, then spend real time on the evaluation traps — class imbalance, decision thresholds, and the macro-vs-micro averaging choice — and end on the staff-level move: sometimes the fix isn't a bigger model at all, it's a better metric.",
    scenario: "You own the model that routes and scores incoming product reviews as positive or negative for a retail app. It started as a scrappy Naive Bayes classifier on bag-of-words features, shipped in an afternoon, and has quietly served millions of reviews. Now leadership wants better accuracy and asks: should we replace it with fine-tuned BERT, or just prompt an LLM zero-shot? Meanwhile a second team ships a five-way topic router for support tickets (billing, shipping, quality, account, other) and reports 91% micro-F1 — until someone checks the 'account' category, a rare 3% slice of tickets, and finds it's being misrouted more often than not. Your interview task: explain text classification from Naive Bayes up through LLMs, the evaluation traps (imbalance, thresholds, precision/recall/F1, macro vs micro), and — for the second team specifically — why a healthy-looking micro-F1 can still be hiding a badly broken rare class, and which single number would have caught it.",
    explanation: [
...
Per-word log P(w|c)  (learned from the training corpus):
  ...
  logs turn the product into a numerically stable SUM.` },
      ...
      "Naive Bayes' independence assumption caps how far it can go — 'not' and 'good' get scored as if 'not' contributes the same regardless of what follows it, so it can't learn that 'not good' and 'good' point opposite directions. Drop that assumption and let the model learn a weight per feature instead, and you get the next rung. Above Naive Bayes sits the classic **linear** tier: **logistic regression** or a **linear SVM** over **TF-IDF** features (term frequency times inverse document frequency — common words down-weighted, distinctive words up-weighted). These drop the independence assumption, learn *weights* per feature, and typically beat Naive Bayes on accuracy while staying fast, cheap, and interpretable (you can read the highest-weighted words per class). For years this — TF-IDF + logistic regression — was the default production text classifier, and it remains an excellent, hard-to-beat baseline.",
      "TF-IDF + logistic regression still has a ceiling, though: it scores word *counts*, so it has no way to tell 'not good' from 'good not' — word order is thrown away entirely. Fixing that means learning from the sequence itself, not just its bag of counts. Then the **neural progression**: **CNN/RNN text classifiers** learned features from word embeddings instead of hand-built counts (capturing some word order and local phrases); **fine-tuned BERT** brought deep bidirectional context and became the accuracy king for classification when you have labeled data; and **zero-shot LLM classification** lets you classify *with no training data at all* by prompting ('is this review positive or negative?'). The tradeoffs are the interview substance: ==more powerful models need more (BERT: labeled data; LLM: none) but cost more latency and money and are less interpretable; simpler models (NB, TF-IDF+LR) are fast, cheap, interpretable, and often *good enough*.== The right choice depends on labeled-data availability, latency/cost budget, and how much accuracy the last few points are worth. Pause before the next section: if only 4 in 100 reviews are ever negative, what does a lazy model that always says 'positive' score on plain accuracy — and is that model actually good?",
