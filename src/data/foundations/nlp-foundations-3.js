    ...
    mcqs: [
      {
        ...
        options: [
          "BERT has too few parameters to generate; a larger BERT would generate fine",
          "BERT uses bidirectional attention and was trained with Masked Language Modeling to predict tokens using both left and right context; generation is inherently left-to-right where right-hand context doesn't exist yet, so the model's training assumption is violated",
          "BERT can generate, but only in languages other than English",
          "Generation requires cross-attention, and BERT's causal mask removes it",
        ],
        correct: 1,
        explanation: "Option B is correct: BERT is an encoder-only model with fully bidirectional attention, pretrained via MLM to reconstruct masked tokens using context on *both* sides. Autoregressive generation produces tokens one at a time left-to-right, so at each step the right-hand context does not yet exist — precisely the context MLM taught BERT to rely on. The training objective and the generation process are mismatched, which is why BERT cannot naturally generate. Option A is wrong: scale doesn't fix a masking/objective mismatch; the limitation is architectural, not capacity. Option C is nonsense — language has nothing to do with it. Option D is doubly wrong: BERT is bidirectional (it has no causal mask), and cross-attention belongs to encoder-decoder models, not to the reason BERT can't generate.",
      },
      {
        question: "Why did decoder-only (GPT-style) models become the dominant general-purpose GenAI architecture, even taking over tasks that encoder-only and encoder-decoder models were designed for?",
        options: [
          "Because causal attention is computationally cheaper than bidirectional attention per token",
          "Because next-token prediction is a simple self-supervised objective that scales on any unlabeled text, the models exhibit in-context/few-shot learning so new tasks are specified in the prompt, and one model can do everything (classify, summarize, translate, chat) via 'continue this text'",
          "Because decoder-only models are always smaller and faster than a fine-tuned BERT for classification",
          "Because bidirectional attention cannot be trained on GPUs at scale",
        ],
        correct: 1,
        explanation: "Option B is correct: three properties drove decoder-only dominance — (1) next-token prediction is trivially self-supervised on any text, so it scales to enormous unlabeled corpora; (2) at scale these models do in-context learning, letting you specify a new task purely in the prompt with no fine-tuning; and (3) framing every task as 'continue this text' collapses the model zoo into one general model. Option A is a red herring — the dominance is about scaling, generality, and in-context learning, not a per-token compute edge (and bidirectional attention is not the bottleneck). Option C is actually false as stated: a fine-tuned BERT is typically *smaller and faster* than a giant decoder for classification, which is why encoders remain the better specialist tool even though decoders generalize. Option D is factually wrong — encoder and encoder-decoder models train at scale on GPUs routinely.",
      },
      {
        ...
        options: [
          "Encoder-only (BERT), because bidirectional attention understands the source perfectly",
          "Encoder-decoder (T5/BART), because a bidirectional encoder reads the whole source while a causal decoder generates the target left-to-right and cross-attends back into the encoded source — the exact shape of 'read all of A, then write B while consulting A'",
          "Decoder-only (GPT), because it is the only family capable of any generation at all",
          "Encoder-only (BERT) with a classification head over the vocabulary",
        ],
        correct: 1,
        explanation: "Option B is correct: translation is the archetypal sequence-to-sequence task — a distinct input must be transformed into a distinct output. Encoder-decoder models are purpose-built for this: the encoder reads the source bidirectionally to understand it fully, the decoder generates the target autoregressively, and cross-attention lets each output token consult the encoded source at every step. Option A is wrong: encoder-only models produce representations but cannot generate the target sequence. Option C is wrong on two counts — decoder-only is not the *only* generative family (encoder-decoders generate too), and while a large GPT *can* translate via conditional generation, encoder-decoder is the most *natural* fit for the seq2seq shape. Option D is wrong: a classification head over the vocabulary does not perform autoregressive sequence generation and mismatches the task.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "Because BIO makes the model run faster on GPUs",
          "Because entities are multi-token *spans*, and BIO encodes span boundaries into the tags (B=begin, I=inside, O=outside) so that 'Bank of America' is recognized as one three-token ORG rather than three separate labels — turning a span task into a coherent token-labeling task",
          "Because per-token classification cannot assign entity types at all",
          "Because BIO tagging removes the need for any training data",
          "",
        ].filter(Boolean),
        correct: 1,
        explanation: "Option B is correct: entities span multiple tokens, so the model must know where an entity *starts* and *ends*. BIO encodes exactly that — B marks the first token of an entity, I marks continuation tokens, O marks non-entities, with the type appended (B-ORG, I-ORG). This lets 'Bank of America' be one ORG span instead of three disconnected labels, expressing a span problem as per-token classification with boundary structure. Option A is wrong — BIO is about representing spans, not GPU speed. Option C is wrong — plain per-token classification *can* assign types, it just can't represent multi-token span boundaries coherently, which is the whole point of BIO. Option D is wrong — BIO is a labeling scheme; it still requires labeled training data.",
      },
      {
        question: "A CRF is preferred over classifying each token independently for sequence labeling. What is the core reason?",
        options: [
          "A CRF uses fewer parameters, so it trains faster",
          "A CRF scores the entire tag sequence jointly and normalizes globally, so it enforces legal transitions (e.g., I-PER cannot follow O) and outputs the single best *consistent* labeling; independent per-token classification can emit illegal, incoherent tag sequences",
          "A CRF is a deep neural network, whereas per-token classifiers are always linear",
          "A CRF eliminates the need for the BIO tagging scheme",
        ],
        correct: 1,
        explanation: "Option B is correct: the label of each token depends on its neighbors (I-PER can only follow B-PER or I-PER, never O). A CRF models the whole sequence and normalizes over all possible labelings *globally*, so it learns valid transition constraints and selects the single most consistent tag sequence — preventing the illegal outputs (an I-ORG with no preceding B-ORG) that independent per-token classifiers can produce. Option A is wrong — the advantage is global consistency, not parameter count or speed (CRFs add inference cost). Option C is wrong — a CRF is a probabilistic sequence model, not inherently a deep network; it's often layered on top of neural or linear features (BiLSTM-CRF, BERT-CRF). Option D is wrong — a CRF still operates over BIO/BIOES tags; it enforces their transitions rather than replacing the scheme.",
      },
      {
        question: "On a pipeline extracting parties and dates from 40,000 contracts a night, a team keeps a fine-tuned spaCy-style NER tagger instead of calling an LLM for every document. Which combination of reasons best justifies this?",
        options: [
          "The LLM is less accurate on every possible input, so it should never be used",
          "Latency and cost at high volume, reliability with structured guarantees (a sequence labeler can only emit spans that actually exist in the text, avoiding hallucinated entities), and exact character-span offsets for click-to-source — advantages an LLM's free-form JSON doesn't guarantee",
          "Fine-tuned taggers require no training data, unlike LLMs",
          "LLMs cannot perform named-entity recognition at all",
        ],
        correct: 1,
        explanation: "Option B is correct: at 40,000 documents a night, a fine-tuned tagger is far cheaper and lower-latency than an LLM; because it *points at* the input, every extracted span provably exists in the document (no hallucinated parties); and token labeling yields exact character offsets for click-to-source, which free-form LLM output doesn't guarantee. Option A overstates it — LLMs are often *more* flexible and better on messy, long-tail extraction; the point is right-tool-per-constraint, not 'never use the LLM.' Option C is false — fine-tuned taggers absolutely require labeled training data. Option D is false — LLMs can do NER zero/few-shot; the tradeoff is reliability, latency, cost, and exact spans, not capability.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "It assumes the training data is perfectly balanced; it works because most corpora happen to be balanced",
          "It assumes every word is conditionally independent of the others given the class (so P(words|c)=∏P(word|c)) — clearly false, but it still usually selects the correct class because ranking classes needs the right argmax, not calibrated probabilities",
          "It assumes words are drawn uniformly at random; it works because of Laplace smoothing",
          "It assumes the classes are independent of the words, which is why priors don't matter",
        ],
        correct: 1,
        explanation: "Option B is correct: the naive assumption is *conditional independence of features given the class*, letting P(words|c) factor into a product of per-word probabilities. That's false in real language ('not' and 'good' interact), but Naive Bayes only needs to rank classes correctly, and the argmax is often right even when the estimated probabilities are miscalibrated — hence a strong, cheap baseline. Option A is wrong — the assumption is about word independence, not class balance, and NB does not require balanced data. Option C misstates the assumption (it's independence given the class, not uniform-random words) and misattributes its success to smoothing, which only prevents zero probabilities. Option D is backwards — priors P(c) very much matter in Bayes' rule; the independence is among words, not between classes and words.",
      },
      {
        ...
        options: [
          "The model is well-calibrated; 96% accuracy proves it is working and no change is needed",
          "This is the class-imbalance trap: predicting the majority class yields high accuracy while catching none of the rare class; report precision, recall, and F1 (harmonic mean) on the negative class — recall on negatives is ~0, so its F1 is ~0, revealing the failure accuracy hid",
          "The problem is underflow in the probability computation; log-probabilities would fix the accuracy",
          "The fix is to raise the decision threshold above 0.5, which always improves recall on the rare class",
        ],
        correct: 1,
        explanation: "Option B is correct: with 4% negatives, a model that predicts positive for everything scores 96% accuracy yet has zero recall on the class you care about. Accuracy is a trap on imbalanced data; precision, recall, and F1 on the minority class expose it — recall on negatives near 0 drives F1 near 0. Option A is wrong — high accuracy here reflects imbalance, not a working model. Option C confuses concerns — underflow and log-probabilities are Naive Bayes numerics, unrelated to the imbalance/metric problem. Option D is wrong on direction — *lowering* the threshold catches more negatives (raising recall at some precision cost); raising it does the opposite, and threshold tuning trades precision against recall rather than 'always improving' recall.",
      },
      {
        question: "For a multiclass model where several rare classes matter as much as the dominant one, why might you report macro-F1 rather than micro-F1?",
        options: [
          "Micro-F1 is always mathematically incorrect for multiclass problems",
          "Micro-F1 pools all decisions so large classes dominate and can mask poor performance on rare classes; macro-F1 averages the per-class F1 scores equally, so a tiny class counts as much as a big one — the right choice when minority-class performance matters",
          "Macro-F1 requires no labels, whereas micro-F1 requires labeled data",
          "Macro-F1 and micro-F1 are identical whenever the data is imbalanced",
        ],
        correct: 1,
        explanation: "Option B is correct: micro-F1 aggregates true/false positives and negatives across all classes before computing the score, so the dominant class drives the result and a rare class doing badly is hidden — the accuracy trap in another form. Macro-F1 computes F1 per class and averages them with equal weight, so each class (including rare ones) contributes equally, which is what you want when minority-class performance matters. Option A is wrong — micro-F1 is a valid, useful metric (it just answers a different question). Option C is nonsense — both require labels. Option D is the opposite of the truth — macro and micro diverge *most* under imbalance; they coincide only in special balanced/degenerate cases.",
      },
    ],
