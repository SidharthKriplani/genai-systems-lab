...
eval               contamination impossible          holder; can't be
  ...
  (96%→72% here — same overconfidence shape as the 95%→70% figure
  used above, just real per-bin numbers) — exactly the kind of bin
  a confidence threshold trusts most.` },
      ...
      { list: [
        "plot a reliability diagram and compute ECE on *your* distribution",
        "expect RLHF-induced overconfidence and don't take stated probabilities at face value",
        "recalibrate with temperature scaling and re-measure ECE",
        "only then set thresholds",
      ] },
...
FIX:  temperature scaling — divide logits by T>1 to soften; fit T on
      ...
      "The closing scenario next puts this to work on a router that trusts 95%-confidence answers into an empty human queue — see if you can name the property that failed, and why it isn't accuracy, before the reasoning does. Afterward, the Hands-On interactive lets you drag a reliability curve off the diagonal, watch ECE climb, and pull it back with temperature scaling without touching accuracy.",
    ],
    keyPoints: [
  ...
  "prompt-caching": {
    ...
    scenario: "Now let's put all of that to work on a real one. Your agent runs on a beefy 9,000-token system prompt: tool definitions, formatting rules, a long few-shot block, and safety instructions — the same every call. On top of that the user's actual question is usually ~150 tokens. Latency to first token feels sluggish and the input-token bill is brutal, and it's obviously dominated by that giant fixed preamble you resend on every single request. A teammate says 'just make the system prompt shorter,' but you need those instructions. Take a moment before reading on: given everything just covered, where is that cost actually going, and what can you skip without deleting a single instruction? Here's the reasoning, step by step. The cost lives in *prefill* — the phase that builds the KV cache for the whole prompt — and you pay it in full on every call even though 9,000 of your ~9,150 tokens never change. So you don't need a shorter prompt; you need a *cacheable* one: keep the 9,000 fixed tokens as a stable prefix at the front, move anything per-request (the user turn, any timestamp or IDs) to the very end, and mark the prefix for caching if your provider requires it. Then the expensive prefill over those 9,000 tokens happens roughly once and is reused: TTFT drops because you skip re-prefilling the preamble, and the input bill drops because the cached prefix is billed at a fraction of full price.",
    explanation: [
...
CACHE-FRIENDLY (static first, dynamic last):
  ...
  "multiturn-context": {
    ...
    explanation: [
      ...
      "That resend strategy hits two walls at once. The hard wall is the **finite context window** — eventually the transcript won't fit, and something must be dropped. The *soft* wall is more insidious and arrives sooner: even when everything technically fits, models don't attend uniformly across a long context — they reliably attend best to the *beginning* and the *end*, and worst to the *middle*. ==A constraint stated at turn 3 sinks into the middle of a 30-turn transcript and gets effectively ignored, not because it fell out of the window but because the model under-attends to the middle.== This effect has a name: **context rot**, or **lost-in-the-middle**. Long context degrades usefulness *before* it hits the size limit.",
...
SUMMARIZATION (compress): condense old turns into a summary
  [ "SUMMARY: user's early constraint noted, wants X, ruled out Y" ][ recent turns ]
...
RETRIEVAL (history as a store): embed all past turns, fetch only the
  ...
  [ system ][ fetched: early-constraint turn ][ recent turns ][ user Q ]
