...
export const RUNNER_DEEPEN_THIN = {
  "reranking": {
    ...
    scenario: "A different team's product search retrieves the top 8 products by embedding similarity, and recall@100 checks out at 0.97 — the right product is almost always somewhere in that window. But conversion on search results is flat, and when you sample sessions, the product a user actually clicks is sitting anywhere from rank 6 to rank 40 inside that window, never reliably near the top. One engineer wants to just raise top-k from 8 to 30 so more of the window reaches the results page. Another wants to add a cross-encoder rerank stage instead, but is worried about the extra latency on every search. Before you weigh in: which of the two is actually solving the problem you're seeing, which metric would you check first to be sure, and what does a rerank stage cost you here that raising top-k doesn't?",
    explanation: [
      "Retrieval and reranking are two different scoring machines with two different cost/quality profiles, and RAG works best when you use each for what it is good at.\n\nYour first stage is a **bi-encoder** (the embedding model behind your vector DB). It encodes the query into a vector *once*, encodes every document into a vector *once* (offline, ahead of time), and scores a query-document pair by a cheap **dot product or cosine similarity** between the two vectors. Because the document vectors are precomputed and the comparison is just a distance in vector space, you can score *millions* of documents in milliseconds via an index that finds approximate nearest neighbors without scanning everything — an ANN index (HNSW, IVF). ==That speed is the entire reason bi-encoders exist — but it comes from a compression you should be suspicious of.==",
      "The compression is this: a bi-encoder squashes an entire document into **one fixed vector before it has ever seen the query**. Query and document never interact until the final dot product. So a subtle mismatch — the query asks about *refunds after 30 days* and the chunk covers *refunds within 30 days* — can be invisible, because both map to nearly the same region of embedding space. The bi-encoder trades **term-level, query-aware reasoning** for the ability to precompute. Pause on that trade for a second: if a document never got to see the query while it was being scored, would you expect it to reliably land in the right neighborhood, or to land at the exact right rank inside that neighborhood? That is why **recall is high but precision at the very top is mediocre**: the right chunk is *in* the neighborhood (recall@50 = 0.94), but its exact rank among near-neighbors is noisy (it lands at 18, not 3).",
...
Cross-encoder cost is ~LINEAR in k (one forward pass per candidate):
     ...
     → n is bounded by context budget & distractor tolerance, not by the reranker` },
      "The costs are real and you should quote them. A cross-encoder adds **tens to a few hundred milliseconds** and its cost scales roughly **linearly in k** — reranking 100 candidates is ~2× the work of 50. On CPU it can dominate your latency; on a GPU you batch the k pairs but still pay for the accelerator. There are middle grounds: **late-interaction models like ColBERT** precompute per-*token* document embeddings and do a cheaper query-aware `MaxSim` match — more accurate than a bi-encoder, cheaper than a full cross-encoder — and hosted rerank APIs (Cohere Rerank, Voyage, Jina) trade a network hop for not self-hosting, while open-source families like BGE-reranker give you the same architecture self-hosted.\n\n**When is reranking worth it?** When **recall@k is already high but precision@n is low** — the same pattern you just walked through above — the answer is being retrieved but ranked too low to make the cut. It is *not* worth it when recall@k itself is poor: a reranker can only reorder what stage 1 handed it, so if the right chunk isn't in the top-k, reranking cannot invent it — fix retrieval (embeddings, chunking, hybrid/BM25) first. ==Reranking is a precision tool; reaching for it to fix a recall problem is the classic misdiagnosis.==",
    ],
    keyPoints: [
