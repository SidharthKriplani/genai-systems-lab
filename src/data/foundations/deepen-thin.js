...
Cross-encoder cost is ~LINEAR in k (one forward pass per candidate):
  ...
  "rag-eval": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Let's start with the most tempting number in the world and why it lies to you. You have a RAG system, and the obvious way to grade it is one score: *what fraction of answers were correct?* Say it's 78%. Feels informative. It isn't — at least not for the thing you actually need, which is *knowing what to fix*.\n\nHere's the reason, and it's the spine of this module. A RAG system is really two machines wired one after another — one that *finds* the right passage, one that *writes* an answer from it — plus a trio of checks that watch that second machine from three angles at once: did it stay faithful to what it found, did it actually answer the question asked, and was what it found even worth answering from. Any of these can break on its own. So a single blended 78% is like a car dashboard with one lamp labeled \"something's off\" — true, useless. You can't tell whether the finder missed the page, the writer ignored a page it *did* get, or the writer answered a question nobody asked.\n\nSo we'll build up how to *decompose* that one number into stage-specific measurements, and how to gather the small ground-truth set that powers them. Take your time. The reward is turning \"it's 78%\" — a shrug — into \"retrieval is fine, faithfulness is the leak\" — a plan.",
    scenario: "Your RAG assistant scores 8.2/10 on a single 'answer quality' LLM-judge metric, and leadership is happy — until a customer catches it citing a policy that doesn't exist in any of your documents. You suspect one of two things happened: either retrieval handed the generator the wrong chunks (so even a perfect answer would've been impossible), or retrieval was fine and the generator ignored the context, inventing a plausible-sounding answer from its own parametric memory. Your one blended score can't tell you which. You pull up the log for that exact query, and next to it you have your team's gold-set labels for which chunks are actually relevant to it. Given just those two lists — what the retriever returned, and what should have been returned — what's the one comparison that tells you whether this was a retrieval miss or a generation hallucination, and what result would point to each?",
    explanation: [
      "A RAG system is a **pipeline of two stages** — *retrieve* then *generate* — and the cardinal rule of evaluating it is that you must **measure the two stages separately**, because a single end-to-end score cannot localize failure. ==If retrieval feeds the generator garbage, even a flawless generator produces a wrong answer; if retrieval is perfect but the generator ignores it, you also get a wrong answer. Same symptom, opposite cure.== A blended 91% hides which stage failed.",
      "The generation side is best understood as a **triad of three distinct relationships** among three objects: the **question**, the **retrieved context**, and the **answer**. Popularized by the RAG-eval tooling wave (TruLens named it the 'RAG triad'; RAGAS operationalizes the same ideas), the triad is:\n\n- **Faithfulness / groundedness** — *answer ↔ context.* Is every claim in the answer actually supported by the retrieved context? This is your **anti-hallucination** metric. A low score means the answer contains claims the retrieved context never supports — the model drew on its own memory instead of what it was actually given.\n- **Answer relevance** — *answer ↔ question.* Does the answer actually address what was asked, without padding or drift? A faithful answer can still be useless if it answers a different question.\n- **Context relevance / precision** — *context ↔ question.* Of the retrieved chunks, how much is actually pertinent? Low context relevance means retrieval dumped noise, which both wastes tokens and tempts the model to hallucinate off irrelevant text.\n\n==These three are orthogonal: you can be faithful but irrelevant, relevant but unfaithful, or answering well off a context full of noise. One number collapses all three.==",
...
nDCG@5     : gain per relevant chunk = 1/log2(rank+1)
           DCG  = 1/log2(3) + 1/log2(5)     = 0.63 + 0.43 = 1.06
           IDCG = 1/log2(2) + 1/log2(3)     = 1.00 + 0.63 = 1.63   (ideal: both at ranks 1,2)
...
precision 0.40). A reranker would help; more retrieval breadth would not.` },
    ...
    mcqs: [
      {
        question: "Your RAG assistant answers a return-policy question by stating the return window is 45 days. Your gold-set check confirms the correct chunk WAS retrieved — and that exact chunk states the window is 30 days. Which metric is failing, and what does it point to?",
        options: [
        ...
        correct: 3,
        explanation: "Option D is correct: the retrieved chunk is confirmed present and correct, so retrieval did its job. The failure is that the answer contradicts the number that very chunk states — the definition of low faithfulness/groundedness — which localizes the bug to generation: the model drifted from the provided number instead of restating it. Option A is wrong because the gold-set check confirms the correct chunk was retrieved, so recall@k is fine. Option B is wrong because context relevance is confirmed good — the retrieved chunk is exactly the one that answers the question, not noise. Option C is wrong because answer relevance concerns whether the answer addresses the question, not whether its stated number is accurate — the answer here is on-topic but wrong.",
      },
      {
  ...
  "llm-as-judge": {
    ...
    scenario: "Your team is bidding your support agent against a competitor's in a formal bake-off: 200 prompts, answered by both systems, judged pairwise. You use GPT-4 as judge — same family as your own agent — and, ruling out the reorder trap, run every pair both ways, keeping only order-agnostic wins. Your agent wins 61% of those. Encouraged, you rerun the same 200 pairs, same rubric, same order-control, but swap in a different-family judge. Your agent's win rate drops to 47% — a near-tie. Neither set of answers changed between runs. Only the judge did — which means the 61% was never a measurement of your agent at all, just a same-family judge rewarding its own family's phrasing. The 47% near-tie, from a judge with no stake in either system's style, is the number that belongs in the leadership deck.",
    explanation: [
      ...
      "**The rubric is the whole game.** A judge prompt that says 'rate this answer 1–10' invites the model to invent its own inconsistent standard. **G-Eval**-style design fixes this: you give the judge",
      { list: [
        "an explicit **rubric** — the specific criteria that define quality for *this* task (e.g., factual accuracy, completeness, tone, safety)",
        "a **scoring scale with anchored descriptions** of what each level means",
        "often **chain-of-thought** — have the judge reason through the criteria *before* emitting a score, which improves consistency and gives you an auditable rationale",
      ] },
      " A vague rubric produces a vague, drifting metric; a sharp, anchored rubric is what makes the judge repeatable. ==Most 'the judge is noisy' complaints are actually 'the rubric is underspecified' complaints.==",
...
G-EVAL STYLE (anchored, auditable):
  ...
  → CoT-before-score = more consistent + a rationale you can audit.` },
      "Now the **biases** — the systematic tendencies that move a judge's number without moving the underlying quality at all. Judges have systematic, documented tendencies:\n\n- **Position bias** — in pairwise comparison, the judge favors the answer in a particular slot (often the first) regardless of content. This is exactly the 'winner flips when I swap the order' symptom.\n- **Verbosity / length bias** — judges reward longer, more elaborate answers even when the extra text adds no correctness. This is the model-update mystery: longer answers scored 0.6 higher for free.\n- **Self-preference (self-enhancement) bias** — a judge tends to rate outputs from *its own model family* higher, so using GPT-4 to judge GPT-4 outputs against a competitor is not neutral.\n- Others: **sensitivity to superficial style/formatting**, **anchoring** to numbers already present in the prompt (an early score or example pulls the judge's later scores toward it, regardless of merit), and **leniency drift** over a long eval set (scores creep upward as the judge works through many examples in a row, independent of quality).\n\n==These biases are not random noise you can average away — they are *systematic*, so they bias comparisons in a consistent direction and silently reward the wrong behavior (be longer, go first, sound like me).==\n\nPause before the numbers below: given the exact same answer and the exact same rubric, would you expect a judge from the *same* model family as the answer's author to score it higher, lower, or about the same as a judge from a *different* family? Hold that guess — the worked run below measures exactly that comparison, isolated from the position and verbosity effects already shown.",
      { type: "illustration", label: "A worked judge run — the biases made visible, then their mitigations", content: `WORKED RUN — position and verbosity bias on the same two answers below, then self-preference bias on one answer judged twice:
  A: "Restart the router, wait 30 seconds, then reconnect." (8 words,
     fully correct)
  B: restates the same fix, padded with three extra sentences of
     caveats and reassurance (54 words, zero new correctness)

  Pointwise (independent scores, 1-10):
    A → 7.1        B → 7.7   (+0.6 on B for no new correctness — the
                               inflation)

  Pairwise ("which answer is better?"):
    order (A, B) → judge names the FIRST-listed answer → "A wins"
    order (B, A) → judge names the FIRST-listed answer → "B wins"
    same two answers, same content, opposite verdict — the winner is
    whichever slot the judge favors, not whichever answer is better.

  Self-preference (answer A only, same rubric, two different judges):
    same-family judge (shares lineage with A's author)      → 8.4/10
    different-family judge (independent lineage)            → 6.9/10
    one answer, one rubric, zero change in content — the score
    still moves 1.5 points, purely because of who is grading.
...
overall              "score ≠ quality"           CALIBRATE against human labels` },
  ...
  "chunking": {
    ...
    explanation: [
      "**Chunking is the step that decides what a 'retrievable unit' is** — you split source documents into pieces, embed each piece, and retrieve pieces. It sits *upstream* of everything else in RAG, which makes it deceptively high-leverage: a chunk is simultaneously",
      { list: [
        "the thing that gets embedded (so it must be semantically coherent enough for one vector to represent it)",
        "the thing that gets handed to the LLM (so it must carry enough context to be *usable* on its own)",
      ] },
      " ==Those two jobs pull in different directions, and every chunking decision is a negotiation between them.==",
...
Trustworthy safety measurement instead:
  ...
  over-refusal set + human review on the tail  →  a PROFILE, not a %.` },
      ...
      "**Putting it together — how to actually measure safety.** Never report a single number. Report a **profile**:",
      { list: [
        "refusal rate on a private red-team harmful set (want high)",
        "over-refusal rate on a benign-but-scary set like XSTest (want low)",
        "jailbreak attack-success-rate across an evolving technique suite (want low)",
      ] },
      " — all measured on **held-out, leakage-controlled** data with human review on the ambiguous tail, and plotted against **helpfulness** so the tradeoff is explicit. Safety is adversarial and dynamic — attackers adapt, so a one-time pass decays; re-red-team on every model/prompt change. ==The scenario's two failures — over-refusal and a jailbreak — are exactly the two dimensions a single benchmark percentage cannot see, which is the whole lesson.==",
    ],
    keyPoints: [
