...
export const RUNNER_DPO = {
  "dpo": {
    ...
    explanation: [
      "Start from what RLHF Stage 3 is actually solving. The RL objective is: **maximize E[r(x, y)] − β · KL(π_θ(y|x) || π_ref(y|x))**.\n\nIn words: make the policy produce responses the reward model r scores highly, but penalize it for drifting away from the reference (frozen SFT) model by β times the KL divergence. ==The KL term is what keeps the model on-distribution== — without it, the policy chases reward-model artifacts into gibberish that happens to score high.\n\nClassic RLHF solves this with **two moving parts**: a separately-trained reward model, and PPO to optimize against it. Both are expensive and fragile — the reward model is an imperfect proxy that gets **hacked**, and PPO is a sampling-based RL loop that is notoriously sensitive to hyperparameters and can **collapse**.",
      "DPO's insight is a **piece of math, not a new algorithm**. The RLHF objective above has a known closed-form optimum: the optimal policy π* is the reference policy re-weighted by the exponentiated reward — **π*(y|x) ∝ π_ref(y|x) · exp(r(x,y)/β)**.\n\nThis equation can be rearranged to solve for the reward in terms of the policy: r(x,y) = β · log(π*(y|x)/π_ref(y|x)) + β·log Z(x), where Z(x) is a normalizing constant.\n\nThe key move: ==the reward is now expressed IMPLICITLY through the policy's log-probability ratio against the reference.== You no longer need a separate network to represent the reward — the policy itself, compared to its frozen starting point, **IS the reward model**.",
      "That substitution **collapses the whole pipeline**. Human preference data is modeled with the Bradley-Terry model: the probability a human prefers y_chosen over y_rejected is the logistic function of the reward difference, σ(r(x, y_chosen) − r(x, y_rejected)).\n\nSubstitute the implicit reward expression into that. ==The intractable normalizer Z(x) is identical for both responses to the same prompt, so it cancels.==\n\nWhat remains is a loss you can compute directly from **four forward passes** — chosen and rejected under the current policy and under the frozen reference — with **no reward model and no reinforcement learning**.",
