    ...
    mcqs: [
      {
        ...
        options: [
          "The optimal RLHF policy has a closed-form relationship to the reward, so reward rewrites as a scaled log-ratio of the policy against the reference — the policy acts as the reward model",
          "DPO trains on a much larger and far more diverse preference dataset than PPO-based RLHF typically does, so the extra data volume makes a separately-trained reward model statistically unnecessary",
          "DPO freezes the main policy network for the entire training run and instead trains a reward model in its place, discarding that trained reward model once the run finishes",
          "DPO swaps the learned reward model for a rule-based scoring function that human annotators write by hand, then optimizes the policy directly against those fixed handwritten rules",
        ],
        correct: 0,
        explanation: "The core DPO derivation shows the optimal policy under the KL-constrained RLHF objective satisfies π*(y|x) ∝ π_ref(y|x)·exp(r(x,y)/β), which rearranges to r(x,y) = β·log(π_θ(y|x)/π_ref(y|x)) + β·log Z(x). The reward is thus expressed implicitly through the policy's log-ratio against the reference, so no separate reward network is needed. Option 1 is correct. Option 2 is wrong — dataset size is not the mechanism; even with the same preference set, the elimination of the reward model comes from the closed-form reward-policy relationship, not data volume. Option 3 reverses the setup — DPO trains the policy, not a reward model; the reference (which is frozen, not the reward model) is what stays fixed. Option 4 is wrong — DPO does not substitute a hand-written rule-based scorer; the reward is learned implicitly through the policy's own probabilities, and the human preferences enter through the pairwise loss, not a rules engine.",
      },
      {
        ...
        options: [
          "The learning rate handed directly to the model optimizer during each backpropagation step, not a term inside the loss — β replaces tuning that separately.",
          "The number of preference pairs the trainer samples into each mini-batch, which in turn controls how noisy every single gradient estimate becomes",
          "The temperature applied to the final model's output softmax at inference time, controlling how randomly the deployed model samples output tokens",
          "How strongly preferences push the policy vs. how tightly it stays anchored to the reference model — small β stays close, large β lets preferences dominate",
        ],
        correct: 3,
        explanation: "β is the temperature on the implicit KL anchor. Because every score is a ratio against the frozen reference, β scales how hard the preference signal pushes relative to staying near π_ref. Small β (e.g. 0.1) keeps the policy tightly tethered to the SFT model; large β lets preferences dominate and permits larger deviation. Option 4 is correct. Option 1 is wrong — β is not the optimizer learning rate; they are distinct hyperparameters, and β specifically governs the reference-anchoring strength inside the loss. Option 2 is wrong — β does not set batch or pair count; it is a scalar inside the loss, unrelated to sampling. Option 3 is wrong — β is a training-time coefficient on the log-ratio difference, not an inference-time softmax temperature.",
      },
      {
        question: "A DPO-trained model started producing short, repetitive, and sycophantic responses even though its training loss kept decreasing smoothly. Which two statements correctly diagnose what's happening?",
        options: [
          "This is the classic DPO over-optimization signature — loss keeps falling while real quality degrades, since the loss stops tracking quality once it over-fits preference quirks",
          "The likely cause is β set too large or too many training epochs, which sharpens the policy toward whatever the preference set rewards and lets it drift off the anchoring SFT distribution",
          "The Bradley-Terry normalizer Z(x) failed to cancel between the chosen and rejected log-ratio terms, injecting extra unwanted noise into every gradient step of the whole training run and beyond",
          "DPO's loss function structurally cannot represent long, detailed responses, so the model gradually collapsed toward producing much shorter outputs over the entire course of training",
        ],
        correct: [0, 1],
        explanation: "Options 1 and 2 are both correct — this is DPO over-optimization: when β is too large or training runs too long, the policy over-sharpens toward the preference set and drifts off the anchoring SFT distribution (option 2, the cause), which shows up as the classic signature of falling loss alongside degrading real quality (option 1, the tell), because the classification loss stops tracking genuine response quality once the policy over-fits to preference-set idiosyncrasies. Option 3 is wrong — Z(x) provably cancels because it is identical for both responses to the same prompt; that cancellation is exact, not a source of noise. Option 4 is wrong — DPO has no inherent inability to represent longer responses; the collapse toward short outputs is a symptom of over-optimization, not a representational limit.",
      },
      {
        question: "When would you still prefer online PPO-based RLHF over offline DPO, despite DPO's simplicity and stability?",
        options: [
          "When you want to avoid tuning any hyperparameters at all in the pipeline, since PPO's built-in reward model removes any need to ever tune β",
          "When you need fresh on-policy exploration — PPO keeps sampling from the improving policy, correcting regions a fixed offline set never covered",
          "When your preference data is small and low quality, since DPO is guaranteed to outperform PPO whenever the training dataset itself is weak",
          "When you only have single thumbs-up/thumbs-down labels instead of paired comparisons, since PPO trains directly on unpaired labels while DPO cannot",
        ],
        correct: 1,
        explanation: "DPO is strictly off-policy: it learns from a fixed preference set with no fresh sampling, so it cannot explore or correct in regions the static data never covered. Online PPO keeps generating and scoring new samples from the improving policy, letting it self-correct — which is why well-tuned online RLHF can still win at the frontier. Option 2 is correct. Option 1 is wrong — PPO is notoriously MORE hyperparameter-sensitive than DPO, not less; you would not pick it to avoid tuning. Option 3 is wrong — with small, low-quality data, DPO's lack of online correction makes it MORE fragile, not guaranteed to win; the claim that DPO always outperforms PPO there is false. Option 4 describes the use case for KTO (unpaired binary feedback), not a reason to choose PPO — PPO also typically consumes a reward model trained on comparisons, not raw single labels.",
      },
    ],
