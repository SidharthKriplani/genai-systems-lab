...
export const RUNNER_GAP_B = {
  "model-routing-cascades": {
    ...
    groundUp: "Let's start with the simplest possible design, the one every LLM product actually begins with: pick the best model you can afford and send *everything* to it. It's the obvious choice — one model, one code path, and the quality is as high as it gets. For a demo, it's exactly right.\n\nHere's the small observation that unravels it. Look at your real traffic for a minute and you'll notice the questions aren't all the same *shape*. 'Reset my password' and 'thanks!' sit right next to 'walk me through this subtle multi-step bug.' They're wildly different in difficulty — and yet, in the send-everything design, they all pay the *same* price: the frontier model's price, the frontier model's latency. You're buying a Ferrari to fetch the mail.\n\nNo need to rush. We'll build up from that one observation — traffic is a *mix* of difficulties — and let two different fixes fall out of it: deciding which model to use *before* you see an answer, versus *after*. Then we'll handle a separate problem the whole scheme ignores (what happens when your provider simply goes down), and by the end you'll be able to look at a bloated LLM bill and say exactly which lever to pull.",
    scenario: "Now let's put all of that to work on a real one. Your LLM product started on the frontier flagship because it was simplest and it worked — then the bill arrived. You're paying flagship prices for *every* query, including 'reset my password,' 'what's your refund window,' and 'thanks!'. p95 latency is bad because even trivial queries wait on a giant model, and last Tuesday your single provider had a 40-minute outage and the product was simply *down*. Finance wants cost halved, on-call wants the outage risk gone, product refuses to let quality drop on the hard questions. Take a moment before reading on: which of these are the *same* problem, and which is a different axis entirely? Here's the reasoning, step by step. Cost and latency are one problem with one root — you're sending a whole *distribution* of difficulty to the worst-case model, so a router or cascade that matches each query to the cheapest-capable model fixes both at once (recall the ~2-3x savings when 70% of traffic is easy). The outage is a *different axis*: routing only ever chooses among *healthy* models, so it does nothing for availability — that needs provider failover, a fallback chain to another provider or a degraded model. Notice how cleanly the three demands separate: two of them are routing, the third is failover, and a mature stack layers both.",
    explanation: [
      "Start from a single observation about traffic itself: **real queries are a distribution of difficulty, not a uniform stream.** A large fraction is trivial (greetings, FAQ lookups, simple rewrites), a middle band is moderate (summarise this, classify that, short reasoning), and a thin tail is genuinely hard (multi-step reasoning, subtle code, adversarial edge cases). Now put that fact next to the send-everything-to-one-model design and the waste becomes visible: routing the whole distribution to one big model means you pay the *worst-case* cost and latency on the *average-case* query — a price the average query never needed. ==So the entire discipline of routing follows directly from the distribution: match each query to the cheapest model that can still answer it correctly, and spend your quality budget only where it actually moves the needle.==",
...
CASCADE (decide AFTER seeing an attempt):
  ...
  • but escalated queries pay BOTH models + the check's latency` },
      "A cascade lives or dies on the **confidence signal** — the check that decides 'is this cheap answer good enough, or escalate?'. Interviewers want you to name concrete signals and their tradeoffs.",
      { list: [
        "**Token logprobs / sequence probability**: if the small model's own output probability is low, it's unsure — cheap to read, but a poorly-calibrated model can be *confidently wrong* (high logprob, wrong answer), so this misses the dangerous cases.",
        "**Self-consistency**: sample the small model a few times; if the samples *disagree*, escalate. Robust, but multiplies the cheap model's cost.",
        "**A verifier / judge**: a separate (often small) model or rule-set that grades the answer — e.g. 'does this cite a source? does it actually answer the question?'. Most flexible and most aligned with 'good enough,' but adds a call and can itself be wrong.",
      ] },
      " ==The confidence signal is where cascades succeed or fail: a good signal escalates exactly the queries the small model got wrong; a bad one either escalates everything (no savings) or nothing (quality craters).==",
...
Take: both ~cut cost 2-3x. Router = lowest latency, riskier misroute.
    ...
    mcqs: [
      {
        ...
        options: [
          "They are the same thing under two names; cascade is simply the marketing term vendors use instead of router in their pricing pages",
          "A router is always more accurate than any cascade, because it uses a dedicated classifier, whereas a cascade guesses randomly between models",
          "A cascade sends every query to every model in parallel and picks the best answer, which makes it the fastest option of the two by far",
          "A router decides before an answer exists, committing up front with no recovery; a cascade decides after an attempt, escalating on failure",
        ],
        correct: 3,
        explanation: "Option 4 is correct: the defining distinction is *when* the decision is made. A router classifies the query and commits it to a single model before any answer is produced — one inference, lowest latency, but if it misjudges there's no second look. A cascade runs the cheap model first and uses a confidence check on that actual answer to decide whether to escalate — it has strictly more information (an attempt to inspect), so it routes more accurately, but an escalated query pays for the small model *and* the large model in series, adding tail latency. Option 1 is wrong — they're distinct patterns. Option 2 is backwards — the cascade's after-the-fact decision is typically *more* accurate, and it never 'guesses randomly.' Option 3 describes parallel ensembling/best-of-N, not a cascade, which is serial and escalates only on failure.",
      },
      {
        ...
        options: [
          "Logprobs are impossible to obtain from almost every hosted model API, so a logprob-based cascade can basically never be built at all",
          "Models are often poorly calibrated and confidently wrong — a high-logprob wrong answer passes the check and never escalates, silently",
          "Reading logprobs makes the small model run noticeably slower than the large model itself would, which defeats the whole purpose of trying it first",
          "Logprobs guarantee perfect escalation on every single query type, so there is effectively zero risk in using them as the sole confidence signal",
        ],
        correct: 1,
        explanation: "Option 2 is correct: a logprob threshold assumes the model 'knows when it doesn't know,' but LLMs are frequently overconfident and can assign high probability to wrong outputs. Those confident-but-wrong answers clear the confidence check and are returned without escalation — the cascade fails silently on the dangerous cases. That's why more robust signals (self-consistency across samples, or a separate verifier/judge) or a combination are often preferred, at extra cost. Option 1 is wrong — logprobs are available from many models. Option 3 is wrong — reading logprobs doesn't make the small model slower than the large one. Option 4 is the exact overconfidence trap the question warns against.",
      },
      {
        question: "After adding a router that cuts cost, your product still went fully down during a 40-minute outage at your model provider. Which two statements correctly explain why routing didn't help, and what actually addresses this?",
        options: [
          "Routing matches queries to the cheapest-capable model only among healthy options — a cost/latency lever, not availability",
          "Uptime is addressed by provider failover: on a primary error or rate-limit, fall back to a different provider or a degraded model",
          "Routing should have prevented the outage entirely on its own; the router must simply have been badly misconfigured by whoever built it",
          "The only real fix here is to buy a second, fully identical copy of the exact same provider flagship model to use as a standby backup",
        ],
        correct: [0, 1],
        explanation: "Options 1 and 2 are both correct: routing and failover are orthogonal. Routing decides which model to use among healthy options to save cost and latency; it makes no promise about what happens when the chosen provider is unavailable (option 1). Availability is handled by a failover/fallback chain — on a primary error, timeout, or rate-limit (after a short retry), automatically route to a different provider's comparable model or a degraded-but-available smaller model, degrading gracefully rather than erroring out (option 2). Option 3 misunderstands routing's purpose — it was never meant to guarantee uptime. Option 4 doesn't remove the single-provider dependency; a second copy of the same provider's model goes down in the same outage.",
      },
    ],
  ...
  "llm-security-beyond-injection": {
    depthTier: "deep",
    interviewWeight: "high",
    groundUp: "Let's start by noticing what prompt injection actually *is*, because it shapes what people wrongly think 'securing an LLM' means. Injection is about bad instructions getting *in* — an attacker sneaking a command into the model's input. Defend against that and it's tempting to feel done. Most teams stop right there.\n\nBut here's the reframe that opens the whole module. Step back and picture an LLM app not as a brain to be tricked, but as a *pipe*: data flows *in* (the user's request, retrieved documents), it pools in the middle (the context — which may hold secrets, other users' data, your own system prompt), and it flows *out* (to the user, and, for an agent, to tools that can *act*). Injection guards only one end of that pipe — the entrance. Every other point where data can *escape*, or a privilege can be *abused*, is still wide open.\n\nNo need to rush. We'll build the picture from that pipe: name each boundary, see the specific way data leaks or power is misused there, and derive the control that plugs it — PII redaction, exfiltration defenses, guardrails that gate *tool calls* not just replies, least-privilege scoping, and the compliance rules that make all of it non-optional. By the end 'I stopped prompt injection' will read as covering one door of a building full of windows.",
    scenario: "Now let's put all of that to work on a real one. You've hardened your LLM app against prompt injection, so you feel covered — then three incidents land in one week. First, a support-bot transcript shows the model cheerfully repeating a customer's full credit-card number and home address back in its answer, with no attacker at all. Second, an internal agent with a database tool takes a cleverly-worded request and returns a chunk of *another tenant's* records, plus, on inspection, part of your system prompt. Third, a security review finds your 'read a file' agent was provisioned with a cloud credential that can *also* delete production buckets. Take a moment before reading on: what do these three have in common, given injection was already handled? Here's the reasoning, step by step. Not one is an injection — nothing bad got *in*. Instead, each is something escaping *out* at a different boundary of the pipe: PII leaking to the user (an output-redaction gap), other-tenant data and the system prompt exfiltrating through context, and an over-scoped credential turning any future slip into a catastrophic blast radius (a least-privilege gap). Notice how cleanly the fixes fall out once you see the boundaries: two-sided PII redaction, output guardrails that also gate tool calls, and credentials scoped to the narrowest capability the agent actually needs.",
    explanation: [
      "Start from a structural definition of what an LLM app *is*, ignoring attackers for a moment: it's a pipeline that takes *input* (which may carry sensitive data), holds *context* (which may hold secrets, other users' data, and your own system prompt), and produces *output* that flows to a user — and, for an agent, to *tools and systems* that can act. Once you see it as data moving across those boundaries, a general principle follows: ==every boundary where data crosses is a place data can leak or a privilege can be abused.== Prompt injection is one specific abuse at exactly one boundary — bad data getting *in* — and it has its own module, so set it aside. Everything else in this module is the mirror concern: sensitive data and privilege escaping *out* across the other boundaries, and the controls that sit on each one.",
      "Start with the most concrete: **PII detection and redaction**, on *both* the input and the output. On the **input** side, a user may paste a credit-card number, an SSN, a medical detail — and if you log that prompt, embed it into a vector DB, or send it to a third-party model API, you've now *spread* that PII into systems that were never scoped to hold it (a compliance problem the moment it's written down). On the **output** side, the model may *emit* PII — regurgitating something from its context, or reconstructing it — straight to a user who shouldn't see it (e.g. regurgitating a card number pasted earlier in the conversation). The control is a **redaction pass** (regex for structured PII like cards/SSNs/emails, plus an NER model or a classifier for names/addresses) that detects and masks PII *before* it's logged/stored (input) and *before* it reaches the user (output). ==PII is a two-sided problem: you filter it going *in* so you don't spread it, and going *out* so you don't leak it.==",
      "The second category is **data exfiltration** — sensitive information *escaping* the boundary it was supposed to stay inside. Three distinct flavours interviewers probe.",
      { list: [
        "**System-prompt leakage**: your system prompt often contains proprietary instructions, business logic, or even embedded keys; a model can be coaxed into reciting it, handing competitors (or attackers) your playbook.",
        "**Cross-tenant / training-data leakage**: in a multi-tenant app, another user's data may be in the model's context (via a shared cache, a mis-scoped retrieval, or a tool), and the model surfaces it to the wrong user — or, for a fine-tuned model, it *memorised* training records and reproduces them verbatim.",
        "**Tool-mediated exfiltration**: an agent with tools can *actively* leak — e.g. it's induced to put secret data into an outbound request (a URL it fetches, an email it sends), carrying data *out* of your perimeter through a legitimate-looking action.",
      ] },
      " ==Exfiltration is the mirror of injection: injection is bad data getting *in* to influence the model; exfiltration is sensitive data getting *out* through the model or its tools.==",
...
LEAST PRIVILEGE (scope to the exact need):
  ...
  Principle: you can't guarantee the model is safe → make the TOOLS safe.` },
    ...
    mcqs: [
      {
        question: "Your support bot repeated a customer's full credit-card number back in its answer, and separately your prompt logs now contain SSNs users pasted in. Which two statements correctly explain why PII is a two-sided problem?",
        options: [
          "On input, PII a user pastes spreads into logs, vector stores, third-party APIs the moment you store it — redact before storing",
          "On output, the model can emit PII to a recipient who should not see it, like regurgitating a card number, so replies need a redaction pass",
          "PII only ever matters on the output side of a request; input PII is harmless since the user already knows their own personal data anyway",
          "PII detection becomes entirely unnecessary once prompt-injection defenses are in place, since those defenses already fully cover PII handling",
        ],
        correct: [0, 1],
        explanation: "Options 1 and 2 are both correct: PII must be handled on both boundaries. On input, once a user's PII is logged, embedded into a vector DB, or forwarded to a third-party model API, it has spread into systems never scoped to hold it — a compliance problem independent of any attacker — so it should be detected and masked before storage/forwarding (option 1). On output, the model can regurgitate or reconstruct PII and hand it to someone who shouldn't see it (the credit-card transcript), so an output redaction pass must run before the response reaches the user (option 2). Structured PII (cards, SSNs, emails) is caught with regex; unstructured PII (names, addresses) needs an NER model or classifier. Option 3 ignores the input-spread/compliance risk. Option 4 conflates injection defenses with PII handling; they're different controls.",
      },
      {
        ...
        options: [
          "Output guardrails are entirely useless in an agent setting like this one; only input-side guardrails actually matter for security here",
          "The guardrail should have been placed only on the input side of the pipeline instead of on the output side of the model",
          "The agent simply needed a larger, far more capable model, which by itself would never have exfiltrated the sensitive data",
          "The guardrail must also sit between the model and its tools, not only the user — a tool call carrying data out needs inspection",
        ],
        correct: 3,
        explanation: "Option 4 is correct: for an agent, the highest-risk output is not the text shown to a human but the *action* it takes — a tool call that can carry data out of your perimeter (an outbound request, an email, a DB write). A guardrail placed only between the model and the user never inspects those tool calls, so tool-mediated exfiltration sails past it. The output guardrail must gate the tool invocation itself, inspecting and blocking calls that would leak secrets or PII before they execute. Option 1 wrongly dismisses output guardrails. Option 2 ignores that the leak is on the action/output path. Option 3 is wrong — model size doesn't address a missing control on the tool boundary.",
      },
      {
        ...
        options: [
          "Leave the credential broad but add a much longer system-prompt warning telling the model to never delete anything under any circumstance",
          "Scope the credential to the narrowest capability needed (read-only, single bucket); require approval for irreversible actions",
          "Give the agent full admin credentials but log every single action it takes carefully, so any mistakes can always be undone later on",
          "Replace the agent entirely with a bigger, smarter foundation model that would presumably know not to ever misuse the credential",
        ],
        correct: 1,
        explanation: "Option 2 is correct: least privilege scopes each agent to only the tools it needs and each tool to the narrowest credential that does its job — here, read-only and restricted to the intended bucket — plus human approval gates on irreversible actions. This matters because model-level safety can never be guaranteed (injection, confused-deputy tricks, and plain model mistakes all exist), so the durable defense is to shrink the blast radius structurally: a compromised read-only, single-bucket agent can, at worst, read the one bucket it was meant to read. Option 1 relies on the model obeying instructions, which is exactly what can't be guaranteed. Option 3 keeps a catastrophic blast radius and 'undo later' fails for irreversible deletes. Option 4 wrongly assumes a smarter model removes the need to scope privileges.",
      },
    ],
