...
export const RUNNER_MARKET_GAP = {
  "rope": {
    ...
    groundUp: "Let's start with something a little unsettling about attention: on its own, it can't tell word order. The math that lets every token look at every other token treats the input as an unordered *bag* — 'dog bites man' and 'man bites dog' look identical to it. Since order is obviously the whole point of language, the position information has to be *added in* somehow. This module is about how, and why the how matters enormously the moment you want longer context.\n\nHere's the puzzle we'll chase. A model trained to handle a few thousand tokens can be excellent, then fall apart the instant you feed it far longer inputs — going fluent but weirdly ungrounded, as if it's lost the thread. The tempting fix is to bump a config number and call it a day. For some position schemes that fails completely; for the one modern models use, that number isn't even the right lever.\n\nWe'll build it up from zero: why attention needs a position signal at all, why the naive 'add a position vector' idea can't stretch, then the rotation trick (RoPE) that ties the score to the *gap* between tokens instead of their absolute spots — and finally the knob that actually buys you the longer context.",
    ...
    explanation: [
      "Start from a property of attention that forces everything else: it is **permutation-invariant**. The raw dot product `q·k` between a query and a key carries no notion of *where* either token sits, so 'dog bites man' and 'man bites dog' produce the same set of Q/K vectors — without injecting position, the model cannot tell order at all. That constraint means every Transformer *must* add some **position signal** back in.\n\nThe original solution was the most direct one imaginable: an **additive position embedding** — a vector `p_i` (fixed sinusoids in the 2017 paper, or a learned lookup table like GPT-2/BERT) *added* to the token embedding before attention. This works, but notice what it commits to: it encodes **absolute** position, the token's index counted from the start. And that choice is the seed of the length-extension problem we're chasing.",
    ...
    mcqs: [
      ...
      },
      {
        ...
        options: [
          ...
          "After rotating q at m and k at n by their angles, their dot product reduces to a function of the offset (m − n) alone, not individually",
        ],
        correct: 3,
        explanation: "The fourth option is correct: RoPE rotates q_m by R(m) and k_n by R(n), and because rotations compose, ⟨R(m)q, R(n)k⟩ = ⟨q, R(n−m)k⟩ — the score depends only on the relative offset (m − n). That is why a pair 5 tokens apart looks the same at positions (0,5) or (1000,1005), which is the source of RoPE's graceful generalization. The first option describes additive absolute PE, which is exactly what RoPE replaces. The second option describes ALiBi's linear distance penalty, a different relative scheme that bakes in recency bias. The third option is wrong: unit-normalizing Q/K does not encode position at all and would not produce relative dependence.",
      },
      {
