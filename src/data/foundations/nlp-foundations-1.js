    ...
    mcqs: [
      {
        ...
        options: [
          "Subword tokenization keeps a fixed vocabulary and splits rare words into known sub-pieces — 'unhappiness' becomes un+happi+ness — bounding vocabulary size, killing OOV, and exposing morphology via shared pieces",
          "Subword tokenization simply enlarges the vocabulary until every common inflection has its own dedicated entry, so 'happy', 'happiness', and 'unhappiness' each remain whole tokens instead of ever being split apart",
          "Subword tokenization first runs a lemmatizer that maps each form to its dictionary root using a POS tagger and a 50k-entry lookup table, so 'happy', 'happiness', 'unhappiness' collapse onto one shared token",
          "Subword tokenization actually operates at the character level, breaking every word into individual letters one Unicode codepoint at a time, which is why OOV becomes structurally impossible as a side effect",
        ],
        correct: 0,
        explanation: "Option A is correct: subword tokenization keeps a fixed-size vocabulary of the most useful pieces — frequent whole words stay whole, rare or complex ones split into known sub-pieces. That simultaneously bounds the vocabulary, eliminates OOV (an unseen word is just decomposed into pieces you already have), and makes morphology visible because related words like 'happy'/'happiness'/'unhappiness' share the 'happi' piece. Option B is wrong and backwards — subword *shrinks* and *fixes* the vocabulary rather than enlarging it to cover every inflection, and that wouldn't solve OOV for truly unseen words anyway. Option C conflates subword with lemmatization; subword doesn't require a dictionary or POS lookup and works by splitting into pieces, not collapsing forms into one. Option D is wrong — subword is coarser than character-level; it's a middle ground, and while it also avoids OOV, that's not because it operates letter-by-letter.",
      },
      {
        question: "Which two statements accurately describe SentencePiece's relationship to BPE and Unigram? (Select two.)",
        options: [
          "SentencePiece and BPE are two interchangeable names for the identical merge procedure, so choosing BPE over SentencePiece has no practical effect on vocabulary size or the resulting token boundaries",
          "SentencePiece is a framework, not one algorithm — it can run BPE (frequency merges) or Unigram (likelihood pruning) underneath to decide which pieces it keeps",
          "SentencePiece only works on whitespace-segmented languages like English, so languages such as Chinese, Japanese, or Thai still require a separate rule-based word-splitting step applied beforehand",
          "SentencePiece encodes whitespace as a visible meta-symbol inside the token stream, making decoding fully reversible and removing dependence on language-specific word boundaries",
        ],
        correct: [1, 3],
        explanation: "Options B and D are correct together: SentencePiece's defining move is treating raw text as a Unicode stream and encoding whitespace as a token (▁), which makes it reversible and language-agnostic (works on Chinese/Japanese without a separate splitter) — and it is a framework that can run either BPE or Unigram as its underlying merge/segmentation algorithm, so BPE is a choice within it, not a synonym for it. Option A is wrong: SentencePiece and BPE are not interchangeable names — BPE is one algorithm SentencePiece can use, and Unigram is another, so the choice does matter. Option C is backwards: SentencePiece removes the need for a separate language-specific word-splitting step precisely because it treats whitespace as part of the token stream, so it's actually well-suited to languages without spaces.",
      },
      {
        ...
        options: [
          "No — once a model is large enough it has fully internalized every aspect of language, so no preprocessing step, including tokenization itself, has any measurable effect on cost or output quality",
          "Yes, partially — casing still needs to be forced to lowercase before an LLM sees text via a fixed case-folding table, since otherwise the model cannot tell 'Apple' the company from 'apple' the fruit",
          "Classical cleaning mostly retired — the model learns morphology itself, and cleaning discards signal. Tokenization matters more than ever: it drives cost and context usage",
          "Yes — stemming and stopword removal are still required before feeding text to an LLM, because without them the input sequence would overflow the context window on almost any realistic prompt",
        ],
        correct: 2,
        explanation: "Option C is correct: it draws the right line. Classical cleaning (stemming, lemmatization, stopword removal, forced lowercasing) mostly retired for LLMs because a large model learns morphology and which function words matter, and aggressive cleaning throws away signal — but tokenization did not become less important, it became more central: it sets cost and context usage (you pay per token), causes number-splitting inconsistencies that drive arithmetic errors, and inflates token counts for non-English text (2-3x), which is both a cost and fairness concern. Option A overstates the retirement and wrongly claims tokenization has no effect on cost or quality — it has a large one. Option B is wrong: LLMs don't require forced lowercasing, since casing carries meaning ('Apple' vs 'apple'), and the model learns to handle case itself. Option D is wrong: you generally should NOT stem or strip stopwords before an LLM, and the reason isn't context-window overflow — it's that doing so removes signal the model would otherwise use.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "TF-IDF weights terms only by their raw frequency across the corpus, so 'the', being by far the most frequent word overall, should actually end up with the highest weight of all, not the lowest",
          "Cosine similarity runs as a preprocessing filter before TF-IDF is even computed, and it discards any word appearing in more than half of all documents in the corpus, which is why 'the' contributes nothing",
          "'password' simply outweighs 'the' because it is the longer string — TF-IDF assigns extra weight for every character beyond a fixed length threshold, independent of how often either word appears",
          "The IDF factor log(N/df) explains it: 'the' appears in all 1000 docs so df=N, idf=log(1)=0, zeroing its weight regardless of frequency; 'password' appears in 30, giving idf≈3.5 and a high weight",
        ],
        correct: 3,
        explanation: "Option D is correct: the IDF factor log(N/df) is what separates them. 'the' appears in every document, so df equals N and idf = log(N/N) = log(1) = 0 — multiplying by zero annihilates its weight no matter how high its term frequency is. 'password' appears in only 30 of 1000 documents, so idf = log(1000/30) ≈ 3.5, and its weight is high even with modest frequency. IDF operationalizes 'a term is informative in proportion to its rarity across the corpus.' Option A is backwards — TF-IDF is TF *times* IDF, and the IDF term specifically suppresses ubiquitous words rather than rewarding them. Option B is wrong — cosine similarity is a comparison step applied after vectors are built; it doesn't pre-filter words, and IDF, not cosine, is what discounts common terms. Option C is wrong — weighting is driven by document frequency, not string length.",
      },
      {
        question: "Which two statements correctly explain why you L2-normalize TF-IDF vectors and compare them with cosine similarity rather than raw dot products? (Select two.)",
        options: [
          "L2-normalizing each vector to unit length cancels out document length — a longer document would otherwise accumulate larger raw TF-IDF values across more terms and win purely by being long",
          "Cosine similarity adds a layer of learned semantic understanding on top of TF-IDF via a pretrained embedding matrix, letting it recognize 'reset' and 'recover' mean similar things without shared terms",
          "Cosine similarity, the dot product of unit vectors, measures the angle between them — which terms dominate each document's direction — independent of magnitude, so ranking stays length-invariant",
          "L2 normalization converts the sparse TF-IDF vectors into dense, 300-dimensional low-rank embeddings via SVD, which is what actually makes the cosine comparison meaningful in practice",
        ],
        correct: [0, 2],
        explanation: "Options A and C are correct together: a longer document accumulates larger raw TF-IDF values across more terms, so uncorrected dot products would favor length over relevance; L2-normalizing to unit length removes that magnitude effect, and cosine similarity — which for unit vectors equals their dot product and the cosine of the angle between them — then compares direction (which terms characterize the document) independent of length. That combination is exactly why a short, on-topic article can outrank a long filler-heavy one. Option B is wrong: cosine similarity over TF-IDF vectors adds no semantics — it is still purely lexical, so 'reset' and 'recover' would still score near zero. Option D is wrong: normalization rescales the existing sparse vector, it does not compress it into a dense, low-dimensional embedding.",
      },
      {
        ...
        options: [
          "TF-IDF is too computationally slow to run at production scale, so BM25 is essentially just a faster reimplementation of the identical scoring formula, changing runtime but not ranking behavior at all",
          "TF-IDF matches only literal terms, missing synonyms (vocabulary mismatch); BM25 keeps IDF but adds TF saturation and length normalization; hybrid fuses it with a dense retriever",
          "TF-IDF actually captures word meaning quite well via its log(N/df) term, but ignores term frequency entirely, and BM25's main contribution is simply reintroducing raw frequency counts into the score",
          "Hybrid retrieval replaces TF-IDF and BM25 outright, since dense embedding retrievers are now considered strictly better at every retrieval scenario, including queries built from rare or exact terms",
        ],
        correct: 1,
        explanation: "Option B is correct: TF-IDF's core limits are that it matches only literal shared terms — so a query and a synonymous document have near-zero similarity (the vocabulary-mismatch problem) — and that it inhabits a very high-dimensional sparse space. BM25 retains the IDF insight but fixes two TF-IDF shortcomings: it saturates term frequency (the 20th occurrence adds far less than the 2nd) and normalizes for document length inside the score. Hybrid retrieval then pairs this sparse lexical retriever (strong on exact/rare terms) with a dense embedding retriever (strong on meaning), fusing them because their failure modes are opposite. Option A is wrong — the shift is about ranking quality (saturation, length norm), not raw computational speed, and BM25 is not merely a faster version of the same formula. Option C has it backwards — TF-IDF captures frequency, not meaning, and ignoring frequency was never its problem. Option D overstates dense retrieval's advantage — it is not strictly better; it blurs exact and rare terms, which is exactly why the sparse leg is still kept in hybrid systems.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "The 5-gram model is overfitting to its training data here, and the straightforward fix is simply to raise the learning rate and train for more epochs with dropout until it converges on unseen inputs",
          "Possible n-grams grow as V^n — about 10^23 for 5-grams — while a corpus holds only ~10^9 words, so nearly every needed 5-gram is unseen, MLE assigns it probability zero, and one zero kills the sentence",
          "5-gram models simply demand far more GPU memory than bigrams do during training, roughly 8x more per the standard sizing rule, and what looks like a collapse is really an out-of-memory failure at inference",
          "The Markov assumption itself stops holding once n reaches 5 words of context, which is specifically why the resulting model becomes statistically invalid and effectively unusable at that particular context length",
        ],
        correct: 1,
        explanation: "Option B is correct: the count of possible n-grams is V^n, exponential in n, so at n=5 with a 50k vocabulary there are on the order of 10^23 possible 5-grams while a large corpus contains only ~10^9 words — it can hold at most ~10^9 distinct n-grams, a vanishing fraction. The overwhelming majority of 5-grams users type were never seen, MLE gives any unseen n-gram count 0 hence probability 0, and because sentence probability is a product of next-word probabilities, one zero makes the entire sentence probability zero. This is the data-sparsity explosion, inherent to counting-based models, not a tuning issue. Option A is wrong — n-gram MLE has no learning rate or epochs; it's pure counting, not gradient-based training. Option C misattributes the collapse to hardware; the failure is statistical (zeros from unseen counts), not GPU memory. Option D is wrong — the Markov assumption is a modeling choice valid at any n; it doesn't 'stop holding' at n=5, sparsity is the actual issue.",
      },
      {
        ...
        options: [
          "Perplexity is 2 raised to the cross-entropy — the average branching factor, how many equally-likely words it's choosing among per step; lower is better, so A at 30 beats B at 180",
          "Perplexity climbs as a model gets better at prediction, so model B's higher score of 180 actually means it is considering a much richer, more nuanced set of 180 plausible continuations than model A",
          "Perplexity is primarily a proxy for the model's total parameter count, so a score of 30 versus 180 mainly tells you model A has roughly six times fewer trainable parameters than model B does",
          "Perplexity is computed only over the exact training set a model was fit on, so a gap between 30 and 180 mostly reflects differing training-set sizes rather than any real generalization ability",
        ],
        correct: 0,
        explanation: "Option A is correct: perplexity is 2 raised to the per-word cross-entropy (average bits to encode each word of the test text) and is interpreted as the model's average branching factor — the effective number of equally-likely next words it's choosing among at each position. A model that puts high probability on the words that actually occur has low cross-entropy and thus low perplexity, so lower is better; model A at 30 is markedly better than B at 180 (as confused as guessing uniformly among 180 words). Option B inverts the direction — higher perplexity signals more confusion, not richer, better predictions. Option C is wrong — perplexity reflects predictive quality on held-out text, not parameter count. Option D is wrong — perplexity here is computed on held-out test text, which is what makes it an intrinsic measure of generalization, not a training-set artifact.",
      },
      {
        question: "Smoothing fixes the zero-probability problem in n-gram models, yet the field still moved to neural language models. Which two statements correctly identify what smoothing leaves unfixed? (Select two.)",
        options: [
          "Smoothing techniques like Kneser-Ney are computationally expensive enough that n-gram models became slower than the neural models that replaced them",
          "Smoothed n-gram models still cannot be evaluated with perplexity, so there was no reliable way to compare them against neural language models",
          "A fixed window of n-1 words can never capture long-range dependencies, such as subject-verb agreement spanning more words than the window covers",
          "Words remain opaque, unrelated integer symbols to an n-gram model, so it cannot generalize what it learned about one word to a similar word",
        ],
        correct: [2, 3],
        explanation: "Options C and D are correct together: these are n-grams' two structural ceilings that smoothing never touches. A fixed window of n-1 words cannot capture dependencies that span more tokens than the window — agreement or reference beyond that range is simply invisible to the model. And because n-grams treat words as opaque, unrelated integer symbols with no shared representation, the model cannot transfer what it learned about one word to a similar word ('cat' and 'dog' are entirely separate events). Distributed word embeddings gave words shared representations, and transformers added attention over long, variable-length context — fixing precisely these two gaps. Option A is wrong: the shift to neural models was about modeling power, not runtime speed, and Kneser-Ney is not what made n-grams slow. Option B is wrong: perplexity applies equally well to n-gram and neural language models — it's the standard intrinsic yardstick for both.",
      },
    ],
