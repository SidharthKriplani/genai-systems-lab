...
export const RUNNER_PRODUCTION_TONE = {
  "cost-latency-concepts": {
    ...
    explanation: [
      ...
      "Let me show you why, because the arithmetic is more persuasive than any slogan. Take a customer-support bot the old way: 500 input tokens at $2.50/1M is $0.00125, plus 120 output tokens at $10/1M is $0.0012. Total $0.00245. Now run the same request after someone tripled the input — say a longer system prompt got bolted on — to 1,500 tokens: 1,500 input tokens at $2.50/1M is $0.00375, plus the *unchanged* 120 output tokens at $0.0012. Total $0.00495.\n\nNotice what happened. The expensive-per-token output line never moved — it's still $0.0012 on both invoices. The cheap-per-token input line tripled, from $0.00125 to $0.00375, and *that* is what dragged the per-request cost up almost exactly 2× (0.00495/0.00245 ≈ 2.02×). ==The cheap tokens drove the whole thing, precisely because there were so many more of them.==",
      { type: "illustration", label: "Cost per request — old vs. new", content: `Cost per request breakdown:\n                    OLD (500 input)       NEW (1,500 input)\n  Input cost:       $0.00125              $0.00375   (+3×)\n  Output cost:      $0.0012               $0.0012    (unchanged)\n  ──────────────────────────────────────────────────\n  Total:            $0.00245              $0.00495   (+2.02× per request)\n\nInput is only $2.50/1M vs $10/1M for output — yet input owns the swing,\nbecause a long-prompt app carries 10–50× more input tokens than output tokens.\nPer-token price is not per-request share.` },
      "Now hold cost in one hand and pick up latency in the other, because they do *not* have the same shape and interviewers love catching people who assume they do. The equation is **total latency = TTFT + (output_tokens × TPOT)**. **TTFT** is Time To First Token — the wait before anything appears — and it scales with how long your input is and how backed-up the queue is. **TPOT** is Time Per Output Token — the steady drip of generation — and at a fixed model size it's roughly constant per token. (The module's Hands-On tool calls this same idea **TBT**, Time Between Tokens, and calls total latency **E2E** — different labels, same three quantities.)\n\nSo trace the same 3× input-growth request through it. The output didn't change, so the `output_tokens × TPOT` term didn't change — generation takes the same wall-clock time it always did. But TTFT climbed roughly in proportion to the input, call it ~3× higher than before, on the same hardware. ==Input growth is quiet on the generation clock and loud on the first-token clock.==",
    ...
    keyPoints: [
      ...
      "**In long-prompt, short-output apps, input dominates total cost** — there are simply 10–50× more input tokens. A modest-looking multiple on the input side can drive a comparable multiple on total per-request cost while the pricier output line never moves at all.",
  ...
  "observability-concepts": {
    ...
    scenario: "Your RAG pipeline for B2B analytics has been humming along for three months. Then customer success forwards a note: answers 'seem worse' over the last couple of weeks. You check the obvious things first. No code deployed. Error rate is flat at 0.1%. Request volume is steady. Every dashboard you own is green. And yet the customers are right — something has quietly gotten worse, and none of your monitoring caught it. You already worked out the fix for this above — now apply it to this exact incident. Walk through the four suspects and say which one you'd chase first here, name the single signal in the minimum-viable stack that would have flagged this within hours instead of two weeks, and be ready to explain to the team why the metric everyone trusts — error rate — never had a chance of catching it.",
    explanation: [
      ...
      "Here are the signals that actually earn their place. **Retrieval-quality indicators:** the distribution of retrieval scores, and the share of queries whose top-k chunk even contains a keyword from the question — when retrieval starts missing, this moves first. **Answer-quality proxies:** answer-length distribution, response-format compliance, citation completeness — cheap structural tells that something shifted. **Downstream behavior:** thumbs-up/down rate, follow-up-question rate, session abandonment right after a response — your users voting with their clicks. And plainly, **the model version returned in every API response body's `model` field**, so a silent swap can't hide from you.",
      "Zoom out and these signals sort into four pillars — the same four the module's interactive widget walks through. **Traces** are the per-call record: prompt, response, tool calls, latency, token count, and model ID/version for one request. **Metrics** are those traces aggregated over time: p50/p95 latency, throughput, cost, error rate. **Quality Signals** are the layer traditional APM (Application Performance Monitoring) skips entirely: user feedback, hallucination rate, retrieval hit rate, and the answer-quality proxies above. **Alerts** are automated triggers firing when any of the above crosses a threshold. The stack below instruments all four in one pass.",
      { type: "illustration", label: "Minimum viable observability stack", content: `LOG on every request:
  input tokens · output tokens · retrieval scores · model name (from response body) · latency
...
ALERT when:
  ...
  model version in response body              ≠ expected
...
answer-length change within HOURS — not after weeks of support tickets.` },
    ...
    keyPoints: [
      ...
      "**Signals worth logging:** retrieval-score distribution, top-k keyword-match rate, answer-length distribution, format/citation compliance, user feedback, and the model version from the response body's `model` field.",
      "**Minimum viable stack:** log tokens + retrieval scores + the model field + latency on every request; sample ~5% for LLM-as-judge scoring; alert on distribution shifts and model-field mismatches.",
    ...
    recap: [
      ...
      "**Log the right things**: retrieval scores, answer-length + format compliance, user feedback signals, model version from the response body's `model` field.",
    ...
    mcqs: [
      ...
      {
        question: "A team confirms the inference provider's model version, read from the response body's `model` field, now differs from the version they expected. Which listed root cause does this match?",
        options: [
        ...
        correct: 1,
        explanation: "Option B is correct: a changed model-version value in the response body is the exact fingerprint of a silent model update — providers refresh weights behind a stable endpoint name without telling you, which is why logging the `model` field from every response body catches it. Option A is wrong: data drift comes from a stale index and shows up in retrieval-score shifts, not a model-version change. Option C is wrong: a traffic shift is about new query patterns and wouldn't change the model field in the response body. Option D is wrong: an embedding-API change is a separate dependency-drift cause; the model field here belongs to the generation model's response, not the embedding service.",
      },
    ],
    takeaway: "LLM systems degrade silently: zero errors and normal latency can still mean confidently wrong answers, because error rate measures failure, not truth. Instrument quality directly — log retrieval scores, answer-length distributions, and the model field from response bodies, and sample responses for lightweight judge scoring — and a silent regression surfaces in hours instead of weeks of support tickets.",
  },
  ...
  "latency-planner": {
    ...
    explanation: [
      ...
      "**Second, streaming** — and here's the reframe that changes everything. Even at that queued-up p95 tail, TTFT is still only a fraction of the full 5.4s — call it roughly 3s once the queue wait is folded in, just over half the total. A response that *starts* rendering at 3s doesn't feel like a 5.4s wait; it feels like a responsive tool that's actively working, and at the p50 case TTFT is faster still (~400ms). The wall-clock total latency is identical either way. What changed is that TTFT — not total time — became the number the user experiences. ==Streaming is almost always the highest-leverage improvement to *perceived* latency you can make without touching the model.==",
...
LEVERS (by leverage)
  ...
  p99 (8.2s)             → provisioned throughput endpoint  (infra change only)` },
    ...
    keyPoints: [
      ...
      "**Streaming attacks *perceived* latency:** a 5.4s response that starts rendering in a few seconds (not the full 5.4s) feels fast because the user experiences TTFT, not wall-clock total. Highest-leverage UX fix without a model swap.",
    ...
    mcqs: [
      {
        ...
        options: [
          ...
          "Streaming delivers the first tokens well before the full 5.4s completes, so users see content immediately instead of experiencing one long undifferentiated wait",
        ...
        correct: 1,
        explanation: "Option B is correct: in a streaming UI the felt latency is TTFT — the wait before the first character — so a tool that starts streaming well before the full 5.4s completes feels faster than a non-streaming tool that returns everything at 3.0s in one silent block. Option A is wrong: speculative decoding can cut real generation time, but nothing in this scenario indicates it's in use — the described cause is the early stream start, not a decoding-algorithm change. Option C is wrong: p95 doesn't systematically exclude network overhead, and being 'lower than actual' isn't a property of the percentile. Option D is wrong: a fixed animation would delay real content and make the tool feel slower, not faster.",
      },
      {
