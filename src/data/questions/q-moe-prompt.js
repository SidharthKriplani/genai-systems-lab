...
export const Q_MOE_PROMPT = [
  ...
  // ============================================================
  {
    ...
    options: ["A layer that runs the entire feed-forward network on every token, then averages the outputs","An ensemble of several fully-trained models whose logits are averaged at inference time","A layer with many parallel feed-forward 'expert' sub-networks, where a router selects a small subset of experts per token","A layer that mixes attention heads from different pretrained checkpoints"],
    correct: 2,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Total = every parameter in the model — all experts' weights plus the shared non-expert layers (attention, embeddings, norms); active = the shared layers plus the top-k selected experts actually used for a given token",
    ...
    keywords: [],
    explanation:
      "Total parameters counts every parameter in the model — every expert's weights plus the shared non-expert layers (attention, embeddings, norms) — all of which must be held in memory. Active parameters is the shared layers plus whichever top-k experts fire for a given token, a far smaller number actually run per forward pass. This gap is the whole point of MoE: large capacity, small per-token compute.",
  ...
  // ============================================================
  {
    ...
    options: ["~56B total, ~14B active","~7B total, ~2B active","~47B total, ~13B active","~47B total, ~47B active"],
    correct: 2,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["To make every expert produce identical outputs so routing does not matter","To reduce the total parameter count by pruning idle experts","To force top-1 routing regardless of the router's scores","To discourage the router from collapsing onto a few experts, spreading tokens more evenly so all experts get trained and hardware stays utilized"],
    correct: 3,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["The max number of experts a router may select; extra experts are ignored","The max tokens an expert can hold per batch; excess tokens are dropped and passed through via the residual connection without expert processing","The GPU memory ceiling; exceeding it silently increases active parameters","The number of layers an expert spans; exceeding it duplicates the expert"],
    correct: 1,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["The experts' outputs are concatenated and passed through a final projection","Only the single highest-scoring expert's output is kept; the rest are discarded","The outputs are averaged with equal weight regardless of router scores","A weighted sum of the selected experts' outputs, weighted by the router's (softmax) gating values"],
    correct: 3,
    keywords: [],
    explanation:
  ...
  // ============================================================
  {
    ...
    options: ["Lower memory footprint and lower compute per token","Higher compute per token but lower memory, since experts are pruned at inference","Slightly higher memory but no compute change, since routing adds parameters","Same memory footprint but much lower compute (FLOPs) per token, since only top-k experts activate"],
    correct: 3,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["No — MoE's advantage is compute-per-token; its total weights (and thus memory) are large, so it fits memory-constrained setups poorly","Yes — MoE reduces the weights that must be resident in memory","Yes — MoE's sparse activation reduces the working-set of weights touched per token, easing VRAM pressure","No — MoE increases per-token FLOPs, which is what a memory-constrained team wants to avoid"],
    correct: 0,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["Both reduce the same thing: total parameter count","Quantization reduces active parameters; MoE reduces bits per weight","MoE reduces active compute per token while keeping total weights large; quantization shrinks the memory/bytes per weight (and often bandwidth) without changing which weights run","Neither affects inference cost; both are training-only techniques"],
    correct: 2,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "A latency-sensitive, memory-tight single-GPU deployment where routing/gather-scatter overhead and holding all experts resident in VRAM outweigh the compute savings",
      "Training on a massive corpus where scaling capacity is the only goal",
      "Any case — dense models are strictly cheaper to serve than MoE regardless of deployment target",
    ],
    correct: 1,
    keywords: [],
    explanation:
      "MoE shines when compute-per-token is the constraint and you can afford large memory and expert-parallel infrastructure. On a memory-tight, single-device, latency-sensitive setup, the cost of storing all experts plus routing/gather-scatter overhead can make a right-sized dense model the better choice. (All-to-all communication specifically is a multi-GPU expert-parallel cost — a true single-GPU deployment with all experts co-resident has no cross-device communication at all.)",
    trap: "MoE is not universally cheaper. Routing/gather-scatter overhead and the memory to hold every expert can erase its compute advantage in small or latency-critical deployments — even without any cross-device communication cost.",
  },
  {
    id: "moe-l2-5",
    topic: "foundation-models",
    ...
    options: ["Because MoE grows total capacity (many experts) while keeping active FLOPs fixed, letting experts specialize — more knowledge stored, similar compute spent per token","Because MoE runs every expert on every token, giving more total FLOPs","Because MoE uses lower-precision weights, which are inherently more accurate","Because the router discards hard tokens, so the reported quality is inflated"],
    correct: 0,
    keywords: [],
    explanation:
  ...
  // ============================================================
  {
    ...
    options: ["Zero-shot uses a smaller model; few-shot uses a larger model","Zero-shot means no system prompt; few-shot means a long system prompt","Zero-shot fine-tunes the model on zero data; few-shot fine-tunes on a few samples","Zero-shot gives no task examples; few-shot includes a handful of worked input/output examples in the prompt"],
    correct: 3,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["Politely ask for JSON in prose and hope the model complies","Raise the temperature so the model explores more JSON variants","Provide an explicit schema/format, few-shot examples of valid JSON, and use constrained/structured decoding or a response_format when available","Ask for JSON inside a chain-of-thought so it reasons about the schema"],
    correct: 2,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["Because each example permanently updates the model's weights","Because more examples always monotonically improve accuracy regardless of content","Because examples change the tokenizer's vocabulary","Because in-context learning is sensitive to example selection, label distribution, and ordering (e.g., recency/position biases), which shift the model's conditional predictions"],
    correct: 3,
    keywords: [],
    explanation:
  ...
  // ============================================================
  {
    ...
    options: ["Always fine-tune; prompting never generalizes","Always few-shot; fine-tuning is obsolete","Use few-shot when you want fast, no-training iteration and have few examples; fine-tune when you have many labeled examples, need consistent behavior at lower per-call token cost, and prompting alone plateaus","They are interchangeable with identical cost and latency"],
    correct: 2,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["Fine-tuning — it is the only way to add any new facts","Prompt-stuffing all documents into every prompt — simplest and always cheapest","RAG — retrieve the current facts at query time and put them in context, so knowledge stays fresh without retraining and scales beyond the context window","None — models cannot use information they weren't trained on"],
    correct: 2,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["When the task is simple/well-covered by instruction-tuned behavior, context budget is tight, or example tokens add latency/cost without clear accuracy gains","Never — few-shot always outperforms zero-shot","Only when the model has fewer than 1B parameters","Only for classification, never for generation"],
    correct: 0,
    keywords: [],
    explanation:
  ...
  },
  {
    ...
    options: ["ReAct and CoT are identical; both only reason internally","CoT can call tools but ReAct cannot","Plain CoT reasons over parametric knowledge only (can hallucinate stale facts); ReAct interleaves actions/tool calls so it can fetch and ground on external, current information","ReAct is only for math, CoT only for retrieval"],
    correct: 2,
    keywords: [],
    explanation:
