...
export const Q_GAP_A = [
  ...
  // ── L0: Define (3) ─────────────────────────────────────────────────────────
  {
    ...
    options: ["Outcome eval runs on the training set and trajectory eval runs on the test set","Outcome eval is for LLMs and trajectory eval is for classifiers","They are two names for the same task-success metric","Outcome eval grades only the agent's final answer (pass/fail on the endpoint); trajectory eval grades each step of the agent's process — its plans, tool calls, and observations"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["How fast each tool returns a response","The total number of tools available to the agent","Whether the final answer cited a tool","Whether, at each step, the agent selected the right tool AND called it with the right arguments"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["It isn't — a wrong answer is a wrong answer, so the agent should be blamed","Trajectory eval would also mark the run PASS, hiding the wrong answer","Trajectory eval retries the API automatically","Trajectory eval shows every step was correct, localizing the fault to the environment (stale data / API error) rather than the agent's reasoning — so you fix the data source instead of wrongly debugging the agent"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["Because in real environments tools do fail, and a robust agent is one whose trajectory shows it DETECTING a failure and adapting (retry, fallback, ask the user) instead of barreling ahead as if it succeeded — recovery is a distinct, measurable capability","Because errors never happen in well-built agents, so recovery is irrelevant","Because recovering from errors always produces a correct final answer","Because error recovery is only relevant to outcome eval"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["The judge can be swayed by fluent-but-wrong traces, shows verbosity/position bias, and is non-deterministic; mitigate by anchoring it with golden examples, using a clear rubric, and spot-auditing its verdicts against human labels","The judge is deterministic and unbiased, so no mitigation is needed","The only pitfall is that it is slow; caching fixes everything","The judge always agrees with per-step assertions, so it is redundant"],
    correct: 0, keywords: [],
  ...
  // ── L2: Cross-concept / tradeoffs (5) ──────────────────────────────────────
  {
    ...
    options: ["Use only trajectory eval; outcome eval is obsolete once you score steps","Use only outcome eval in CI; trajectory eval is just for research papers","Use whichever is cheaper for the given task; they are interchangeable","Ship/track headline progress on outcome eval (does it ultimately work), but debug, gate releases, and catch silent-safety regressions on trajectory eval (why, whether it generalizes, whether it's safe) — you need both, playing different roles"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["Use per-step assertions against golden trajectories for the deterministic facts (exact tool/argument match, forbidden-tool checks — cheap, exact, reusable as a regression suite) and an LLM-as-judge with a rubric for the open-ended quality, anchored by goldens and human audits","Use LLM-as-judge for everything, since it is the most flexible","Use only exact assertions and drop all open-ended scoring","Score only the final answer, since per-step scoring is too noisy"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["With captured trajectories you can localize the regression to the exact step where a good run and a bad run diverged (e.g. tool selection changed at step 2), whereas an outcome-only log tells you it broke but not where or why","It doesn't — a pass/fail on the final answer is enough to find any regression","Trajectories only help for latency profiling, not correctness","Capturing trajectories makes debugging harder because there is more data"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["Efficiency is irrelevant as long as the answer is correct","Efficiency metrics are only for training, not evaluation","Steps-to-goal is identical to task success, so it adds nothing","Two agents can both reach the correct answer, but one that takes many redundant tool calls and steps is more expensive, slower, and more failure-prone in production; efficiency is a real quality axis, and redundant calls also signal a shakier trajectory even when the outcome passes"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["parse → clean/dedup → extract metadata → chunk → embed → index","embed → parse → chunk → index → clean","index → embed → chunk → clean → parse","chunk → embed → parse → index → clean"],
    correct: 0, keywords: [],
  ...
  // ── L1: Deep single-concept (5) ────────────────────────────────────────────
  {
    ...
    options: ["The embedding stage; a bigger embedding model would fix it","The index stage; rebuilding the index fixes garbled text","The parse stage: a naive reader linearized the two columns left-margin-to-right-margin, interleaving them; retrieval can only surface what ingestion wrote, so no better embeddings or higher top-k reconstruct meaning from already-garbled text — the fix is structure-aware parsing","The query path; rewriting the query fixes the parse"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["It's fine; permissions are purely a query-time concern","It only affects citation quality, not access control","The index only carries fields ingestion wrote, so without ACL metadata the retriever has nothing to filter on and can surface documents a user was never allowed to read — a data leak; retrieval filters before it ranks, so permissions must be attached at ingestion","It slows down embedding, so skipping it is a pure optimization"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Increase batch size so the full rebuild is faster; full rebuilds are unavoidable on any edit","Switch to a faster embedding model so re-embedding everything nightly is cheap","Incremental re-indexing: make the unit of work the document via a stable doc-id → chunk-ids map, re-embed only the edited doc's chunks, and upsert them in place — so cost is proportional to the change (tens of chunks), not the corpus (all 12,000 docs)","Cache query results so the stale index doesn't matter"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["The embedding model memorized the doc; retraining is required","The query path cached the doc; clearing the cache is the only fix","Incremental re-indexing handled edits but not deletes: nothing removed the deleted doc's chunks from the index, so they linger and keep being retrieved; the fix is to actively delete a doc's chunks (via the doc-id → chunk-ids map) on removal, and upsert-plus-delete on supersession","Deleted docs cannot be removed from a vector index once written"],
    correct: 2, keywords: [],
  ...
  // ── L2: Cross-concept / tradeoffs (5) ──────────────────────────────────────
  {
    ...
    options: ["Both are equally latency-critical, so expensive work should be split evenly","Retrieval is off the critical path, so expensive work belongs there","Expensive work should be avoided entirely; both pipelines must be cheap","Ingestion is throughput-bound and runs off the critical path (you pay per chunk once at write time); retrieval is latency-bound and runs per query — so push expensive work (rich parsing, metadata extraction, per-chunk summaries/synthetic questions) into ingestion, where you pay once and amortize it over every future query"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["You can — embedding models are interchangeable at query time","You only need to re-embed the queries, not the corpus","Query and document vectors are only comparable if produced by the same model (same vector space); a new-model query vector compared against old-model document vectors is meaningless and retrieval collapses, so you must re-embed (backfill) the whole corpus with the new model and rebuild the index — typically via a shadow/dual index cutover to validate before switching traffic","Just normalize the old vectors and they become compatible with the new model"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Dedup has no downside; always remove every similar document","Keeping duplicates always improves recall with no cost","Dedup removes redundant/near-duplicate content so retrieval isn't flooded with many near-identical chunks (improving diversity and effective top-k), but over-aggressive dedup can discard genuinely distinct docs that merely look similar (e.g. per-region policy variants), losing needed information; keeping everything preserves coverage but wastes index space and lets duplicates crowd out diverse results","Dedup only matters for storage cost, never for retrieval quality"],
    correct: 2, keywords: [],
