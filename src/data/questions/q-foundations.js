...
export const Q_FOUNDATIONS = [
  ...
  // ── L0: Define (3) ─────────────────────────────────────────────────────────
  {
    ...
    options: ["It converts raw text into a sequence of integer token IDs the model can consume, and back again","It compresses the model weights so they fit on a single GPU","It ranks retrieved documents by relevance before generation","It applies the softmax over the vocabulary to pick the next word"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["About 1,300 tokens — tokens are on average smaller than words","About 250 tokens — tokens are larger than words","About 1,000 tokens — one token per word","About 4,000 tokens — one token per character"],
    correct: 0, keywords: [],
  ...
  // ── L1: Deep single-concept (5) ────────────────────────────────────────────
  {
    ...
    options: ["It starts from individual characters/bytes and greedily merges the most frequent adjacent pair, repeating until it hits the target vocab size","It clusters word embeddings and keeps one token per cluster centroid","It splits text on whitespace and punctuation, then keeps the N most frequent words","It runs an EM algorithm that maximises the likelihood of a probabilistic segmentation"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["A larger vocab yields shorter sequences (fewer tokens per text) but a bigger, more sparsely-trained embedding/softmax table; a smaller vocab trains each token more but lengthens sequences and raises compute per text","Larger vocab always improves quality, so the only limit is disk space","Vocabulary size only affects decode speed, never training","Smaller vocabularies eliminate the out-of-vocabulary problem entirely"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["The model is slower on Hindi; upgrade the GPU","Hindi genuinely needs more tokens because it is more information-dense; nothing can be done","The tokenizer was trained mostly on English/Latin text, so Devanagari falls back to many byte- or character-level tokens; the fix is a tokenizer with adequate multilingual coverage (or a language-specific vocab)","Increase top_k retrieval so fewer tokens are needed"],
    correct: 2, keywords: [],
  ...
  // ── L2: Cross-concept / tradeoffs (5) ──────────────────────────────────────
  {
    ...
    options: ["BPE is character-level and WordPiece is word-level","BPE merges the most frequent adjacent pair; WordPiece merges the pair that most increases the corpus likelihood (highest score = freq(pair) / (freq(a)·freq(b)))","WordPiece merges the most frequent pair; BPE maximises likelihood","They are identical; only the vocab file format differs"],
    correct: 1, keywords: [],
  ...
  },
  {
    ...
    options: ["Is faster because it only stores merge rules","Can only be trained on whitespace-delimited languages","Guarantees a single deterministic segmentation, unlike BPE","Starts from a large candidate vocabulary and iteratively prunes tokens that least hurt corpus likelihood, and can produce multiple probabilistic segmentations of the same string (enabling subword regularization)"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["It removed the language-specific pre-tokenization/whitespace assumption by treating the raw input (including spaces, e.g. as '▁') as just another symbol, so it works uniformly on languages without spaces (Chinese, Japanese) and round-trips losslessly","It made vocabularies larger","It replaced subword tokens with word tokens","It is a different merge algorithm that beats BPE on quality"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["It combines subword efficiency with a 256-symbol byte base alphabet, so it can encode ANY Unicode string with zero out-of-vocabulary risk, while keeping sequences far shorter than character-level and avoiding word-level's OOV and huge-vocab problems","Byte-level uses the smallest vocabulary of the three","Character-level is strictly better but too slow to train","Word-level handles multilingual text best, so byte-level is only a fallback"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["The straight-line (Euclidean) distance between the vectors","The number of dimensions the two vectors share","The dot product divided by the number of dimensions","The cosine of the angle between the vectors — their directional alignment, ignoring magnitude — bounded in [-1, 1]"],
    correct: 3, keywords: [],
  ...
  // ── L1: Deep single-concept (5) ────────────────────────────────────────────
  {
    ...
    options: ["Predict the surrounding context words given a center word (so words appearing in similar contexts get similar vectors — the distributional hypothesis)","Reconstruct the exact input sentence from a compressed vector","Classify each word into a part-of-speech tag","Minimise the cosine distance between every pair of words in the vocabulary"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["The embeddings memorised a hardcoded analogy table","The model has true reasoning ability","It is a coincidence with no structure behind it","Certain semantic relationships are encoded as roughly linear (translation) directions in the vector space, so analogies can be solved with vector arithmetic"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["Static embeddings are too slow to compute at inference","Static embeddings cannot be stored in a vector database","Static embeddings assign one fixed vector per word, so polysemous words like 'bank' (river vs finance) collapse to a single averaged vector; contextual embeddings produce a different vector per occurrence based on the surrounding sentence","Static embeddings have too many dimensions"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Higher d captures more nuance and can raise ceiling accuracy, but increases memory, index size, and query latency, needs more data to train well, and past a point yields diminishing returns; too-low d underfits and can't separate concepts","Higher d is always better; the only cost is disk space","Dimension only affects training time, never retrieval quality","Lower d is always better because of the curse of dimensionality"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["word2vec — because it is fastest","Raw BERT token embeddings — because they are contextual","One-hot bag-of-words — because it is exact","Sentence embeddings (SBERT / a dedicated embedding model) — they output one calibrated vector per passage whose cosine distance reflects meaning; static embeddings lack context and have no good sentence-level vector, and raw BERT token embeddings aren't cosine-comparable and are O(n^2) to compare pairwise"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["Dense embeddings match on meaning, so a query can retrieve a passage that answers it even with zero shared words (synonyms, paraphrase); keyword search misses these — though in practice hybrid (dense + BM25) beats either alone, since lexical still wins on exact terms, codes, and rare entities","Embeddings are always strictly better than keyword search on every query","Embeddings are faster than an inverted index at all scales","Keyword search cannot be used with LLMs"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["ANN is exact and free — there is no tradeoff","ANN increases embedding dimension to speed things up","ANN trades a small amount of recall (it may miss some true nearest neighbours) for orders-of-magnitude faster, memory-bounded search; if recall drops too far, RAG silently retrieves worse context and answer quality degrades — so you tune the recall/latency knob (e.g. efSearch, nprobe) against your eval set","ANN only works if you switch from cosine to Euclidean distance"],
    correct: 2, keywords: [],
