...
export const Q_GAP_B = [
  ...
  // ── L0: Define (3) ─────────────────────────────────────────────────────────
  {
    ...
    options: ["To make the frontier model answer faster by giving it more GPUs","To encrypt queries before they reach the model provider","Not every query needs the frontier model — traffic is a difficulty distribution, so you send each query to the cheapest model that can still answer it correctly, saving cost and latency","To increase the temperature on hard queries and lower it on easy ones"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Send every query to all models in parallel and pick the best answer","Always use the large model, then summarise its answer with a small model","Try the small/cheap model first; only if a confidence check on its answer fails do you escalate to the larger model","Randomly pick a model for each query to balance load"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Choosing the cheapest model when everything is healthy","Compressing the model so it fits on cheaper hardware","Caching answers so repeated queries skip the model","When the primary model errors, times out, or is rate-limited, automatically falling back to a different provider's model or a degraded smaller model instead of returning an error to the user"],
    correct: 3, keywords: [],
  ...
  // ── L1: Deep single-concept (5) ────────────────────────────────────────────
  {
    ...
    options: ["A router decides *before* any answer exists (a cheap classifier commits the query up front — lowest latency, but a misroute has no recovery); a cascade decides *after* an attempt (small model first, escalate on a confidence check — more accurate, but escalated queries pay small+large serially)","There is no difference; the terms are interchangeable","A router decides after the large model answers; a cascade decides before any model runs","A cascade is always cheaper than a router on every query"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["Logprobs cannot be read from any model, so the cascade won't run","Logprobs always force escalation, so nothing is ever served cheaply","Models are often poorly calibrated and can be confidently wrong — a high-logprob but incorrect answer passes the check and never escalates, so the cascade silently returns wrong cheap answers on exactly the cases it should escalate","Logprobs only work for the large model, not the small one"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["The router should be a copy of the large model for maximum accuracy","The router must call every model once before deciding","The router should never use heuristics, only a fine-tuned LLM","The router must be much cheaper and faster than the models it routes to, or it defeats its own purpose — a router as expensive as the large model would erase the savings"],
    correct: 3, keywords: [],
  ...
  },
  {
    ...
    options: ["Logprobs are cheapest to read but miss confident-wrong answers (calibration failure); self-consistency (sample several times, escalate on disagreement) is more robust but multiplies the small model's cost; a verifier/judge is the most aligned with 'good enough' and most flexible but adds a call and can itself be wrong — so it's a cost-vs-reliability spectrum, often layered","They are interchangeable; pick any one","The verifier is always the cheapest because it's a small model","Self-consistency is free because it reuses one forward pass"],
    correct: 0, keywords: [],
  ...
  // ── L0: Define (3) ─────────────────────────────────────────────────────────
  {
    ...
    options: ["You must redact PII on the *input* (so it isn't spread into logs, vector stores, and third-party APIs) and on the *output* (so the model doesn't emit it to someone who shouldn't see it)","You only need to filter PII on the output, since input PII belongs to the user anyway","PII only needs filtering during model training, not at inference","Redaction is only needed if the model is fine-tuned"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["An input and/or output filter wrapping the model that inspects, redacts, or blocks content — e.g. blocking PII, secrets, or unsafe output before it reaches the user or a tool","A rate limiter that caps how many queries a user can send","A backup copy of the model on a second GPU","A cache that stores previous answers to save cost"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["The agent should be able to call only the tools it actually needs, and each tool should hold only the narrowest credential that lets it do its job — nothing more","Give the agent admin access so it never gets blocked mid-task","The agent should have no tools at all","The agent should share one credential across all tools for simplicity"],
    correct: 0, keywords: [],
  ...
  // ── L1: Deep single-concept (5) ────────────────────────────────────────────
  {
    ...
    options: ["System-prompt leakage (reciting proprietary instructions or embedded keys), cross-tenant / memorised-training-data leakage (surfacing another user's or a memorised record), and tool-mediated exfiltration (an agent smuggling secrets out via an outbound URL, email, or tool call)","Higher latency, higher cost, and more tokens","Prompt injection, jailbreaks, and role-play attacks","Slow responses, wrong answers, and hallucinations"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["Output guardrails are useless; only input guardrails matter","The guardrail should have been on the input side only","The output guardrail must also gate the model's *tool calls*, not just the user-facing reply — an agent's most dangerous output is executed (a tool call sending an email, fetching a URL, or writing to a DB), so a tool call carrying secret data out must be inspected before it runs, which a user-facing-only filter never sees","The agent simply needed a bigger model"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["It's fine as long as you add a system-prompt instruction telling the model not to delete anything","The agent needs even broader access to handle edge cases","The capability granted vastly exceeds the function performed, so any compromise inherits the whole blast radius (it could wipe production). Fix: scope the credential to the narrowest need (read-only, restricted to the specific bucket/path), allow-list only required tools, and require approval for irreversible actions","Nothing is wrong; agents should always have admin rights"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Sending a prompt containing EU user data to a model/provider hosted in another region is itself a data transfer — so residency constrains *which model, provider, and region* you may route a prompt to, and a third-party API in the wrong region can be a violation on its own","It doesn't; residency only applies to databases at rest","Residency only matters if you fine-tune the model","You can ignore residency as long as you delete the data afterward"],
    correct: 0, keywords: [],
  ...
  // ── L2: Cross-concept / tradeoffs (5) ──────────────────────────────────────
  {
    ...
    options: ["They are the same threat; injection defenses fully cover exfiltration","Exfiltration is impossible if you use a large enough model","Injection is bad data getting *in* to influence the model; exfiltration is sensitive data getting *out* through the model or its tools — mirror-image directions. Injection defenses filter/neutralise malicious input but do nothing to stop the model emitting PII, reciting its system prompt, surfacing another tenant's data, or smuggling secrets out via a tool call","Injection only affects training, exfiltration only affects inference"],
    correct: 2, keywords: [],
  ...
  },
  {
    ...
    options: ["Regex is fast/cheap and great for structured patterns (card numbers, API-key formats) but brittle and blind to context; a classifier model catches fuzzier PII/toxicity/secrets but adds cost and can misfire; an LLM judge is the most context-aware and flexible for 'off-policy' content but is slowest and can itself be wrong — so production usually layers them (cheap deterministic checks first, model-based checks for nuance)","Always use only regex, since it never has false positives","The LLM judge is always cheapest and most reliable","Classifiers are unnecessary if you have an LLM judge"],
    correct: 0, keywords: [],
  ...
  },
  {
    ...
    options: ["Because it makes the model itself immune to manipulation","Because scoped credentials make the model run faster","Because you cannot guarantee the model won't be manipulated or mistaken, so you shrink the blast radius by construction at the *tool* layer — even a fully-compromised agent can only do what its scoped permissions allow, turning a potential catastrophe into a bounded, often harmless, action","Because it eliminates the need for any output guardrails"],
    correct: 2, keywords: [],
