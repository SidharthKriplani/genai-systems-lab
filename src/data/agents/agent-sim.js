...
export const RUNNER_AGENT_SIM = {
  "agent-computer-use": {
    ...
    explanation: [
      ...
      "Contrast that with an **API/function-calling tool agent**. There the model emits a *structured* call — `submit_expense(id=4200, ...)` — against a typed contract, and the tool returns a *structured* result: `{status: 'ok', confirmation: 'EXP-88'}`. The agent knows unambiguously whether the action succeeded, what the new state is, and gets a stable identifier back. ==Computer use throws all of that away.== There is no return value — only the next screenshot, which the model must *re-perceive from scratch* to infer what happened. Every source of fragility in this module traces back to this one difference: **the feedback channel is a picture the model must re-interpret, not a fact the system hands it.**",
