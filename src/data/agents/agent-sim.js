    ...
    mcqs: [
      {
        ...
        options: [
          "Increase the screenshot resolution so the model grounds the button coordinate more precisely",
          "Attach a client-side idempotency key to the submit so a repeated action is a no-op, and add a human confirmation gate before the irreversible step",
          "Prompt the model to be more careful and to always double-check whether it already clicked",
          "Switch the vision model to a larger one so it reasons about page state more accurately",
        ],
        correct: 1,
        explanation: "Option B is correct: the root cause is that the screenshot is a lagging, non-transactional view of state the action already changed, so the model can re-perceive a stale frame and repeat an irreversible action. The durable fix is at the system level — an idempotency key makes a duplicate submit a no-op regardless of what the model perceives, and a confirmation gate keeps a human in the loop for money-movement. Option A is wrong: higher resolution helps grounding accuracy but does nothing about a *stale* frame — the model grounded correctly and clicked the right button twice. Option C is wrong because prompting cannot make a non-transactional feedback channel reliable; you cannot instruct away a race condition. Option D is wrong for the same reason as A/C — a better model still only sees a lagging screenshot; the problem is the missing transactional acknowledgment, which no model quality can supply.",
      },
      {
        question: "Why is a computer-use agent categorically more fragile than an equivalent API/function-calling agent for the same task?",
        options: [
          "Vision models are inherently less intelligent than text models, so their reasoning is worse",
          "Computer-use agents run more steps, and more steps always means proportionally more errors regardless of feedback type",
          "The feedback channel is a screenshot the model must re-perceive from scratch, with no structured return value or unambiguous acknowledgment — so grounding can miss, coordinates go stale across resolutions/reflows, and actions can't be transactionally confirmed",
          "API agents cannot perform irreversible actions, so they never need confirmation gates",
        ],
        correct: 2,
        explanation: "Option C is correct: the defining difference is the feedback channel. An API tool returns a typed, unambiguous result (status, new id), so the agent knows exactly what happened; a computer-use agent only gets the next screenshot and must re-infer state from pixels, which introduces grounding error (wrong/stale coordinates) and makes actions non-transactional. Every listed fragility traces to that. Option A is wrong — the fragility is architectural, not a claim that vision models are dumber; the same model behind an API contract is far more reliable. Option B is wrong because step count alone isn't the cause; an API chain of many steps stays reliable precisely because each step is structurally confirmed. Option D is wrong: API agents absolutely perform irreversible actions (sending email, moving money) and do need confirmation gates — that is not the distinction.",
      },
    ],
...
opens it once. Durability protects your OWN state; idempotency protects the WORLD.` },
    ...
    mcqs: [
      {
        question: "Your 6-hour migration agent was OOM-killed at step 180 and restarted from step 1, re-spending ~$60 of tokens AND re-opening PRs it had already opened. Which pair of mechanisms addresses BOTH problems?",
        options: [
          "A bigger worker pod (to avoid OOM) and a larger context window (to avoid overflow)",
          "Checkpointing state to durable storage so it resumes at step 180, plus idempotency keys on side effects so a repeated 'open PR' is a no-op",
          "Retrying the whole workflow automatically on failure, and lowering the temperature so the agent is more deterministic",
          "Summarizing the context every step, and adding a human approval gate before every action",
        ],
        correct: 1,
        explanation: "Option B is correct because the two problems have two distinct root causes needing two mechanisms. The wasted re-work + re-spend comes from restarting at step 1 with no persisted state — checkpointing fixes it by letting the agent rehydrate and resume at step 180. The duplicate PRs come from external side effects with no de-duplication — idempotency keys fix it so a repeated side effect is a no-op. Durability protects your own state; idempotency protects the outside world; you need both. Option A only reduces the chance of one interruption type and does nothing about resumability or duplicate side effects — interruptions (deploys, node rotations, timeouts) are inevitable regardless of pod size. Option C re-runs everything from zero (making the cost problem worse, not better) and temperature has no bearing on duplicate PRs. Option D helps context cost and safety but neither resumes from a checkpoint nor de-duplicates the PRs.",
      },
      {
        ...
        options: [
          "The model forgets its system prompt over time; re-injecting the system prompt every step fixes it",
          "Long histories always exceed rate limits; the fix is to slow down the request rate",
          "The ever-growing context eventually overflows the window and raises per-step latency/cost (the whole transcript is re-sent each call) while noise degrades quality — so compact old turns, externalize bulky results to pointers, and keep a durable scratchpad as the source of truth",
          "Nothing degrades as long as the model supports a large enough context window; just pick a 1M-token model",
        ],
        correct: 2,
        explanation: "Option C is correct: because each step re-sends the entire history, an ever-growing transcript overflows the window, makes every subsequent call slower and more expensive, and injects noise that hurts quality. The fix is active context management — summarize/compact older turns, externalize bulky tool results to storage and keep only a pointer + summary, and treat a durable scratchpad/task list as the source of truth rather than the raw message log. Option A misdiagnoses it as forgetting the system prompt, which isn't the accumulation problem. Option B is wrong — the issue is context size and cost, not request rate. Option D is wrong because a larger window delays but does not solve the problem: cost and latency still grow linearly with history, quality still degrades from noise, and hours-long runs can outgrow even huge windows — you must actively manage context, not just enlarge it.",
      },
    ],
    ...
    mcqs: [
      {
        ...
        options: [
          "Give the agent full autonomy over refunds so it resolves tickets faster and reduces human load",
          "Gate the refund tool behind a human-in-the-loop confirmation and make escalate-to-human the fallback on low confidence or a tripped guardrail — biasing errors toward the cheaper failure",
          "Remove the escalation path entirely so the agent is forced to resolve every ticket itself",
          "Increase the step cap so the agent can keep retrying the refund until it's confident",
        ],
        correct: 1,
        explanation: "Option B is correct: when one failure mode (wrong refund) is far costlier than another (unnecessary escalation), the design should bias errors toward the cheaper one. That means lowering autonomy on the high-stakes irreversible action — gate the refund behind human confirmation — and making escalate-to-human the safe fallback whenever the agent is uncertain or a guardrail trips. Cost asymmetry defined in step 1 of the framework is exactly what drives autonomy and fallback choices. Option A does the opposite, maximizing exposure to the expensive failure. Option C removes the very safety valve the asymmetry calls for, forcing the agent to act even when it shouldn't. Option D lets the agent keep attempting the dangerous action and raises cost/runaway risk without addressing the asymmetry — more retries on an irreversible action is more exposure, not less.",
      },
      {
        question: "A candidate designs the support agent by putting the entire knowledge base into the system prompt and giving the agent every internal tool 'so it's flexible.' What are the two clearest design errors?",
        options: [
          "Using RAG instead of a bigger prompt, and adding a step cap that limits flexibility",
          "Memorizing knowledge in the prompt instead of retrieving it via RAG (unupdatable, bloats context, degrades quality), and violating least-privilege by exposing every tool (needless attack/error surface, ungated irreversible actions)",
          "Choosing a single agent instead of a multi-agent system, and not using computer use for the tools",
          "Designing eval up front instead of after launch, and adding guardrails that slow responses",
        ],
        correct: 1,
        explanation: "Option B is correct: stuffing the whole KB into the prompt is the classic memory mistake — it can't be updated without redeploy, bloats context (raising cost/latency and lowering quality), and doesn't scale; the right pattern is RAG so knowledge is retrieved on demand and stays current. Giving the agent every tool violates least-privilege — each tool is attack and error surface, and exposing irreversible tools without confirmation gates is dangerous; the agent should get exactly the tools the task needs. Option A inverts both fixes (RAG and step caps are correct, not errors). Option C is not the issue — single-agent is the right default and computer use is worse than structured tools where APIs exist. Option D describes good practices (up-front eval, guardrails) as if they were mistakes.",
      },
    ],
...
fed results back. The model only decided WHAT to do and WHEN to stop.` },
    ...
    mcqs: [
      {
        question: "In the agent control loop, which statement correctly separates the roles of the model and the runtime?",
        options: [
          "The model executes tool calls directly and maintains state across turns; the runtime just displays the results to the user",
          "The model only decides the next action (or a terminal output) from the context it's shown; the stateless model is driven by a stateful runtime that actually executes tools, appends each observation to the history, and re-calls the model until a stopping condition fires",
          "The runtime decides which action to take and the model executes it against external systems",
          "There is no separation — the LLM is the entire agent and the loop is an implementation detail with no functional role",
        ],
        correct: 1,
        explanation: "Option B is correct: the model is a stateless component that only emits a decision (an action such as a tool call, or a terminal output) based on the accumulated context it's re-shown; it never executes anything itself. The runtime is the stateful harness that parses that output, actually executes the tool, appends the observation to the growing history, and calls the model again — repeating perceive→plan→act→observe until a stopping condition. Option A is wrong on both counts: the model does not execute tools and does not carry state across turns (memory lives in the appended context). Option C inverts the roles — the model decides, the runtime executes. Option D is wrong because the loop is the agent; collapsing it to 'just the LLM' erases tool execution, memory management, and termination, which are exactly what makes it an agent.",
      },
      {
        ...
        options: [
          "This is premature termination; fix it by lowering the step cap so it stops sooner",
          "This is non-termination / looping on a failing action; the runtime must enforce a step (max-iteration) cap and/or repeated-action detection, plus a budget cap — the model can't be trusted to self-terminate",
          "This is normal agent behavior and needs no guard; it will eventually succeed if given enough turns",
          "This is a memory overflow; fix it by clearing the context every turn so the agent forgets the error",
        ],
        correct: 1,
        explanation: "Option B is correct: repeating the same failing action forever is the classic non-termination/looping failure, and because the model cannot be relied on to break out on its own, the runtime must impose hard rails — a step (max-iteration) cap, ideally repeated-action detection, and a budget cap on tokens/dollars. These exist precisely so a stuck loop can't run up an unbounded bill. Option A misnames it as premature termination (which is the opposite problem — stopping too early) and a lower step cap is a blunt version of the right idea but the framing is wrong. Option C is dangerously wrong — an unadapting loop won't spontaneously succeed and will keep spending. Option D misdiagnoses it as memory overflow; clearing context each turn would destroy the agent's ability to learn from the error and wouldn't stop the loop.",
      },
    ],
