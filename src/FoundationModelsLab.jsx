        ...
        {FM_SCENARIOS.map(s => {
          ...
          return (
            <button key={s.id} onClick={() => switchScenario(s.id)}
              ...
              style={active ? { borderLeft: "2px solid var(--gal-build)" } : {}}>
              <span className={`text-xs font-semibold leading-snug ${active ? "text-white" : "text-zinc-300"}`}>{s.title}</span>
              <span className="text-[9px] font-mono" style={{ color: active ? "var(--gal-build)" : undefined }} >{s.tag}</span>
            </button>
          );
        })}
        <div className="mt-4 mx-3 p-3 rounded-lg border border-zinc-800/60" style={{ background: "var(--gal-build-tint)" }}>
        ...
        <div className="p-5 sm:p-7 space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[9px] font-mono px-2 py-0.5 rounded" style={{ background: "var(--gal-build-tint)", border: "1px solid var(--gal-build-border)", color: "var(--gal-build)" }}>{scenario.tag}</span>
            </div>
              ...
              {scenario.configs.map((cfg, i) => {
                ...
                return (
                  <button key={i} onClick={() => setSelectedConfig(i)}
                    ...
                    style={{ background: selected ? "var(--gal-build-tint)" : "var(--surface-2)", border: selected ? "1.5px solid var(--gal-build-border)" : undefined }}>
          ...
          {outcome && (
            <div className={`rounded-xl border p-5 space-y-4 ${st.border}`} style={{ background: st.bg }}>
              ...
              {/* Synthesis close */}
              <div className="p-3 rounded-lg" style={{ border: "1px solid var(--gal-build-border)", background: "var(--gal-build-tint)" }}>
                <div className="text-[9px] font-mono uppercase tracking-widest mb-1.5" style={{ color: "var(--gal-build)" }}>THE DESIGN PRINCIPLE</div>
